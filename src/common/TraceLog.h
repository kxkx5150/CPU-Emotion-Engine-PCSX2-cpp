...
class BaseTraceLogSource {
  ...
  public:
    ...
    {
        return 0;    //(m_Descriptor->Description != NULL) ? pxGetTranslation(m_Descriptor->Description) :
                     //wxEmptyString;
    }
...
class TextFileTraceLog : public BaseTraceLogSource {
  public:
    TextFileTraceLog(const TraceLogDescriptor *desc) : BaseTraceLogSource(desc)
    {
    }
