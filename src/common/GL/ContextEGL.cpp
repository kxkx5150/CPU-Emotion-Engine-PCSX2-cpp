...
{
    m_wi.display_connection = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    m_display =
        m_wi.display_connection;    // eglGetDisplay(static_cast<EGLNativeDisplayType>(m_wi.display_connection));
    if (!m_display) {
...
bool ContextEGL::CreateSurface()
{
    ...
    SDL_GetWindowWMInfo(sdlwindow, &sysInfo);

    m_display = eglGetDisplay(EGL_DEFAULT_DISPLAY);
    m_surface = eglCreateWindowSurface(m_display, m_config, (EGLNativeWindowType)sysInfo.info.x11.window, 0);
    if (!m_surface) {
...
bool ContextEGL::CreateContext(const Version &version, EGLContext share_context)
{
    ...
    surface_attribs[nsurface_attribs++] =
    ...
    if (!eglBindAPI((version.profile == Profile::ES) ? EGL_OPENGL_ES_API : EGL_OPENGL_API)) {
        Console.Error("eglBindAPI(%s) failed",
