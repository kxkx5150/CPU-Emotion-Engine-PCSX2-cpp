...
{
    static const TraceLogDescriptor myDesc = {L"p&xThread", L"pxThread",
...
class StaticMutex : public Mutex {
  ...
  public:
    StaticMutex(bool &deletedFlag) : m_DeletedFlag(deletedFlag)
    {
    }
...
{
    pxAssumeDev(!tkl_destructed,
                "total_key_lock is destroyed; program is shutting down; cannot create new thread key.");
...
bool Threading::_WaitGui_RecursionGuard(const wxChar *name)
{
    ...
    if (!guard.IsReentrant())
        return false;
    pxThreadLog.Write(pxGetCurrentThreadName(), pxsFmt(L"Yield recursion in %s; opening modal dialog.", name));
    return true;
}
...
Threading::pxThread::pxThread(const wxString &name)
    : m_name(name), m_thread(), m_native_id(0), m_native_handle(0),
...
bool Threading::pxThread::AffinityAssert_AllowFromSelf(const DiagnosticOrigin &origin) const
{
    ...
    if (IsDevBuild)
        pxOnAssert(origin,
                   pxsFmt(L"Thread affinity violation: Call allowed from '%s' thread only.", WX_STR(GetName())));
...
bool Threading::pxThread::AffinityAssert_DisallowFromSelf(const DiagnosticOrigin &origin) const
{
    ...
    if (IsDevBuild)
        pxOnAssert(origin,
                   pxsFmt(L"Thread affinity violation: Call is *not* allowed from '%s' thread.", WX_STR(GetName())));
...
void Threading::pxThread::Start()
{
    ...
    if (pthread_create(&m_thread, NULL, _internal_callback, this) != 0)
        throw Exception::ThreadCreationError(this).SetDiagMsg(L"Thread creation error: " +
                                                              wxString(std::strerror(errno)));
    ...
    if (!m_sem_startup.WaitWithoutYield(wxTimeSpan(0, 0, 0, 100))) {
        if (m_sem_startup.Count() == 0)
            throw Exception::ThreadCreationError(this).SetDiagMsg(
                L"Thread creation error: %s thread never posted startup semaphore.");
    }
#else
...
void Threading::pxThread::_selfRunningTest(const wxChar *name) const
{
    ...
    if (!m_running) {
        throw Exception::CancelEvent(pxsFmt(
            L"Blocking thread %s was terminated while another thread was waiting on a %s.", WX_STR(GetName()), name));
    }
