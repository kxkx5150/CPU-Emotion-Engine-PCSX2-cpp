...
SysThreadBase::SysThreadBase() : m_ExecMode(ExecMode_NoThreadYet), m_ExecModeMutex()
{
}
...

    {
        ...
        switch (m_ExecMode.load()) {
            ...
            case ExecMode_Paused:
                if (!isBlocking)
                    throw Exception::CancelEvent(
                        L"Cannot suspend in non-blocking fashion: Another thread is pausing the VM state.");
