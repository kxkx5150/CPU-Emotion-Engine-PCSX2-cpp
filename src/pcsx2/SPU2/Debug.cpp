...
//     it's still better to outright not call it on tight loop scenarios, by testing MsgToConsole() (which is inline and
//     very quick). Else, there's some (small) overhead in calling and returning from ConLog.
        ...
        for (c = 0; c < 2; c++) {
            ...
            for (v = 0; v < 24; v++) {
                ...
                fprintf(dump,
                        ...
                        "     - Phase: %x\n"
                        "     - Value: %x\n",
                        Cores[c].Voices[v].ADSR.regADSR1, Cores[c].Voices[v].ADSR.regADSR2,
                        Cores[c].Voices[v].ADSR.AttackRate, Cores[c].Voices[v].ADSR.AttackMode,
                        Cores[c].Voices[v].ADSR.DecayRate, Cores[c].Voices[v].ADSR.SustainLevel,
                        Cores[c].Voices[v].ADSR.SustainRate, Cores[c].Voices[v].ADSR.SustainMode,
                        Cores[c].Voices[v].ADSR.ReleaseRate, Cores[c].Voices[v].ADSR.ReleaseMode,
                        Cores[c].Voices[v].ADSR.Phase, Cores[c].Voices[v].ADSR.Value);
