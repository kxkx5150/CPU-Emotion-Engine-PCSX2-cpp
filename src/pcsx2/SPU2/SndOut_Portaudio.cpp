...
class Portaudio : public SndOutModule {
  private:
    ...
    class SampleReader {
      public:
        virtual int ReadSamples(const void *inputBuffer, void *outputBuffer, unsigned long framesPerBuffer,
                                const PaStreamCallbackTimeInfo *timeInfo, PaStreamCallbackFlags statusFlags,
                                void *userData) = 0;
    };

    template <class T> class ConvertedSampleReader : public SampleReader {
        int *written;
  ...
  public:
    ...
    s32 Init()
    {
        ...
        if (deviceIndex >= 0) {
                ...
                case 7:
                    switch (dplLevel) {
                        case 0:
                            ...
                            ActualPaCallback =
                                new ConvertedSampleReader<Stereo51Out32>(&writtenSoFar);    //"normal" stereo upmix
                            break;
                        case 1:
                            ConLog("* SPU2 > 5.1 speaker expansion with basic ProLogic dematrixing enabled.\n");
                            ActualPaCallback = new ConvertedSampleReader<Stereo51Out32Dpl>(
                                &writtenSoFar);    // basic Dpl decoder without rear stereo balancing
                            break;
                        case 2:
                            ConLog(
                                "* SPU2 > 5.1 speaker expansion with experimental ProLogicII dematrixing enabled.\n");
                            ActualPaCallback =
                                new ConvertedSampleReader<Stereo51Out32DplII>(&writtenSoFar);    // gigas PLII
                            break;
                    }
...
#ifdef _WIN32
            PaWasapiStreamInfo info = {sizeof(PaWasapiStreamInfo), paWASAPI, 1, paWinWasapiExclusive};
            ...
            err = Pa_OpenStream(&stream, nullptr, &outParams, SampleRate, SndOutPacketSize, paNoFlag, PaCallback,
            ...
            if (err == paInvalidSampleRate &&
                SampleRate == 96000)    // It didn't work, so lets just put the samplerate back
                SampleRate = 48000;
        } else {
            err = Pa_OpenDefaultStream(&stream, 0, actualUsedChannels, paInt32, 48000, SndOutPacketSize, PaCallback,
                                       nullptr);
        }
        if (err != paNoError) {
            if (err == paInvalidSampleRate)
                Console.Warning("Failed to create Port Audio Device %dkhz, Please use Exclusive Mode",
                                SampleRate / 1000);
            fprintf(stderr, "* SPU2: PortAudio error: %s\n", Pa_GetErrorText(err));
            Pa_Terminate();
            return -1;
...
////////////////////////////////////////////////////////////////
  ...
  private:
                ...
                if (idx > 0) {
                    ...
                    for (int j = 0; j < Pa_GetDeviceCount(); j++) {
                        ...
                        if (info->hostApi == api_idx && info->maxOutputChannels > 0) {
                            SendMessage(GetDlgItem(hWnd, IDC_PA_DEVICE), CB_ADDSTRING, 0,
                                        (LPARAM)wxString::FromUTF8(info->name).wc_str());
                ...
                switch (wmId) {
                    case IDOK: {
                        ...
                        const PaDeviceInfo *info =
                            (const PaDeviceInfo *)SendMessage(GetDlgItem(hWnd, IDC_PA_DEVICE), CB_GETITEMDATA, idx, 0);
                        if (info)
                        ...
                        m_SuggestedLatencyMinimal =
                            SendMessage(GetDlgItem(hWnd, IDC_MINIMIZE), BM_GETCHECK, 0, 0) == BST_CHECKED;

                        m_WasapiExclusiveMode =
                            SendMessage(GetDlgItem(hWnd, IDC_EXCLUSIVE), BM_GETCHECK, 0, 0) == BST_CHECKED;
    ...
    static BOOL CALLBACK DSEnumCallback(LPGUID lpGuid, LPCTSTR lpcstrDescription, LPCTSTR lpcstrModule,
                                        LPVOID lpContext);
...
int PaCallback(const void *inputBuffer, void *outputBuffer, unsigned long framesPerBuffer,
               const PaStreamCallbackTimeInfo *timeInfo, PaStreamCallbackFlags statusFlags, void *userData)
{
    return PA.ActualPaCallback->ReadSamples(inputBuffer, outputBuffer, framesPerBuffer, timeInfo, statusFlags,
                                            userData);
}
