...
// Base algorithm: aim at specific average number of samples at the buffer (by GUI), and adjust tempo simply by
// current/target. An extra mechanism is added to keep adjustment at perfect 1:1 ratio (when emulation speed is stable
// around 100%)
//   to prevent constant stretching/shrinking of packets if possible.
//   This mechanism is triggered when the adjustment is close to 1:1 for long enough (defaults to 100 iterations within
//   hys_ok_factor - defaults to 3%). 1:1 state is aborted when required adjustment goes beyond hys_bad_factor (defaults
//   to 20%).
//
...
//   This algorithm stabilises when the actual latency is <speed>*<required_latency>. While this is just fine at 100%
//   speed, it's problematic especially for slow speeds, as the number of actual samples at the buffer gets very small
//   on that case, which may lead to underruns (or just too much latency when running very fast).
// To compensate for that, the algorithm has a slowly moving compensation factor which will eventually bring the actual
// latency to the required one. compensationDivider defines how slow this compensation changes. By default it's set to
// 100,
//   which will finalize the compensation after about 200 iterations.
//
//  Note, this algorithm is intentionally simplified by not taking extreme actions at extreme scenarios (mostly
//  underruns when speed drops sharply),
//   and let's the overrun/underrun protections do what they should (doesn't happen much though in practice, even at big
//   FPS variations).
//
//   These params were tested to show good respond and stability, on all audio systems (dsound, wav, port audio,
//   xaudio2),
//     even at extreme small latency of 50ms which can handle 50%-100% variations without audible glitches.

int targetIPS = 750;

// Dynamic tuning changes the values of the base algorithm parameters (derived from targetIPS) to adapt, in real time,
// to
//   different number of invocations/sec (mostly affects number of iterations to average).
//   Dynamic tuning can have a slight negative effect on the behavior of the algorithm, so it's preferred to have it
//   off.
...
// If changes are made to SPU2 which affects this number (but it's still the same on all systems), then just change
// targetIPS. If we find out that some systems are very different, we can turn on dynamic tuning by uncommenting the
// next line. #define NEWSTRETCHER_USE_DYNAMIC_TUNING
...
template <class T> bool IsInRange(const T &val, const T &min, const T &max)
{
...
void SndBuffer::UpdateTempoChangeSoundTouch2()
{
    ...
    {    // test current iterations/sec every 0.5s, and change algo params accordingly if different than previous IPS
         // more than 30%
        static long       iters = 0;
    ...
    int         hys_min_ok_count    = GetClamped((int)(50.0 * (float)targetIPS / 750.0), 2,
                                                 100);    // consecutive iterations within hys_ok before going to 1:1 mode
    ...
    if (IsInRange(tempoAdjust, 0.9f, 1.1f) &&
        IsInRange(dynamicTargetFullness, baseTargetFullness * 0.9f, baseTargetFullness * 1.1f))
        dynamicTargetFullness = baseTargetFullness;
    ...
    if (MsgOverruns()) {
        ...
        if (delta.GetMilliseconds() > 1000) {    // report buffers state and tempo adjust every second
            ConLog("buffers: %4d ms (%3.0f%%), tempo: %f, comp: %2.3f, iters: %d, (N-IPS:%d -> avg:%d, minokc:%d, "
                   "div:%d) reset:%d\n",
                   (int)(data / 48), (double)(100.0 * bufferFullness / baseTargetFullness), (double)tempoAdjust,
                   (double)(dynamicTargetFullness / baseTargetFullness), iters, (int)targetIPS, AVERAGING_WINDOW,
...
void SndBuffer::UpdateTempoChangeSoundTouch()
{
    ...
    if (cTempo < 0.965f || cTempo > 1.060f || pctChange < -0.38f || pctChange > 0.54f || statusPct < -0.42f ||
        statusPct > 0.70f || eTempo < 0.89f || eTempo > 1.19f) {
        // printf("Emergency stretch: cTempo = %f eTempo = %f pctChange = %f statusPct =
...
void SndBuffer::timeStretchWrite()
{
    ...
    int tempProgress;
    while (tempProgress = pSoundTouch->receiveSamples((float *)sndTempBuffer, SndOutPacketSize), tempProgress != 0) {
