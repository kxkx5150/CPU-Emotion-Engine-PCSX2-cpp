...
void V_Core::UpdateEffectsBufferSize()
{
    ...
    // EffectsEndA, newbufsize * 2, newbufsize * 2, FxEnable);
...
__forceinline void UpdateSpdifMode()
{
    ...
    if (OPM != PlayMode) {
        ConLog("* SPU2: Play Mode Set to %s (%d).\n",
               (PlayMode == 0)
                   ? "Normal"
                   : ((PlayMode == 1) ? "PCM Clone" : ((PlayMode == 2) ? "PCM Bypass" : "BitStream Bypass")),
               PlayMode);
    }
}
...
void V_Core::WriteRegPS1(u32 mem, u16 value)
{
    ...
    if ((reg >= 0x1c00) && (reg < 0x1d80)) {
        ...
        switch (reg) {
            ...
            case 0x1d9e:    //         // Voice 15..23 ON/OFF (status) (ENDX) (R) // writeable but hw overrides it
                            //         shortly after
                // Regs.ENDX &= 0xffff;
            ...
            case 0x1dae:    // 1F801DAEh - SPU Status Register (SPUSTAT) (R)
                            // The SPUSTAT register should be treated read-only (writing is possible in so far that the
                            // written value can be read-back for a short moment, however, thereafter the hardware is
                            // overwriting that value).
                            // Regs.STATX = value;
                break;
...
template <int CoreIdx, int VoiceIdx, int param> static void __fastcall RegWrite_VoiceParams(u16 value)
{
    ...
    switch (param) {
        ...
        case 4:    // ADSR2 (Envelope)
            ...
            // REG_VP_ENVX, REG_VP_VOLXL and REG_VP_VOLXR have been confirmed to not be allowed to be written to, so
            // code has been commented out. Colin McRae Rally 2005 triggers case 5 (ADSR), but it doesn't produce issues
            // enabled or disabled.
...
template <int CoreIdx, int VoiceIdx, int address> static void __fastcall RegWrite_VoiceAddr(u16 value)
{
    const int core  = CoreIdx;
    const int voice = VoiceIdx;
...
template <int CoreIdx, int cAddr> static void __fastcall RegWrite_Core(u16 value)
{
    ...
    switch (omem) {
        ...
        case REG_C_ATTR: {
            ...
            if (thiscore.IRQEnable != irqe) {
                ...
                //	core, ((thiscore.IRQEnable==0)?"disabled":"enabled"), Cycles, thiscore.IRQA,
                //thiscore.EffectsStartA);
...
template <int CoreIdx, int addr> static void __fastcall RegWrite_CoreExt(u16 value)
{
...
template <int core, int addr> static void __fastcall RegWrite_Reverb(u16 value)
{
...
template <int addr> static void __fastcall RegWrite_SPDIF(u16 value)
{
    *(regtable[addr >> 1]) = value;
    UpdateSpdifMode();
}

template <int addr> static void __fastcall RegWrite_Raw(u16 value)
{
    *(regtable[addr >> 1]) = value;
}
...
#define VoiceParamsSet(core, voice)                                                                                    \
    RegWrite_VoiceParams<core, voice, 0>, RegWrite_VoiceParams<core, voice, 1>, RegWrite_VoiceParams<core, voice, 2>,  \
        RegWrite_VoiceParams<core, voice, 3>, RegWrite_VoiceParams<core, voice, 4>,                                    \
        RegWrite_VoiceParams<core, voice, 5>, RegWrite_VoiceParams<core, voice, 6>,                                    \
        RegWrite_VoiceParams<core, voice, 7>
...
#define VoiceAddrSet(core, voice)                                                                                      \
    RegWrite_VoiceAddr<core, voice, 0>, RegWrite_VoiceAddr<core, voice, 1>, RegWrite_VoiceAddr<core, voice, 2>,        \
        RegWrite_VoiceAddr<core, voice, 3>, RegWrite_VoiceAddr<core, voice, 4>, RegWrite_VoiceAddr<core, voice, 5>


#define CoreParamsPair(core, omem) RegWrite_Core<core, omem>, RegWrite_Core<core, ((omem) + 2)>

#define ReverbPair(core, mem) RegWrite_Reverb<core, mem>, RegWrite_Core<core, ((mem) + 2)>
...
static RegWriteHandler *const tbl_reg_writes[0x401] = {
    ...
    CoreParamsPair(0, REG_S_PMON), CoreParamsPair(0, REG_S_NON), CoreParamsPair(0, REG_S_VMIXL),
    CoreParamsPair(0, REG_S_VMIXEL), CoreParamsPair(0, REG_S_VMIXR), CoreParamsPair(0, REG_S_VMIXER),

    RegWrite_Core<0, REG_P_MMIX>, RegWrite_Core<0, REG_C_ATTR>,

    CoreParamsPair(0, REG_A_IRQA), CoreParamsPair(0, REG_S_KON), CoreParamsPair(0, REG_S_KOFF),
    CoreParamsPair(0, REG_A_TSA), CoreParamsPair(0, REG__1AC),

    RegWrite_Core<0, REG_S_ADMAS>, REGRAW(0x1b2),

    REGRAW(0x1b4), REGRAW(0x1b6), REGRAW(0x1b8), REGRAW(0x1ba), REGRAW(0x1bc), REGRAW(0x1be),

    // 0x1c0!

    VoiceAddrSet(0, 0), VoiceAddrSet(0, 1), VoiceAddrSet(0, 2), VoiceAddrSet(0, 3), VoiceAddrSet(0, 4),
    VoiceAddrSet(0, 5), VoiceAddrSet(0, 6), VoiceAddrSet(0, 7), VoiceAddrSet(0, 8), VoiceAddrSet(0, 9),
    VoiceAddrSet(0, 10), VoiceAddrSet(0, 11), VoiceAddrSet(0, 12), VoiceAddrSet(0, 13), VoiceAddrSet(0, 14),
    VoiceAddrSet(0, 15), VoiceAddrSet(0, 16), VoiceAddrSet(0, 17), VoiceAddrSet(0, 18), VoiceAddrSet(0, 19),
    VoiceAddrSet(0, 20), VoiceAddrSet(0, 21), VoiceAddrSet(0, 22), VoiceAddrSet(0, 23),
    ...
    // 0x346 here
    REGRAW(0x346), REGRAW(0x348), REGRAW(0x34A), REGRAW(0x34C), REGRAW(0x34E), REGRAW(0x350), REGRAW(0x352),
    REGRAW(0x354), REGRAW(0x356), REGRAW(0x358), REGRAW(0x35A), REGRAW(0x35C), REGRAW(0x35E), REGRAW(0x360),
    REGRAW(0x362), REGRAW(0x364), REGRAW(0x366), REGRAW(0x368), REGRAW(0x36A), REGRAW(0x36C), REGRAW(0x36E),
    REGRAW(0x370), REGRAW(0x372), REGRAW(0x374), REGRAW(0x376), REGRAW(0x378), REGRAW(0x37A), REGRAW(0x37C),
    REGRAW(0x37E), REGRAW(0x380), REGRAW(0x382), REGRAW(0x384), REGRAW(0x386), REGRAW(0x388), REGRAW(0x38A),
    REGRAW(0x38C), REGRAW(0x38E), REGRAW(0x390), REGRAW(0x392), REGRAW(0x394), REGRAW(0x396), REGRAW(0x398),
    REGRAW(0x39A), REGRAW(0x39C), REGRAW(0x39E), REGRAW(0x3A0), REGRAW(0x3A2), REGRAW(0x3A4), REGRAW(0x3A6),
    REGRAW(0x3A8), REGRAW(0x3AA), REGRAW(0x3AC), REGRAW(0x3AE), REGRAW(0x3B0), REGRAW(0x3B2), REGRAW(0x3B4),
    REGRAW(0x3B6), REGRAW(0x3B8), REGRAW(0x3BA), REGRAW(0x3BC), REGRAW(0x3BE), REGRAW(0x3C0), REGRAW(0x3C2),
    REGRAW(0x3C4), REGRAW(0x3C6), REGRAW(0x3C8), REGRAW(0x3CA), REGRAW(0x3CC), REGRAW(0x3CE), REGRAW(0x3D0),
    REGRAW(0x3D2), REGRAW(0x3D4), REGRAW(0x3D6), REGRAW(0x3D8), REGRAW(0x3DA), REGRAW(0x3DC), REGRAW(0x3DE),
    REGRAW(0x3E0), REGRAW(0x3E2), REGRAW(0x3E4), REGRAW(0x3E6), REGRAW(0x3E8), REGRAW(0x3EA), REGRAW(0x3EC),
    REGRAW(0x3EE), REGRAW(0x3F0), REGRAW(0x3F2), REGRAW(0x3F4), REGRAW(0x3F6), REGRAW(0x3F8), REGRAW(0x3FA),
    REGRAW(0x3FC), REGRAW(0x3FE),

    // AND... we reached 0x400!
    // Last verse, same as the first:

    VoiceParamsCore(1),    // 0x000 -> 0x180
    CoreParamsPair(1, REG_S_PMON), CoreParamsPair(1, REG_S_NON), CoreParamsPair(1, REG_S_VMIXL),
    CoreParamsPair(1, REG_S_VMIXEL), CoreParamsPair(1, REG_S_VMIXR), CoreParamsPair(1, REG_S_VMIXER),

    RegWrite_Core<1, REG_P_MMIX>, RegWrite_Core<1, REG_C_ATTR>,

    CoreParamsPair(1, REG_A_IRQA), CoreParamsPair(1, REG_S_KON), CoreParamsPair(1, REG_S_KOFF),
    CoreParamsPair(1, REG_A_TSA), CoreParamsPair(1, REG__1AC),

    RegWrite_Core<1, REG_S_ADMAS>, REGRAW(0x5b2),

    REGRAW(0x5b4), REGRAW(0x5b6), REGRAW(0x5b8), REGRAW(0x5ba), REGRAW(0x5bc), REGRAW(0x5be),

    // 0x1c0!

    VoiceAddrSet(1, 0), VoiceAddrSet(1, 1), VoiceAddrSet(1, 2), VoiceAddrSet(1, 3), VoiceAddrSet(1, 4),
    VoiceAddrSet(1, 5), VoiceAddrSet(1, 6), VoiceAddrSet(1, 7), VoiceAddrSet(1, 8), VoiceAddrSet(1, 9),
    VoiceAddrSet(1, 10), VoiceAddrSet(1, 11), VoiceAddrSet(1, 12), VoiceAddrSet(1, 13), VoiceAddrSet(1, 14),
    VoiceAddrSet(1, 15), VoiceAddrSet(1, 16), VoiceAddrSet(1, 17), VoiceAddrSet(1, 18), VoiceAddrSet(1, 19),
    VoiceAddrSet(1, 20), VoiceAddrSet(1, 21), VoiceAddrSet(1, 22), VoiceAddrSet(1, 23),
    ...
    REGRAW(0x746), REGRAW(0x748), REGRAW(0x74A), REGRAW(0x74C), REGRAW(0x74E), REGRAW(0x750), REGRAW(0x752),
    REGRAW(0x754), REGRAW(0x756), REGRAW(0x758), REGRAW(0x75A), REGRAW(0x75C), REGRAW(0x75E),
    ...
    REGRAW(0x7B0), REGRAW(0x7B2), REGRAW(0x7B4), REGRAW(0x7B6), REGRAW(0x7B8), REGRAW(0x7BA), REGRAW(0x7BC),
    REGRAW(0x7BE),
    ...
    REGRAW(0x7CE), REGRAW(0x7D0), REGRAW(0x7D2), REGRAW(0x7D4), REGRAW(0x7D6), REGRAW(0x7D8), REGRAW(0x7DA),
    REGRAW(0x7DC), REGRAW(0x7DE), REGRAW(0x7E0), REGRAW(0x7E2), REGRAW(0x7E4), REGRAW(0x7E6), REGRAW(0x7E8),
    REGRAW(0x7EA), REGRAW(0x7EC), REGRAW(0x7EE), REGRAW(0x7F0), REGRAW(0x7F2), REGRAW(0x7F4), REGRAW(0x7F6),
...
void StartVoices(int core, u32 value)
{
    ...
    for (u8 vc = 0; vc < V_Core::NumVoices; vc++) {
        ...
        if (IsDevBuild) {
            ...
            if (MsgKeyOnOff())
                ConLog(
                    "* SPU2: KeyOn: C%dV%02d: SSA: %8x; M: %s%s%s%s; H: %04x; P: %04x V: %04x/%04x; ADSR: %04x%04x\n",
                    core, vc, thisvc.StartA, (Cores[core].VoiceGates[vc].DryL) ? "+" : "-",
                    (Cores[core].VoiceGates[vc].DryR) ? "+" : "-", (Cores[core].VoiceGates[vc].WetL) ? "+" : "-",
                    (Cores[core].VoiceGates[vc].WetR) ? "+" : "-", *(u16 *)GetMemPtr(thisvc.StartA), thisvc.Pitch,
                    thisvc.Volume.Left.Value >> 16, thisvc.Volume.Right.Value >> 16, thisvc.ADSR.regADSR1,
                    thisvc.ADSR.regADSR2);
        }
    }
}
