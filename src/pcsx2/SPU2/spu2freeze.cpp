...
s32 __fastcall SPU2Savestate::FreezeIt(DataBlock &spud)
{
    ...
    pxAssertMsg(spu2regs && _spu2mem, "Looks like PCSX2 is trying to savestate while components are shut down.  That's "
                                      "a no-no! It shouldn't crash, but the savestate will probably be corrupted.");
...
s32 __fastcall SPU2Savestate::ThawIt(DataBlock &spud)
{
    if (spud.spu2id != SAVE_ID || spud.version < SAVE_VERSION) {
        ...
        pxAssertMsg(spu2regs && _spu2mem,
                    "Looks like PCSX2 is trying to loadstate while components are shut down.  That's a no-no!  It "
                    "shouldn't crash, but the savestate will probably be corrupted.");
