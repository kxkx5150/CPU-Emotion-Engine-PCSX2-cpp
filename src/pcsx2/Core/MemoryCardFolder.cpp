...
{
    // this should be a good enough arbitrary check, if someone can think of a case where this doesn't work feel free to
    // change
...
void FolderMemoryCard::Open(const wxString &fullPath, const Pcsx2Config::McdOptions &mcdOptions,
                            const u32 sizeInClusters, const bool enableFiltering, const wxString &filter,
                            bool simulateFileWrites)
{
    ...
    if (mcdOptions.Enabled && mcdOptions.Type == MemoryCardType::Folder) {
        ...
        // if the user has disabled this slot or is using a different memory card type, just return without a console
        // log
        return;
    }
...
void FolderMemoryCard::LoadMemoryCardData(const u32 sizeInClusters, const bool enableFiltering, const wxString &filter)
{
    ...
    if (superBlockFileName.FileExists()) {
        ...
        if (superBlockFile.IsOpened() &&
            superBlockFile.Read(&m_superBlock.raw, sizeof(m_superBlock.raw)) >= sizeof(m_superBlock.data)) {
    ...
    if (formatted) {
        ...
        WriteToFile(m_folderName.GetFullPath().RemoveLast() + L"-debug_" +
                    wxDateTime::Now().Format(L"%Y-%m-%d-%H-%M-%S") + L"_load.ps2");
#endif
    }
}
...
void FolderMemoryCard::CreateFat()
{
    ...
    const u32 countFatClusters     = (totalClusters % fatEntriesPerCluster) != 0
                                         ? (totalClusters / fatEntriesPerCluster + 1)
                                         : (totalClusters / fatEntriesPerCluster);
...
void FolderMemoryCard::CreateRootDir()
{
    ...
    rootCluster->entries[0].entry.data.mode = MemoryCardFileEntry::Mode_Read | MemoryCardFileEntry::Mode_Write |
                                              MemoryCardFileEntry::Mode_Execute | MemoryCardFileEntry::Mode_Directory |
                                              MemoryCardFileEntry::Mode_Unknown0x0400 | MemoryCardFileEntry::Mode_Used;
    ...
    rootCluster->entries[1].entry.data.mode = MemoryCardFileEntry::Mode_Write | MemoryCardFileEntry::Mode_Execute |
                                              MemoryCardFileEntry::Mode_Directory |
                                              MemoryCardFileEntry::Mode_Unknown0x0400 |
                                              MemoryCardFileEntry::Mode_Unknown0x2000 | MemoryCardFileEntry::Mode_Used;
...
bool FolderMemoryCard::AddFolder(MemoryCardFileEntry *const dirEntry, const wxString &dirPath,
                                 MemoryCardFileMetadataReference *parent, const bool enableFiltering,
    ...
    if (wxDir::Exists(dirPath)) {
        ...
        for (const auto &file : GetOrderedFiles(dirPath)) {
            ...
            if (file.m_isFile) {
                ...
                // this has the effective result of only files relevant to the current game being loaded into the memory
                // card which means every game essentially sees the memory card as if no other files exist
                ...
                const u32 newNeededClusters = CalculateRequiredClustersOfDirectory(dirPath + L"/" + file.m_fileName) +
                                              ((dirEntry->entry.data.length % 2) == 0 ? 1 : 0);
...
bool FolderMemoryCard::AddFile(MemoryCardFileEntry *const dirEntry, const wxString &dirPath,
                               const EnumeratedFileEntry &fileEntry, MemoryCardFileMetadataReference *parent)
{
    ...
    if (file.IsOpened()) {
        ...
        const u32 countClusters =
            (filesize % clusterSize) != 0 ? (filesize / clusterSize + 1) : (filesize / clusterSize);
...
FolderMemoryCard::AddDirEntryToMetadataQuickAccess(MemoryCardFileEntry *const             entry,
                                                   MemoryCardFileMetadataReference *const parent)
{
...
MemoryCardFileMetadataReference *
FolderMemoryCard::AddFileEntryToMetadataQuickAccess(MemoryCardFileEntry *const             entry,
                                                    MemoryCardFileMetadataReference *const parent)
{
    ...
    do {
        ...
        ++clusterNumber;
    } while ((fileCluster = m_fat.data[0][0][fileCluster & NextDataClusterMask]) !=
             (LastDataCluster | DataClusterInUseMask));
...
MemoryCardFileEntryCluster *FolderMemoryCard::GetFileEntryCluster(const u32 currentCluster, const u32 searchCluster,
                                                                  const u32 fileCount)
{
    ...
    if (it != m_fileEntryDict.end()) {
        ...
        for (unsigned int i = 0; i < filesInThisCluster; ++i) {
            ...
            if (entry->IsValid() && entry->IsUsed() && entry->IsDir() && !entry->IsDotDir()) {
                ...
                MemoryCardFileEntryCluster *ptr =
                    GetFileEntryCluster(entry->entry.data.cluster, searchCluster, newFileCount);
                if (ptr != nullptr) {
                    return ptr;
                }
            }
...
MemoryCardFileEntry *FolderMemoryCard::GetFileEntryFromFileDataCluster(const u32 currentCluster,
                                                                       const u32 searchCluster, wxFileName *fileName,
                                                                       const size_t originalDirCount,
                                                                       u32         *outClusterNumber)
{
    ...
    const u32 nextCluster = m_fat.data[0][0][currentCluster] & NextDataClusterMask;
    if (nextCluster != LastDataCluster) {
        MemoryCardFileEntry *ptr =
            GetFileEntryFromFileDataCluster(nextCluster, searchCluster, fileName, originalDirCount, outClusterNumber);
        if (ptr != nullptr) {
            return ptr;
        }
    }
    ...
    for (int i = 0; i < 2; ++i) {
        MemoryCardFileEntry *const entry = &m_fileEntryDict[currentCluster].entries[i];
        if (entry->IsValid() && entry->IsUsed() && entry->IsDir() && !entry->IsDotDir()) {
            MemoryCardFileEntry *ptr = GetFileEntryFromFileDataCluster(entry->entry.data.cluster, searchCluster,
                                                                       fileName, originalDirCount, outClusterNumber);
            if (ptr != nullptr) {
...
void FolderMemoryCard::Flush()
{
...
#ifdef DEBUG_WRITE_FOLDER_CARD_IN_MEMORY_TO_FILE_ON_CHANGE
    WriteToFile(m_folderName.GetFullPath().RemoveLast() + L"-debug_" + wxDateTime::Now().Format(L"%Y-%m-%d-%H-%M-%S") +
                L"_pre-flush.ps2");
#endif
    ...
    // Keep a copy of the old file entries so we can figure out which files and directories, if any, have been deleted
    // from the memory card.
    std::vector<MemoryCardFileEntryTreeNode> oldFileEntryTree;
    if (IsFormatted()) {
        CopyEntryDictIntoTree(&oldFileEntryTree, m_superBlock.data.rootdir_cluster,
                              m_fileEntryDict[m_superBlock.data.rootdir_cluster].entries[0].entry.data.length);
    }
    ...
    if (m_backupBlock2.programmedBlock != 0xFFFFFFFFu) {
        Console.Warning(L"(FolderMcd) Aborting flush of slot %u, emulation was interrupted during save process!",
                        m_slot);
        return;
    }
    ...
    // Now we have the new file system, compare it to the old one and "delete" any files that were in it before but
    // aren't anymore.
            ...
            if (entry->IsDir()) {
                if (!entry->IsDotDir()) {
                    ...
                    if (m_performFileWrites) {
                        ...
                        if (filenameCleaned || entry->entry.data.mode != MemoryCardFileEntry::DefaultDirMode ||
                            entry->entry.data.attr != 0) {
...
void FolderMemoryCard::FlushDeletedFilesAndRemoveUnchangedDataFromCache(
    const std::vector<MemoryCardFileEntryTreeNode> &oldFileEntries)
{
...
void FolderMemoryCard::FlushDeletedFilesAndRemoveUnchangedDataFromCache(
    const std::vector<MemoryCardFileEntryTreeNode> &oldFileEntries, const u32 newCluster, const u32 newFileCount,
    ...
    for (auto it = oldFileEntries.cbegin(); it != oldFileEntries.cend(); ++it) {
        ...
        if (entry->IsValid() && entry->IsUsed() && !entry->IsDotDir()) {
            ...
            if (newEntry == nullptr) {
                ...
                const wxString subDirPath = dirPath + L"/" + subDirName;
                FlushDeletedFilesAndRemoveUnchangedDataFromCache(it->subdir, newEntry->entry.data.cluster,
                                                                 newEntry->entry.data.length, subDirPath);
            } else if (entry->IsFile()) {
...
void FolderMemoryCard::RemoveUnchangedDataFromCache(const MemoryCardFileEntry *const oldEntry,
                                                    const MemoryCardFileEntry *const newEntry)
{
    // Disclaimer: Technically, to actually prove that file data has not changed and still belongs to the same file,
    // we'd need to keep a copy of the old FAT cluster chain and compare that as well, and only acknowledge the file as
    // unchanged if none of those have changed. However, the chain of events that leads to a file having the exact same
    // file contents as a deleted old file while also being placed in the same data clusters as the deleted file AND
    // matching this condition here, in a quick enough succession that no flush has occurred yet since the deletion of
    // that old file is incredibly unlikely, so I'm not sure if it's actually worth coding for.
    if (oldEntry->entry.data.timeModified != newEntry->entry.data.timeModified ||
        oldEntry->entry.data.timeCreated != newEntry->entry.data.timeCreated ||
        oldEntry->entry.data.length != newEntry->entry.data.length ||
        oldEntry->entry.data.cluster != newEntry->entry.data.cluster) {
        return;
    }
...
void FolderMemoryCard::CopyEntryDictIntoTree(std::vector<MemoryCardFileEntryTreeNode> *fileEntryTree, const u32 cluster,
                                             const u32 fileCount)
{
...
const MemoryCardFileEntry *FolderMemoryCard::FindEquivalent(const MemoryCardFileEntry *searchEntry, const u32 cluster,
                                                            const u32 fileCount)
{
    const MemoryCardFileEntryCluster *entryCluster = &m_fileEntryDict[cluster];
    u32                               fileCluster  = cluster;
    ...
    for (size_t i = 0; i < fileCount; ++i) {
        ...
        if (entry->IsValid() && entry->IsUsed()) {
            if (entry->IsFile() == searchEntry->IsFile() && entry->IsDir() == searchEntry->IsDir() &&
                strncmp((const char *)searchEntry->entry.data.name, (const char *)entry->entry.data.name,
                        sizeof(entry->entry.data.name)) == 0) {
                return entry;
            }
        }
    ...
    if (dir.IsOpened()) {
        ...
        while (hasNext) {
            ...
            if (wxFile::Exists(fileInfo.GetFullPath())) {
                ...
                EnumeratedFileEntry entry{
                    fileName, getOptionalNodeAttribute(node, "timeCreated", creationTime.GetTicks()),
                ...
                const YAML::Node indexForDirectory =
                    LoadYAMLFromFile(wxFileName(fileInfo.GetFullPath(), "_pcsx2_index").GetFullPath());
...
void FolderMemoryCard::CalculateECC(u8 *ecc, const u8 *data)
{
    static const u8 Table[] = {
        };
...
wxFFile *FileAccessHelper::Open(const wxFileName &folderName, MemoryCardFileMetadataReference *fileRef,
                                bool writeMetadata)
{
...
void FileAccessHelper::WriteMetadata(wxFileName folderName, const MemoryCardFileMetadataReference *fileRef)
{
    ...
    const bool  metadataIsNonstandard =
        cleanedFilename || entry->data.mode != MemoryCardFileEntry::DefaultFileMode || entry->data.attr != 0;
...
void FileAccessHelper::WriteIndex(wxFileName folderName, MemoryCardFileEntry *const entry,
                                  MemoryCardFileMetadataReference *const parent)
{
...
wxFFile *FileAccessHelper::ReOpen(const wxFileName &folderName, MemoryCardFileMetadataReference *fileRef,
                                  bool writeMetadata)
{
    std::string internalPath;
    fileRef->GetInternalPath(&internalPath);
