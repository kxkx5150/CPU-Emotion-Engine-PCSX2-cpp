...
wxString SaveStateBase::GetFilename(int slot)
{
    ...
    if (serialName.IsEmpty())
        serialName = L"BIOS";
    return (EmuFolders::Savestates + pxsFmt(L"%s (%08X).%02d.p2s", WX_STR(serialName), ElfCRC, slot)).GetFullPath();
}
...
SaveStateBase &SaveStateBase::FreezeBios()
{
    ...
    if (bioscheck != BiosChecksum) {
        ...
        Console.Indent(2).Error("Current BIOS:   %ls (crc=0x%08x)\n"
                                "Savestate BIOS: %s (crc=0x%08x)\n",
                                BiosDescription.wx_str(), BiosChecksum, biosdesc, bioscheck);
    }
    return *this;
}
static const uint MainMemorySizeInBytes =
    Ps2MemSize::MainRam + Ps2MemSize::Scratch + Ps2MemSize::Hardware + Ps2MemSize::IopRam + Ps2MemSize::IopHardware;
...
memSavingState::memSavingState(SafeArray<u8> &save_to) : SaveStateBase(save_to)
{
}
memSavingState::memSavingState(SafeArray<u8> *save_to) : SaveStateBase(save_to)
{
...
// --------------------------------------------------------------------------------------
memLoadingState::memLoadingState(const SafeArray<u8> &load_from) : SaveStateBase(const_cast<SafeArray<u8> &>(load_from))
{
}
memLoadingState::memLoadingState(const SafeArray<u8> *load_from) : SaveStateBase(const_cast<SafeArray<u8> *>(load_from))
{
...
class SavestateEntry_VU0prog : public MemorySavestateEntry {
  public:
    ...
    wxString GetFilename() const
    {
        return L"vu0MicroMem.bin";
    }
    u8 *GetDataPtr() const
    {
        return vuRegs[0].Micro;
    }
    uint GetDataSize() const
    {
        return VU0_PROGSIZE;
...
class SavestateEntry_VU1prog : public MemorySavestateEntry {
  public:
    ...
    wxString GetFilename() const
    {
        return L"vu1MicroMem.bin";
    }
    u8 *GetDataPtr() const
    {
        return vuRegs[1].Micro;
    }
    uint GetDataSize() const
    {
        return VU1_PROGSIZE;
...
class SavestateEntry_SPU2 : public BaseSavestateEntry {
  public:
    ...
    void FreezeIn(pxInputStream &reader) const
    {
        return SysState_ComponentFreezeIn(reader, SPU2);
    }
    void FreezeOut(SaveStateBase &writer) const
    {
        return SysState_ComponentFreezeOut(writer, SPU2);
    }
...
//     void FreezeIn(pxInputStream &reader) const
//     {
//         return SysState_ComponentFreezeIn(reader, USB);
//     }
//     void FreezeOut(SaveStateBase &writer) const
//     {
//         return SysState_ComponentFreezeOut(writer, USB);
//     }
//     bool IsRequired() const
//     {
//         return true;
...
class SavestateEntry_PAD : public BaseSavestateEntry {
  public:
    ...
    void FreezeIn(pxInputStream &reader) const
    {
        return SysState_ComponentFreezeIn(reader, PAD);
    }
    void FreezeOut(SaveStateBase &writer) const
    {
        return SysState_ComponentFreezeOut(writer, PAD);
    }
    bool IsRequired() const
    {
        return true;
...
class SavestateEntry_GS : public BaseSavestateEntry {
  public:
    ...
    void FreezeIn(pxInputStream &reader) const
    {
        return SysState_ComponentFreezeIn(reader, GS);
    }
    void FreezeOut(SaveStateBase &writer) const
    {
        return SysState_ComponentFreezeOut(writer, GS);
    }
    bool IsRequired() const
    {
        return true;
...
static const std::unique_ptr<BaseSavestateEntry> SavestateEntries[] = {
    ...
    // std::unique_ptr<BaseSavestateEntry>(std::nullptr_t),
...
static void  CheckVersion(pxInputStream &thr)
{
    ...
    if (savever > g_SaveVersion)
        throw Exception::SaveStateLoadError(thr.GetStreamName())
            .SetDiagMsg(
                pxsFmt(L"Savestate uses an unsupported or unknown savestate version.\n(PCSX2 ver=%x, state ver=%x)",
                       g_SaveVersion, savever))
            .SetUserMsg(wxString("Cannot load this savestate. The state is an unsupported version."));
...
static void ZipStateToDiskOnThread(std::unique_ptr<ArchiveEntryList>    srclist,
                                   std::unique_ptr<wxFFileOutputStream> outbase, wxString filename, wxString tempfile)
{
#ifndef PCSX2_CORE
    ...
    if (!wxRenameFile(out->GetStreamName(), filename, true)) {
        Console.Error("Failed to rename save state '%s' to '%s'",
                      static_cast<const char *>(out->GetStreamName().c_str()),
                      static_cast<const char *>(filename.c_str()));
#ifndef PCSX2_CORE
        // Msgbox::Alert(_("The savestate was not properly saved. The temporary file was created successfully but could
        // not be moved to its final resting place."));
#endif
    } else {
...
void SaveState_UnzipFromDisk(const wxString &filename)
{
    ...
    if (!reader->IsOk()) {
        throw Exception::SaveStateLoadError(filename)
            ...
            .SetUserMsg(
                wxString("This savestate cannot be loaded because it is not a valid gzip archive.  It may have been "
                         "created by an older unsupported version of PCSX2, or it may be corrupted."));
    }
    ...
    if (!foundVersion || !foundInternal) {
        throw Exception::SaveStateLoadError(filename)
            ...
            .SetUserMsg(wxString("This file is not a valid PCSX2 savestate.  See the logfile for details."));
    }
    ...
    VmStateBuffer buffer(foundInternal->GetSize(),
                         L"StateBuffer_UnzipFromDisk");    // start with an 8 meg buffer to avoid frequent reallocation.
