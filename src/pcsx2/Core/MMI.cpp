...
void MADD()
{
    ...
    if (_Rd_)
        cpuRegs.GPR.r[_Rd_].SD[0] = cpuRegs.LO.SD[0];
}
...
void MADD1()
{
    ...
    if (_Rd_)
        cpuRegs.GPR.r[_Rd_].SD[0] = cpuRegs.LO.SD[1];
}
...
__fi void _PEXT5(int n)
{
    cpuRegs.GPR.r[_Rd_].UL[n] =
        ((cpuRegs.GPR.r[_Rt_].UL[n] & 0x0000001F) << 3) | ((cpuRegs.GPR.r[_Rt_].UL[n] & 0x000003E0) << 6) |
        ((cpuRegs.GPR.r[_Rt_].UL[n] & 0x00007C00) << 9) | ((cpuRegs.GPR.r[_Rt_].UL[n] & 0x00008000) << 16);
}
...
{
    cpuRegs.GPR.r[_Rd_].UL[n] =
        ((cpuRegs.GPR.r[_Rt_].UL[n] >> 3) & 0x0000001F) | ((cpuRegs.GPR.r[_Rt_].UL[n] >> 6) & 0x000003E0) |
        ((cpuRegs.GPR.r[_Rt_].UL[n] >> 9) & 0x00007C00) | ((cpuRegs.GPR.r[_Rt_].UL[n] >> 16) & 0x00008000);
}
...
static __fi void _PMADDW(int dd, int ss)
{
    ...
    if (ss == 0) {
        if (((cpuRegs.GPR.r[_Rt_].SL[ss] & 0x7FFFFFFF) == 0 ||
             (cpuRegs.GPR.r[_Rt_].SL[ss] & 0x7FFFFFFF) == 0x7FFFFFFF) &&
...
void PHMADH()
{    // JayteeMaster: changed a bit to avoid screw up. Also used 0,2,4,6 instead of 0,1,2,3
    _PHMADH_LO(0, 0);
    _PHMADH_HI(0, 2);
    _PHMADH_LO(2, 4);
    _PHMADH_HI(2, 6);
