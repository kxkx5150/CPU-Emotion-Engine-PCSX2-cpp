...
#define IOPCNT_INT_TOGGLE                                                                                              \
    (1 << 7)    // 0x80  0=Pulse (reset on read), 1=toggle each interrupt condition (in 1 shot not reset after fired)
...
void psxRcntInit()
{
    ...
    psxCounters[6].rate =
        768 * 12;    // 12 SPU ticks. 768 would be ideal but some games slow down internally for some reason
...
static bool __fastcall _rcntFireInterrupt(int i, bool isOverflow)
{
    ...
    if (psxCounters[i].mode & 0x400) {    // IRQ fired
        // DevCon.Warning("Counter %d %s IRQ Fired count %x", i, isOverflow ? "Overflow" : "Target",
        // psxCounters[i].count);
...
static void __fastcall _rcntTestTarget(int i)
{
    ...
    // PSXCNT_LOG("IOP Counter[%d] target 0x%I64x >= 0x%I64x (mode: %x)", i, psxCounters[i].count,
...
static __fi void _rcntTestOverflow(int i)
{
    ...
    // PSXCNT_LOG("IOP Counter[%d] overflow 0x%I64x >= 0x%I64x (mode: %x)", i, psxCounters[i].count, maxTarget,
    //            psxCounters[i].mode);
...
void psxRcntUpdate()
{
    ...
    for (i = 0; i <= 5; i++) {
        ...
        if ((psxCounters[i].mode & 0x40) &&
            !(psxCounters[i].mode &
              0x80)) {    // Repeat IRQ mode Pulsed, resets a few cycles after the interrupt, this should do.
...
void psxRcntWcount32(int index, u32 value)
{
    ...
    if ((psxCounters[index].mode & 0x400) ||
        (psxCounters[index].mode & 0x40)) {    // IRQ not triggered (one shot) or toggle
...
__fi void psxRcntWmode16(int index, u32 value)
{
    ...
    // if (value & (1 << 7)) {
    //     PSXCNT_LOG("16 Counter %d Toggle IRQ on %s", index,
    //                (irqmode & 3) == 1 ? "Target" : ((irqmode & 3) == 2 ? "Overflow" : "Target and Overflow"));
    // } else {
    //     PSXCNT_LOG("16 Counter %d Pulsed IRQ on %s", index,
    //                (irqmode & 3) == 1 ? "Target" : ((irqmode & 3) == 2 ? "Overflow" : "Target and Overflow"));
    // }
    // if (!(value & (1 << 6))) {
...
__fi void psxRcntWmode32(int index, u32 value)
{
    ...
    // if (value & (1 << 7)) {
    //     PSXCNT_LOG("32 Counter %d Toggle IRQ on %s", index,
    //                (irqmode & 3) == 1 ? "Target" : ((irqmode & 3) == 2 ? "Overflow" : "Target and Overflow"));
    // } else {
    //     PSXCNT_LOG("32 Counter %d Pulsed IRQ on %s", index,
    //                (irqmode & 3) == 1 ? "Target" : ((irqmode & 3) == 2 ? "Overflow" : "Target and Overflow"));
    // }
    // if (!(value & (1 << 6))) {
...
void psxRcntWtarget16(int index, u32 value)
{
    ...
    if (psxCounters[index].target <= psxRcntCycles(index) ||
        ((psxCounters[index].mode & 0x400) == 0 && !(psxCounters[index].mode & 0x40)))
        psxCounters[index].target |= IOPCNT_FUTURE_TARGET;
...
u64 psxRcntCycles(int index)
{
    ...
    return (u64)(psxCounters[index].count +
                 (u32)((psxRegs.cycle - psxCounters[index].sCycleT) / psxCounters[index].rate));
}
