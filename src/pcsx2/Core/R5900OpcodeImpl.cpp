...
static int __Deci2Call(int call, u32 *addr)
{
    ...
    switch (call) {
        case 1:    // open
            if (addr != NULL) {
                deci2addr = addr[1];
                // BIOS_LOG("deci2open: %x,%x,%x,%x", addr[3], addr[2], addr[1], addr[0]);
                deci2handler = addr[2];
            } else {
                deci2handler = 0;
        ...
        case 3:    // reqsend
        {
            ...
            //          ((addr == NULL) ? "NULL" : reqaddr), d2ptr[7], d2ptr[6], d2ptr[5], d2ptr[4], d2ptr[3], d2ptr[2],
            //          d2ptr[1], d2ptr[0]);
...
void MMI_Unknown()
{
    Console.Warning("Unknown MMI opcode called");
}
void COP0_Unknown()
{
    Console.Warning("Unknown COP0 opcode called");
}
void COP1_Unknown()
{
    Console.Warning("Unknown FPU/COP1 opcode called");
}
...
}    // Rt = Rs And Im (zero-extended)
void ORI()
{
    if (!_Rt_)
        return;
    cpuRegs.GPR.r[_Rt_].UD[0] = cpuRegs.GPR.r[_Rs_].UD[0] | (u64)_ImmU_;
}    // Rt = Rs Or  Im (zero-extended)
void XORI()
{
    if (!_Rt_)
        return;
    cpuRegs.GPR.r[_Rt_].UD[0] = cpuRegs.GPR.r[_Rs_].UD[0] ^ (u64)_ImmU_;
}    // Rt = Rs Xor Im (zero-extended)
void SLTI()
{
    if (!_Rt_)
        return;
...
void ADDU()
{
    if (!_Rd_)
        return;
    cpuRegs.GPR.r[_Rd_].UD[0] = cpuRegs.GPR.r[_Rs_].SL[0] + cpuRegs.GPR.r[_Rt_].SL[0];
}    // Rd = Rs + Rt
void DADDU()
{
    if (!_Rd_)
        return;
    cpuRegs.GPR.r[_Rd_].UD[0] = cpuRegs.GPR.r[_Rs_].SD[0] + cpuRegs.GPR.r[_Rt_].SD[0];
}
void SUBU()
{
    if (!_Rd_)
...
static const u64 LDL_MASK[8] = {0x00ffffffffffffffULL, 0x0000ffffffffffffULL, 0x000000ffffffffffULL,
                                0x00000000ffffffffULL, 0x0000000000ffffffULL, 0x000000000000ffffULL,
                                0x00000000000000ffULL, 0x0000000000000000ULL};
static const u64 LDR_MASK[8] = {0x0000000000000000ULL, 0xff00000000000000ULL, 0xffff000000000000ULL,
...
static const u64 SDL_MASK[8] = {0xffffffffffffff00ULL, 0xffffffffffff0000ULL, 0xffffffffff000000ULL,
                                0xffffffff00000000ULL, 0xffffff0000000000ULL, 0xffff000000000000ULL,
                                0xff00000000000000ULL, 0x0000000000000000ULL};
static const u64 SDR_MASK[8] = {0x0000000000000000ULL, 0x00000000000000ffULL, 0x000000000000ffffULL,
...
void SDL()
{
    ...
    memRead64(addr & ~7, &mem);
    mem = (cpuRegs.GPR.r[_Rt_].UD[0] >> SDL_SHIFT[shift]) | (mem & SDL_MASK[shift]);
...
void SDR()
{
    ...
    memRead64(addr & ~7, &mem);
    mem = (cpuRegs.GPR.r[_Rt_].UD[0] << SDR_SHIFT[shift]) | (mem & SDR_MASK[shift]);
    memWrite64(addr & ~7, &mem);
}
    ...
    switch (static_cast<Syscall>(call)) {
        case Syscall::SetGsCrt: {
            ...
            switch (cpuRegs.GPR.n.a1.UC[0]) {
                case 0x0:
                case 0x2:
                    mode = "NTSC 640x448 @ 59.940 (59.82)";
                    gsSetVideoMode(GS_VideoMode::NTSC);
                    break;

                case 0x1:
                case 0x3:
                    mode = "PAL  640x512 @ 50.000 (49.76)";
                    gsSetVideoMode(GS_VideoMode::PAL);
                    break;

                case 0x1A:
                    mode = "VESA 640x480 @ 59.940";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x1B:
                    mode = "VESA 640x480 @ 72.809";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x1C:
                    mode = "VESA 640x480 @ 75.000";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x1D:
                    mode = "VESA 640x480 @ 85.008";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;

                case 0x2A:
                    mode = "VESA 800x600 @ 56.250";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x2B:
                    mode = "VESA 800x600 @ 60.317";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x2C:
                    mode = "VESA 800x600 @ 72.188";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x2D:
                    mode = "VESA 800x600 @ 75.000";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x2E:
                    mode = "VESA 800x600 @ 85.061";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;

                case 0x3B:
                    mode = "VESA 1024x768 @ 60.004";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x3C:
                    mode = "VESA 1024x768 @ 70.069";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x3D:
                    mode = "VESA 1024x768 @ 75.029";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x3E:
                    mode = "VESA 1024x768 @ 84.997";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;

                case 0x4A:
                    mode = "VESA 1280x1024 @ 63.981";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;
                case 0x4B:
                    mode = "VESA 1280x1024 @ 79.976";
                    gsSetVideoMode(GS_VideoMode::VESA);
                    break;

                case 0x50:
                    mode = "SDTV   720x480 @ 59.94";
                    gsSetVideoMode(GS_VideoMode::SDTV_480P);
                    break;
                case 0x51:
                    mode = "HDTV 1920x1080 @ 60.00";
                ...
                case 0x72:
                    mode = "DVD NTSC 640x448 @ ??.???";
                    gsSetVideoMode(GS_VideoMode::DVD_NTSC);
                    break;
                case 0x73:
                    mode = "DVD PAL 720x480 @ ??.???";
            ...
            if (CurrentBiosInformation.threadListAddr == 0) {
                ...
                while (offset < 0x5000)    // I find that the instructions are in between 0x4000 -> 0x5000
                {
                        ...
                        ThreadListInstructions[2] == inst3)      // no-op
                    {
                        ...
                        // We (well, I) know that the thread address is always 0x8001 + the immediate of the 6th
                        // instruction from here
                        const u32 op = memRead32(0x80000000 + offset + (sizeof(u32) * 6));
                        CurrentBiosInformation.threadListAddr =
                            0x80010000 + static_cast<u16>(op) -
                            8;    // Subtract 8 because the address here is offset by 8.
                        DevCon.WriteLn(
                            "BIOS: Successfully found the instruction pattern. Assuming the thread list is here: %0x",
                            CurrentBiosInformation.threadListAddr);
                        break;
                    }
                    offset += 4;
                ...
                if (!CurrentBiosInformation.threadListAddr) {
                    ...
                    // If you're here because a user has reported this message, this means that the instruction pattern
                    // is not present on their bios, or it is aligned weirdly.
                    Console.Warning("BIOS Warning: Unable to get a thread list offset. The debugger thread and stack "
                                    "frame views will not be functional.");
                }
            }
        } break;
        case Syscall::sceSifSetDma:
            // The only thing this code is used for is the one log message, so don't execute it if we aren't logging
            // bios messages.
            ...
            //     //          R5900::bios[cpuRegs.GPR.n.v1.UC[0]], n_transfer, dmat->size, dmat->attr, dmat->dest,
            //     //          dmat->src);
            // }
            break;
        ...
        case Syscall::sysPrintOut: {
            if (cpuRegs.GPR.n.a0.UL[0] != 0) {
                ...
                // Instead of the addresse(s) being relative to the PS2 address space, make them relative to program
                // memory. (This fixes issue #2865)
                int curRegArg = 0;
                for (int i = 0; 1; i++) {
                    if (fmt[i] == '\0')
                        break;
                ...
                // sysConLog(fmt, regs[0], regs[1], regs[2], regs[3], regs[4], regs[5], regs[6]);
            }
            break;
        }
...
void TGEIU()
{
    if (cpuRegs.GPR.r[_Rs_].UD[0] >= (u64)_Imm_)
        trap();
}
void TLTIU()
{
