...
static s32 intExecuteBlock(s32 eeCycles)
{
    ...
    while (iopCycleEE > 0) {
        if ((psxHu32(HW_ICFG) & 8) && ((psxRegs.pc & 0x1fffffffU) == 0xa0 || (psxRegs.pc & 0x1fffffffU) == 0xb0 ||
...
R3000Acpu psxInt = {intReserve,         intReset,          intExecute, intExecuteBlock, intClear, intShutdown,

                    intGetCacheReserve, intSetCacheReserve};
