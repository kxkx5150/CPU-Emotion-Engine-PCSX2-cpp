...
__fi bool ReadFifoSingleWord()
{
    ...
    // SIF_LOG("Read Fifo SIF2 Single Word IOP Busy %x Fifo Size %x SIF2 CHCR %x", sif2.iop.busy, sif2.fifo.size,
    //         HW_DMA2_CHCR);
...
static __fi bool ProcessEETag()
{
    ...
    // SIF_LOG("SIF2 EE dest chain tag madr:%08X qwc:%04X id:%X irq:%d(%08X_%08X)", sif2dma.madr, sif2dma.qwc, ptag.ID,
    //         ptag.IRQ, tag[1], tag[0]);
...
static __fi bool ProcessIOPTag()
{
    ...
    if ((sif2data & 0xFFFFFF) == 0xFFFFFF || (HW_DMA2_CHCR & 0x200)) */
    sif2.iop.end = true;
    DevCon.Warning("SIF2 IOP Tag: madr=%lx, counter=%lx (%08X_%08X)", hw_dma2.madr, sif2.iop.counter, sif2words,
                   sif2data);
...
__fi void dmaSIF2()
{
    ...
    // if(sif2dma.chcr.MOD == CHAIN_MODE && sif2dma.qwc > 0) DevCon.Warning(L"SIF2 QWC on Chain CHCR " +
    // sif2dma.chcr.desc());
    psHu32(SBUS_F240) |= 0x8000;
    sif2.ee.busy = true;
