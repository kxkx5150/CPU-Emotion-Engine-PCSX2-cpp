...
// use one of the four "functional" derived classes rather than this class directly: gzLoadingState, gzSavingState
// (gzipped disk-saved states), and memLoadingState, memSavingState (uncompressed memory states).
class SaveStateBase {
  protected:
  ...
  public:
    ...
    template <typename T> void FreezeLegacy(T &data, int sizeOfNewStuff)
    {
...
class ArchiveEntry {
  ...
  public:
    ArchiveEntry(const wxString &filename = wxEmptyString) : m_filename(filename)
    {
        m_dataidx  = 0;
        m_datasize = 0;
    }
...
class ArchiveEntryList {
  ...
  public:
    ...
    ArchiveEntryList(ArchiveDataBuffer *data) : m_data(data)
    {
    }
    ArchiveEntryList(ArchiveDataBuffer &data) : m_data(&data)
    {
