...
static __fi void gsCSRwrite(const tGS_CSR &csr)
{
    ...
    if (csr.SIGNAL) {
        ...
        if (gifUnit.gsSIGNAL.queued) {
            ...
            GSSIGLBLID.SIGID =
                (GSSIGLBLID.SIGID & ~gifUnit.gsSIGNAL.data[1]) | (gifUnit.gsSIGNAL.data[0] & gifUnit.gsSIGNAL.data[1]);
...
// The GS needs to be told at the start of a vsync else it loses half of its picture (could be responsible for some
// halfscreen issues) We got away with it before i think due to our awful GS timing, but now we have it right (ish)
