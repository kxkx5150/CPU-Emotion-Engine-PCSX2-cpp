...
// Each of these offsets has a debugger-accessible equivalent variable without the Offset suffix that will hold the
// actual address (not here because we don't want code using it)
...
class SysMainMemory {
  ...
  public:
    ...
    const VirtualMemoryManagerPtr &MainMemory()
    {
        return m_mainMemory;
    }
    VirtualMemoryBumpAllocator &BumpAllocator()
    {
        return m_bumpAllocator;
    }
...
class SysCpuProviderPack {
  ...
  public:
    ...
    {
        return m_RecExceptionEE.get();
    }
    BaseException *GetException_IOP() const
    {
        return m_RecExceptionIOP.get();
    }
