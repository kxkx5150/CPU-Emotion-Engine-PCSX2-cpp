...
__fi void vif0SetupTransfer()
{
    ...
    if (!(vif0ch.transfer("vif0 Tag", ptag)))
        return;
    ...
    if (vif0ch.chcr.TTE) {
        ...
        if (vif0.irqoffset.enabled) {
            ret = VIF0transfer((u32 *)&masked_tag + vif0.irqoffset.value, 4 - vif0.irqoffset.value,
        ...
        if (!ret && vif0.irqoffset.enabled) {
            ...
            vif0ch.qwc = 0;    // Gumball 3000 pauses the DMA when the tag stalls so we need to reset the QWC, it'll be
                               // gotten again later
...
__fi void vif0VUFinish()
{
    ...
    if (vif0.waitforvu) {
        ...
        if (!(cpuRegs.interrupt & 0x1) && vif0ch.chcr.STR &&
...
__fi void vif0Interrupt()
{
    ...
    if (!(vif0ch.chcr.STR))
        Console.WriteLn("vif0 running when CHCR == %x", vif0ch.chcr._u32);
    ...
    if (vif0.irq && vif0.vifstalled.enabled && vif0.vifstalled.value == VIF_IRQ_STALL) {
        ...
        if (vif0Regs.stat.test(VIF0_STAT_VSS | VIF0_STAT_VIS | VIF0_STAT_VFS)) {
            ...
            if (vif0ch.qwc > 0 || !vif0.done) {
                vif0Regs.stat.VPS = VPS_DECODING;    // If there's more data you need to say it's decoding the next VIF
    ...
    if (vif0ch.qwc > 0)
        Console.WriteLn("vif0 Ending with %x QWC left", vif0ch.qwc);
    if (vif0.cmd != 0)
        Console.WriteLn("vif0.cmd still set %x tag size %x", vif0.cmd, vif0.tag.size);
#endif
...
void dmaVIF0()
{
    ...
    if (vif0ch.qwc > 0)    // Normal Mode
    {
        ...
        } else    // Assume Normal mode.
        {
            ...
            if (vif0.irqoffset.enabled && !vif0.done)
                DevCon.Warning("Warning! VIF0 starting a Normal transfer with vif offset set (Possible force stop?)");
            vif0.done = true;
        }
    ...
    // Using a delay as Beyond Good and Evil does the DMA twice with 2 different TADR's (no checks in the middle, all
    // one block of code), the first bit it sends isnt required for it to work. Also being an end chain it ignores the
    // second lot, this causes infinite loops ;p
    if (!vif0Regs.stat.test(VIF0_STAT_VSS | VIF0_STAT_VIS | VIF0_STAT_VFS))
        CPU_INT(DMAC_VIF0, 4);
}
