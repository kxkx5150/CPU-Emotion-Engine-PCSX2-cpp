...
// I suspect this is GS side which should really be handled by GS which also doesn't current have a fifo, but we can
// guess from our fifo
...
bool CheckPaths()
{
    ...
    if (!gifUnit.CanDoPath3()) {
        if (!gifUnit.Path3Masked()) {
            // DevCon.Warning("Path3 stalled APATH %x PSE %x DIR %x Signal %x", gifRegs.stat.APATH, gifRegs.stat.PSE,
            // gifRegs.stat.DIR, gifUnit.gsSIGNAL.queued);
            GifDMAInt(128);
        }
        return false;
    }
...
int GIF_Fifo::write_fifo(u32 *pMem, int size)
{
    ...
    // GIF_LOG("GIF FIFO Adding %d QW to GIF FIFO at offset %d FIFO now contains %d QW", transferSize, writePos,
    // fifoSize);
...
int GIF_Fifo::read_fifo()
{
    ...
    sizeRead = gifUnit.TransferGSPacketData(GIF_TRANS_DMA, (u8 *)&data, fifoSize * 16) /
               16;    // returns the size actually read
...
{
    // GIF_LOG("gifInterrupt caught qwc=%d fifo=%d(%d) apath=%d oph=%d state=%d!", gifch.qwc, gifRegs.stat.FQC,
    //         gif_fifo.fifoSize, gifRegs.stat.APATH, gifRegs.stat.OPH, gifUnit.gifPath[GIF_PATH_3].state);
    ...
    if (gif_fifo.fifoSize)
        GifDMAInt(8 * BIAS);
    // GIF_LOG("GIF DMA End QWC in fifo %x (%x) APATH = %x OPH = %x state = %x", gifRegs.stat.FQC, gif_fifo.fifoSize,
    //         gifRegs.stat.APATH, gifRegs.stat.OPH, gifUnit.gifPath[GIF_PATH_3].state);
}
...
static u32 WRITERING_DMA(u32 *pMem, u32 qwc)
{
    ...
    if (CheckPaths() == false || ((qwc < 8 || gif_fifo.fifoSize > 0) && CHECK_GIFFIFOHACK)) {
        if (gif_fifo.fifoSize < 16) {
            size = gif_fifo.write_fifo(
                (u32 *)pMem,
                originalQwc);    // Use original QWC here, the intermediate mode is for the GIF unit, not DMA
            incGifChAddr(size);
            return size;
        }
...
void GIFdma()
{
    while (gifch.qwc > 0 || !gif.gspath3done) {
        ...
        if ((dmacRegs.ctrl.STD == STD_GIF) && (gif.prevcycles != 0)) {
            // Console.WriteLn("GS Stall Control Source = %x, Drain = %x\n MADR = %x, STADR = %x", (psHu32(0xe000) >> 4)
            // & 0x3, (psHu32(0xe000) >> 6) & 0x3, gifch.madr, psHu32(DMAC_STADR));
        ...
        if ((gifch.chcr.MOD == CHAIN_MODE) && (!gif.gspath3done) && gifch.qwc == 0)    // Chain Mode
        {
            ...
            // GIF_LOG("gifdmaChain %8.8x_%8.8x size=%d, id=%d, addr=%lx tadr=%lx", ptag[1]._u32, ptag[0]._u32,
            // gifch.qwc,
            //         ptag->ID, gifch.madr, gifch.tadr);
            ...
            if (dmacRegs.ctrl.STD == STD_GIF) {
                ...
                if ((ptag->ID == TAG_REFS) && ((gifch.madr + (gifch.qwc * 16)) > dmacRegs.stadr.ADDR)) {
                    // stalled.
                    // We really need to test this. Pay attention to prevcycles, as it used to trigger GIFchains in the
                    // code above. (rama)
...
void dmaGIF()
{
    ...
    gif.gspath3done =
        false;    // For some reason this doesn't clear? So when the system starts the thread, we will clear it :)

    if (gifch.chcr.MOD ==
        NORMAL_MODE) {    // Else it really is a normal transfer and we want to quit, else it gets confused with chains
        gif.gspath3done = true;
    }
...
static u32 QWCinGIFMFIFO(u32 DrainADDR)
{
    ...
    // SPR_LOG("GIF MFIFO Requesting %x QWC from the MFIFO Base %x, SPR MADR %x Drain %x", gifch.qwc,
    // dmacRegs.rbor.ADDR,
    //         spr0ch.madr, DrainADDR);
...
void mfifoGifMaskMem(int id)
{
    switch (id) {
        ...
        case TAG_END:
            ...
            {
                // SPR_LOG("GIF MFIFO MADR below bottom of ring buffer, wrapping GIF MADR = %x Ring Bottom %x",
                // gifch.madr,
                //         dmacRegs.rbor.ADDR);
                gifch.madr = qwctag(gifch.madr);
            } else if (gifch.madr >
                       (dmacRegs.rbor.ADDR +
                        (u32)dmacRegs.rbsr.RMSK))    // Usual scenario is the tag is near the end (Front Mission 4)
            {
                // SPR_LOG("GIF MFIFO MADR outside top of ring buffer, wrapping GIF MADR = %x Ring Top %x", gifch.madr,
                //         (dmacRegs.rbor.ADDR + dmacRegs.rbsr.RMSK) + 16);
                gifch.madr = qwctag(gifch.madr);
            }
            break;
        default:
...
void mfifoGIFtransfer()
{
    ...
    if (gifch.qwc == 0) {
        ...
        // ptag[1]._u32,
        //         ptag[0]._u32, gifch.qwc, ptag->ID, gifch.madr, gifch.tadr, gif.gifqwc, spr0ch.madr);
