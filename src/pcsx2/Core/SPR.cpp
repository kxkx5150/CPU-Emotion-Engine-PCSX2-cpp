    ...
    if (madr >= 0x11000000 && (madr < 0x11010000)) {
        ...
        if (madr < 0x11004000) {
            ...
            if (((madr & 0xff0) + (qwc * 16)) > 0x1000) {
                DevCon.Warning("Warning! SPR%d Crossing in to VU0 Micro Mirror address! Start MADR = %x, End MADR = %x",
            ...
            if (((madr & 0xff0) + (qwc * 16)) > 0x1000) {
                DevCon.Warning("Warning! SPR%d Crossing in to VU0 Mem Mirror address! Start MADR = %x, End MADR = %x",
                               isWrite ? 0 : 1, madr, madr + (qwc * 16));
            }
        }
    }
...
void SPRFROMinterrupt()
{
    ...
    if (!spr0finished || spr0ch.qwc > 0) {
        ...
        // The qwc check is simply because having data still to transfer from the packet can freak games out if they do
        // a d.tadr == s.madr check and there is still data to come over (FF12 ingame menu)
        if (mfifotransferred != 0 && spr0ch.qwc == 0) {
            switch (dmacRegs.ctrl.MFD) {
                case MFD_VIF1:    // Most common case.
                case MFD_GIF: {
                    if ((spr0ch.madr & ~dmacRegs.rbsr.RMSK) != dmacRegs.rbor.ADDR)
                        Console.WriteLn("GIF MFIFO Write outside MFIFO area");
                    spr0ch.madr = dmacRegs.rbor.ADDR + (spr0ch.madr & dmacRegs.rbsr.RMSK);
...
void _dmaSPR1()    // toSPR work function
{
    switch (spr1ch.chcr.MOD) {
        ...
        case CHAIN_MODE: {
            ...
            if (spr1ch.qwc > 0) {
                // SPR_LOG("spr1 Normal or in Progress size=%d, addr=%lx taddr=%lx saddr=%lx", spr1ch.qwc, spr1ch.madr,
                // spr1ch.tadr, spr1ch.sadr); Transfer Dn_QWC from Dn_MADR to SPR1
                SPR1chain();
                return;
            }
            // Chain Mode
