...
void __fastcall vu1ExecMicro(u32 addr)
{
    if (THREAD_VU1) {
        ...
        // Most of the time with MTVU we want to pretend the VU has finished quickly as to gain the benefit from running
        // another thread however with T-Bit games when the T-Bit is enabled, it needs to wait in case a T-Bit happens,
        // so we need to set "Busy" We shouldn't do this all the time as it negates the extra thread and causes games
        // like Ratchet & Clank to be no faster.
