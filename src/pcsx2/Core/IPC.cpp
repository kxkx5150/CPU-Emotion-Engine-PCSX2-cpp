...
// #include "svnrev.h"
#include "IPC.h"
SocketIPC::SocketIPC(SysCoreThread *vm, unsigned int slot) : pxThread("IPC_Socket")
{
#ifdef _WIN32
    WSADATA            wsa;
...
int SocketIPC::StartSocket()
{
    ...
    if (m_msgsock == -1) {
        ...
        if (!(errno_w == WSAECONNRESET || errno_w == WSAEINTR || errno_w == WSAEINPROGRESS || errno_w == WSAEMFILE ||
              errno_w == WSAEWOULDBLOCK)) {
#else
