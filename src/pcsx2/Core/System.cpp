...
{
    if (m_profiler_name.IsEmpty() || !IsOk())
        return;
...
{
    if (!_parent::Assign(std::move(allocator), baseptr, size))
        return NULL;
...
void RecompiledCodeReserve::ThrowIfNotOk() const
{
    ...
    throw Exception::OutOfMemory(m_name)
        ...
        .SetUserMsg(
            wxString(L"This recompiler was unable to reserve contiguous memory required for internal caches.  This "
                     L"error can be caused by low virtual memory resources, such as a small or disabled swapfile, "
                     L"or by another program that is hogging a lot of memory."));
}
...
template <typename CpuType> class CpuInitializer {
  public:
...
//
template <typename CpuType> CpuInitializer<CpuType>::CpuInitializer()
{
    try {
...
{
    return wxString(
        L"PCSX2 is unable to allocate memory needed for the PS2 virtual machine. Close out some memory hogging "
        L"background tasks and try again.");
}
...
static VirtualMemoryManagerPtr makeMainMemoryManager()
{
    ...
    // On x86-64, code can reach 8*2^28 from its address [-6*2^28, 4*2^28] is the region that allows for code in the
    // 640mb allocation to reach 512mb of code that either starts at codeBase or 256mb before it. We start high and
    // count down because on macOS code starts at the beginning of useable address space, so starting as far ahead as
    // possible reduces address variations due to code size.  Not sure about other platforms.  Obviously this only
    // actually affects what shows up in a debugger and won't affect performance or correctness of anything.
    for (int offset = 4; offset >= -6; offset--) {
        ...
        auto mgr = std::make_shared<VirtualMemoryManager>("Main Memory Manager", base, HostMemoryMap::Size,
                                                          /*upper_bounds=*/0, /*strict=*/true);
        if (mgr->IsOk()) {
            return mgr;
        }
    }
...
SysMainMemory::SysMainMemory()
    : m_mainMemory(makeMainMemoryManager()), m_bumpAllocator(m_mainMemory, HostMemoryMap::bumpAllocatorOffset,
                                                             HostMemoryMap::Size - HostMemoryMap::bumpAllocatorOffset)
{
...
{
    vtlb_Core_Alloc();
    if (m_ee.IsCommitted() && m_iop.IsCommitted() && m_vu.IsCommitted())
        return;
...
bool SysCpuProviderPack::IsRecAvailable_MicroVU0() const
{
    return CpuProviders->microVU0.IsAvailable();
}
bool SysCpuProviderPack::IsRecAvailable_MicroVU1() const
{
    return CpuProviders->microVU1.IsAvailable();
}
BaseException *SysCpuProviderPack::GetException_MicroVU0() const
{
    return CpuProviders->microVU0.ExThrown.get();
}
BaseException *SysCpuProviderPack::GetException_MicroVU1() const
{
    return CpuProviders->microVU1.ExThrown.get();
}
...
{
    return (recOpts.EnableEE && !IsRecAvailable_EE()) || (recOpts.EnableIOP && !IsRecAvailable_IOP()) ||
           (recOpts.EnableVU0 && !IsRecAvailable_MicroVU0()) || (recOpts.EnableVU1 && !IsRecAvailable_MicroVU1());
}
...
u8 *SysMmapEx(uptr base, u32 size, uptr bounds, const char *caller)
{
    ...
    if ((Mem == NULL) || (bounds != 0 && (((uptr)Mem + size) > bounds))) {
        ...
        if ((bounds != 0) && (((uptr)Mem + size) > bounds)) {
            DevCon.Warning("Second try failed allocating %s, block ptr 0x%x does not meet required criteria.", caller,
                           Mem);
            SafeSysMunmap(Mem, size);
