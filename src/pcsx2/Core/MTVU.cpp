...
static __fi u32 size_u32(u32 x)
{
    return (x + 3) >> 2;
}
...
VU_Thread::VU_Thread(BaseVUmicroCPU *&_vuCPU, VURegs &_vuRegs) : vuCPU(_vuCPU), vuRegs(_vuRegs)
{
    m_name = L"MTVU";
    Reset();
}
...
{
    return (vuCycles[0].load(std::memory_order_acquire) + vuCycles[1].load(std::memory_order_acquire) +
            vuCycles[2].load(std::memory_order_acquire) + vuCycles[3].load(std::memory_order_acquire)) >>
           2;
}
...
void VU_Thread::Get_MTVUChanges()
{
    ...
    if (interrupts & InterruptFlagSignal) {
        ...
        // If load of signal was moved after clearing the flag, the other thread could write a new value before we load
        // without noticing the double signal Prevent that with release semantics
    ...
    if (interrupts & InterruptFlagLabel) {
        ...
        // If other thread updates gsLabel for a second interrupt, that's okay.  Worst case we think there's a label
        // interrupt but gsLabel is 0 We do not want the exchange of gsLabel to move ahead of clearing the flag, or the
        // other thread could add more work before we clear the flag, resulting in an update with the flag unset acquire
        // semantics should supply that guarantee
