...
//    Flags (magic sio '+' thingie!), Sector size, eraseBlockSize (in pages), card size (in pages), xor checksum
//    (superblock?), terminator (unused?).
...
// E.g. if the game polls the card 100 times/sec and max tries=100, then after 1 second it will see the card as inserted
// (ms timeout not reached). E.g. if the game polls the card 1 time/sec, then it will see the card ejected 4 times, and
// on the 5th it will see it as inserted (4 secs from the initial access). (A 'try' in this context is the game
// accessing SIO)
...
SIO_WRITE sioWriteStart(u8 data)
{
    ...
    switch (data) {
        case 0x01:
            siomode = SIO_CONTROLLER;
            break;
        case 0x21:
            siomode = SIO_MULTITAP;
            break;
        case 0x61:
            siomode = SIO_INFRARED;
            break;
        case 0x81:
            siomode = SIO_MEMCARD;
            break;
...
// this should be done on the last byte, but it works like this fine for now... How does one know which number is the
// last byte anyway (or are there any more following it). maybe in the end a small LUT will be necessary that has the
// number of bytes for each command... On the real PS1, if the controller doesn't supply an /ACK signal after each byte
// but the last, the transmission falls through... but it seems it is actually the interrupt what 'continues' a
// transfer. On the real PS1, asserting /ACK after the last byte would cause the transfer to fail.
...
SIO_WRITE sioWriteController(u8 data)
{
    ...
    // Console.WriteLn( "SIO: sent = %02X  From pad data =  %02X  bufCnt %08X ", data, sio.buf[sio.bufCount],
    // sio.bufCount);
...
SIO_WRITE memcardSector(u8 data)
{
    ...
    switch (sio.bufCount) {
        ...
        case 3:
            ...
            break;
        case 4:
            mcd->sectorAddr |= data << 16;
            xor_check ^= data;
            break;
        case 5:
            mcd->sectorAddr |= data << 24;
            xor_check ^= data;
            break;
        case 6:
...
SIO_WRITE memcardInit()
{
    ...
    if (mcd->ForceEjection_Timeout) {
        if (mcd->ForceEjection_Timeout == FORCED_MCD_EJECTION_MAX_TRIES && mcd->IsPresent())
            Console.WriteLn(Color_Green, L"[%s] Auto-ejecting memcard [port:%d, slot:%d]", WX_STR(GetTimeMsStr()),
                            sio.GetPort(), sio.GetSlot());
        ...
        if (numTimesAccessed > FORCED_MCD_EJECTION_MIN_TRIES) {
            ...
            if (delta.GetMilliseconds() >= FORCED_MCD_EJECTION_MAX_MS_AFTER_MIN_TRIES) {
                DevCon.Warning(L"[%s] Auto-eject: Timeout reached after mcd was accessed %d times [port:%d, slot:%d]",
        ...
        if (mcd->ForceEjection_Timeout == 0 && mcd->IsPresent())
            Console.WriteLn(Color_Green, L"[%s] Re-inserting auto-ejected memcard [port:%d, slot:%d]",
...
void chkTriggerInt()
{
    ...
    if ((sio.StatReg & IRQ)) {
        sioInterrupt();
        return;
    }    // The interrupt flag in the main INTR_STAT reg should go active on multiple occasions. Set it here for now
         // (hack), until the correct mechanism is made.
    if ((sio.CtrlReg & ACK_INT_EN) && ((sio.StatReg & TX_RDY) || (sio.StatReg & TX_EMPTY))) {
        sioInterrupt();
        return;
    }
    if ((sio.CtrlReg & ACK_INT_EN) && (sio.StatReg & ACK_INP)) {
        sioInterrupt();
        return;
    }
    // The following one may be incorrect.
    // if ((sio.CtrlReg & RX_INT_EN) && ((byteCnt >= (1<< ((sio.CtrlReg & RX_BYTES_INT) >>8))) ? 1:0) ) {
    // sioInterrupt(); return; }
    return;
}
...
static void sioWrite8inl(u8 data)
{
    ...
    switch (siomode) {
        case SIO_START:
            ...
            break;
        case SIO_CONTROLLER:
            sioWriteController(data);
            break;
        case SIO_MULTITAP:
            sioWriteMultitap(data);
            break;
        case SIO_INFRARED:
            sioWriteInfraRed(data);
            break;
        case SIO_MEMCARD:
            sioWriteMemcard(data);
            break;
        case SIO_MEMCARD_AUTH:
            memcardAuth(data);
            break;
        case SIO_MEMCARD_ERASE:
            memcardErase(data);
            break;
        case SIO_MEMCARD_WRITE:
            memcardWrite(data);
            break;
        case SIO_MEMCARD_READ:
            memcardRead(data);
            break;
        case SIO_MEMCARD_SECTOR:
            memcardSector(data);
            break;
        case SIO_MEMCARD_PSX:
            sioWriteMemcardPSX(data);
            break;
        case SIO_DUMMY:
            break;
    };
    sio.StatReg |= RX_RDY;      // Why not set the byte-received flag, when for EVERY sent byte, one is received... it's
                                // just how SPI is...
...
void sioStatRead()
{
    ...
    // The /ACK line should go active for >2us, in a time window between 12us and 100us after each byte is sent
    // (received by the controller). If that doesn't happen, the controller is considered missing. The /ACK line must
    // NOT go active after the last byte in the transmission! (Otherwise some err. may happen - tested.)
    if ((sio.StatReg & ACK_INP))
        clrAckCnt++;
...
void sioWriteCtrl16(u16 value)
{
    ...
    if (sio.CtrlReg & CLR_INTR)
        sio.StatReg &= ~(IRQ | PARITY_ERR);    // clear internal interrupt
