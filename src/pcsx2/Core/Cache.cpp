...
struct CacheTag
{
    ...
    bool isValid() const
    {
        return rawValue & VALID_FLAG;
    }
    bool isDirty() const
    {
        return rawValue & DIRTY_FLAG;
    }
    bool lrf() const
    {
        return rawValue & LRF_FLAG;
    }
    bool isLocked() const
    {
        return rawValue & LOCK_FLAG;
    }
...
template <bool Write, int Bytes> void *prepareCacheAccess(u32 mem, int *way, int *idx)
{
    *way           = 0;
...
template <typename Int> void writeCache(u32 mem, Int value)
{
    int   way, idx;
...
template <typename Int> Int readCache(u32 mem)
{
    int   way, idx;
...
template <typename Op> void doCacheHitOp(u32 addr, const char *name, Op op)
{
    ...
    if (!findInCache(set, ppf, &way)) {
        // CACHE_LOG("CACHE %s NO HIT addr %x, index %d, tag0 %zx tag1 %zx", name, addr, index, set.tags[0].rawValue,
        //           set.tags[1].rawValue);
        return;
    }

    // CACHE_LOG("CACHE %s addr %x, index %d, way %d, flags %x OP %x", name, addr, index, way, set.tags[way].flags(),
    //           cpuRegs.code);
...
void       CACHE()
{
    ...
    switch (_Rt_) {
        case 0x1a:    // DHIN (Data Cache Hit Invalidate)
            doCacheHitOp(addr, "DHIN", [](CacheLine line) { line.clear(); });
            break;
        ...
        case 0x1c:    // DHWOIN (Data Cache Hit WriteBack Without Invalidate)
            doCacheHitOp(addr, "DHWOIN", [](CacheLine line) { line.writeBackIfNeeded(); });
            break;
        ...
        case 0x11:    // DXLDT (Data Cache Load Data into TagLo)
        {
            ...
            // CACHE_LOG("CACHE DXLDT addr %x, index %d, way %d, DATA %x OP %x", addr, index, way, cpuRegs.CP0.n.TagLo,
            //           cpuRegs.code);
            break;
        }
        ...
        case 0x10:    // DXLTG (Data Cache Load Tag into TagLo)
        {
            ...
            // DXLTG demands that SYNC.L is called before this command, which forces the cache to write back, so
            // presumably games are checking the cache has updated the memory For speed, we will do it here.
            line.writeBackIfNeeded();

            // Our tags don't contain PS2 paddrs (instead they contain x86 addrs)
            cpuRegs.CP0.n.TagLo = line.tag.flags();

            // CACHE_LOG("CACHE DXLTG addr %x, index %d, way %d, DATA %x OP %x ", addr, index, way, cpuRegs.CP0.n.TagLo,
            //           cpuRegs.code);
        ...
        case 0x13:    // DXSDT (Data Cache Store 32bits from TagLo)
        {
            ...
            *reinterpret_cast<u32 *>(&line.data.bytes[addr & 0x3C]) = cpuRegs.CP0.n.TagLo;
            ...
            // CACHE_LOG("CACHE DXSDT addr %x, index %d, way %d, DATA %x OP %x", addr, index, way, cpuRegs.CP0.n.TagLo,
            //           cpuRegs.code);
            break;
        }
        ...
        case 0x12:    // DXSTG (Data Cache Store Tag from TagLo)
        {
            ...
            // CACHE_LOG("CACHE DXSTG addr %x, index %d, way %d, DATA %x OP %x", addr, index, way, cpuRegs.CP0.n.TagLo,
            //           cpuRegs.code);
        ...
        case 0x14:    // DXWBIN (Data Cache Index WriteBack Invalidate)
        {
            ...
            // CACHE_LOG("CACHE DXWBIN addr %x, index %d, way %d, flags %x paddr %zx", addr, index, way,
            // line.tag.flags(),
            //           line.addr());
            line.writeBackIfNeeded();
            line.clear();
