...
struct MemoryCardFileEntry
{
    enum MemoryCardFileModeFlags
    {
        ...
        Mode_PSX           = 0x1000,
        Mode_Unknown0x2000 =
            0x2000,    // Supposedly Mode_Hidden but files still show up in the PS2 browser with this set
    ...
    bool IsFile() const
    {
        return !!(entry.data.mode & Mode_File);
    }
    bool IsDir() const
    {
        return !!(entry.data.mode & Mode_Directory);
    }
    bool IsUsed() const
    {
        return !!(entry.data.mode & Mode_Used);
    }
    bool IsValid() const
    {
        return entry.data.mode != 0xFFFFFFFF;
    }
    // checks if we're either "." or ".."
    bool IsDotDir() const
    {
        return entry.data.name[0] == '.' &&
               (entry.data.name[1] == '\0' || (entry.data.name[1] == '.' && entry.data.name[2] == '\0'));
    }

    static const u32 DefaultDirMode =
        Mode_Read | Mode_Write | Mode_Execute | Mode_Directory | Mode_Unknown0x0400 | Mode_Used;
    static const u32 DefaultFileMode =
        Mode_Read | Mode_Write | Mode_Execute | Mode_File | Mode_Unknown0x0080 | Mode_Unknown0x0400 | Mode_Used;
...
class FileAccessHelper {
  private:
    ...
    MemoryCardFileMetadataReference                     *m_lastWrittenFileRef =
        nullptr;    // we remember this to reduce redundant metadata checks/writes
  ...
  public:
    ...
    // Close all open files that start with the given path, so either a file if a filename is given or all files in a
    // directory and its subdirectories when a directory is given
    ...
    static void WriteIndex(wxFileName folderName, MemoryCardFileEntry *const entry,
...
class FolderMemoryCard {
  ...
  public:
    ...
    void Open(const wxString &fullPath, const Pcsx2Config::McdOptions &mcdOptions, const u32 sizeInClusters,
              const bool enableFiltering, const wxString &filter, bool simulateFileWrites = false);
  ...
  protected:
    ...
    MemoryCardFileEntryCluster *GetFileEntryCluster(const u32 currentCluster, const u32 searchCluster,
                                                    const u32 fileCount);
    ...
    // - searchCluster: the cluster that is being accessed, relative to alloc_offset in the superblock
    // - fileName: wxFileName of the root directory of the memory card folder in the host file system (filename part
    // doesn't matter)
    ...
    MemoryCardFileEntry *GetFileEntryFromFileDataCluster(const u32 currentCluster, const u32 searchCluster,
                                                         wxFileName *fileName, const size_t originalDirCount,
    ...
    bool AddFolder(MemoryCardFileEntry *const dirEntry, const wxString &dirPath,
                   MemoryCardFileMetadataReference *parent = nullptr, const bool enableFiltering = false,
                   const wxString &filter = L"");
    ...
    // - parent: pointer to the parent dir's quick-access reference element
    bool AddFile(MemoryCardFileEntry *const dirEntry, const wxString &dirPath, const EnumeratedFileEntry &fileEntry,
                 MemoryCardFileMetadataReference *parent = nullptr);
    ...
    // adds a file to the quick-access dictionary, so it can be accessed more efficiently (ie, without searching through
    // the entire file system) later returns the MemoryCardFileMetadataReference of the first file cluster, or nullptr
    // if the file is zero-length
    MemoryCardFileMetadataReference *AddFileEntryToMetadataQuickAccess(MemoryCardFileEntry *const             entry,
                                                                       MemoryCardFileMetadataReference *const parent);

    // creates a reference to a directory entry, so it can be passed as parent to other files/directories
    MemoryCardFileMetadataReference *AddDirEntryToMetadataQuickAccess(MemoryCardFileEntry *const             entry,
                                                                      MemoryCardFileMetadataReference *const parent);
    ...
    // "delete" (prepend '_pcsx2_deleted_' to) any files that exist in oldFileEntries but no longer exist in
    // m_fileEntryDict also calls RemoveUnchangedDataFromCache() since both operate on comparing with the old file
    // entires
    void
    FlushDeletedFilesAndRemoveUnchangedDataFromCache(const std::vector<MemoryCardFileEntryTreeNode> &oldFileEntries);
    ...
    void CopyEntryDictIntoTree(std::vector<MemoryCardFileEntryTreeNode> *fileEntryTree, const u32 cluster,
                               const u32 fileCount);

    // find equivalent (same name and type) of searchEntry in m_fileEntryDict in the directory indicated by cluster
    const MemoryCardFileEntry *FindEquivalent(const MemoryCardFileEntry *searchEntry, const u32 cluster,
                                              const u32 fileCount);
