...
void COP2_BC2()
{
    Int_COP2BC2PrintTable[_Rt_]();
}
void COP2_SPECIAL()
{
    Int_COP2SPECIAL1PrintTable[_Funct_]();
}
...
__fi void _vu0run(bool breakOnMbit, bool addCycles)
{
    ...
    do {    // Run VU until it finishes or M-Bit
        ...
        (VU0.VI[REG_VPU_STAT].UL & 1)    // E-bit Termination
...
{
    _vu0run(0, 1);
}    // Runs VU0 Micro Until E-Bit End
void vu0Finish()
{
    _vu0run(0, 0);
}    // Runs VU0 Micro Until E-Bit End (doesn't stall EE)
