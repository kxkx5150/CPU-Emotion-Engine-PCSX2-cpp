...
static u32 QWCinVIFMFIFO(u32 DrainADDR, u32 qwc)
{
    ...
    // VIF_LOG("VIF MFIFO Requesting %x QWC of %x Available from the MFIFO Base %x MFIFO Top %x, SPR MADR %x Drain %x",
...
static __fi void mfifo_VIF1chain()
{
    ...
    if (vif1ch.madr >= dmacRegs.rbor.ADDR && vif1ch.madr < (dmacRegs.rbor.ADDR + dmacRegs.rbsr.RMSK + 16u)) {
        ...
        // No need to exit on non-mfifo as it is indirect anyway, so it can be transferring this while spr refills the
        // mfifo
...
void mfifoVifMaskMem(int id)
{
    switch (id) {
        ...
        case TAG_END:
            ...
            {
                // DevCon.Warning("VIF MFIFO MADR below bottom of ring buffer, wrapping VIF MADR = %x Ring Bottom %x",
            ...
            {
                // DevCon.Warning("VIF MFIFO MADR outside top of ring buffer, wrapping VIF MADR = %x Ring Top %x",
...
void mfifoVIF1transfer()
{
    ...
    if (vif1ch.qwc == 0) {
        ...
        // VIF_LOG("dmaChain %8.8x_%8.8x size=%d, id=%d, madr=%lx, tadr=%lx spr0 madr = %x", ptag[1]._u32, ptag[0]._u32,
        //         vif1ch.qwc, ptag->ID, vif1ch.madr, vif1ch.tadr, spr0ch.madr);
...
void vifMFIFOInterrupt()
{
    ...
    if (vif1.irq && vif1.vifstalled.enabled && vif1.vifstalled.value == VIF_IRQ_STALL) {
        ...
        if (vif1Regs.stat.test(VIF1_STAT_VSS | VIF1_STAT_VIS | VIF1_STAT_VFS)) {
            ...
            // VIF_LOG("VIF1 MFIFO Stalled qwc = %x done = %x inprogress = %x", vif1ch.qwc, vif1.done,
            //         vif1.inprogress & 0x10);
