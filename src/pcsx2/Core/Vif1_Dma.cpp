...
void vif1TransferToMemory()
{
    ...
    // Some games such as Alex Ferguson's Player Manager 2001 reads less than GSLastDownloadSize by VIF then reads the
    // remainder by FIFO Clearing the memory is clearing memory it shouldn't be and kills it. The only scenario where
    // this could be used is the transfer size really is less than QWC, not the other way around as it was doing That
    // said, I think this is pointless and a waste of cycles and could cause more problems than good. We will alert this
    // situation below anyway.
    /*if (vif1.GSLastDownloadSize < vif1ch.qwc) {
            if (pMem < pMemEnd) {
                    DevCon.Warning("GS Transfer < VIF QWC, Clearing end of space GST %x QWC %x",
...
__fi void vif1SetupTransfer()
{
    ...
    if (vif1ch.chcr.TTE) {
        ...
        if (vif1.irqoffset.enabled) {
            ret = VIF1transfer((u32 *)&masked_tag + vif1.irqoffset.value, 4 - vif1.irqoffset.value,
                               true);    // Transfer Tag on stall
            ...
            // to the VU's, which breaks stuff, this is where the 128bit packet will fail, so we ignore the first 2
            // words
        ...
        if (!ret && vif1.irqoffset.enabled) {
            ...
            vif1ch.qwc = 0;    // Gumball 3000 pauses the DMA when the tag stalls so we need to reset the QWC, it'll be
                               // gotten again later
...
__fi void vif1Interrupt()
{
    ...
    // Some games (Fahrenheit being one) start vif first, let it loop through blankness while it sets MFIFO mode, so we
    // need to check it here.
    if (dmacRegs.ctrl.MFD == MFD_VIF1) {
        // Console.WriteLn("VIFMFIFO\n");
        //  Test changed because the Final Fantasy 12 opening somehow has the tag in *Undefined* mode, which is not in
        //  the documentation that I saw.
    ...
    if (vif1.irq && vif1.vifstalled.enabled && vif1.vifstalled.value == VIF_IRQ_STALL) {
        ...
        if (vif1Regs.stat.test(VIF1_STAT_VSS | VIF1_STAT_VIS | VIF1_STAT_VFS)) {
            ...
            if ((vif1ch.qwc > 0 || !vif1.done) && !CHECK_VIF1STALLHACK) {
                vif1Regs.stat.VPS = VPS_DECODING;    // If there's more data you need to say it's decoding the next VIF
                                                     // CMD (Onimusha - Blade Warriors)
    ...
    if (vif1.inprogress & 0x1) {
        ...
        if (!(vif1Regs.stat.VGW &&
              gifUnit.gifPath[GIF_PATH_3].state !=
                  GIF_PATH_IDLE))    // If we're waiting on GIF, stop looping, (can be over 1000 loops!)
...
void dmaVIF1()
{
    ...
    // Batman Vengence does something stupid and instead of cancelling a stall it tries to restart VIF, THEN check the
    // stall However if VIF FIFO is reversed, it can continue
