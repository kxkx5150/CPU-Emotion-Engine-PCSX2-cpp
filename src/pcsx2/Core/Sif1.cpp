...
static __fi bool SIFIOPReadTag()
{
    ...
    // SIF_LOG("SIF 1 IOP: dest chain tag madr:%08X wc:%04X id:%X irq:%d", sif1data & 0xffffff, sif1words, sif1tag.ID,
    //         sif1tag.IRQ);
    ...
    if (sif1words > 0xFFFFC)
        DevCon.Warning("SIF1 Overrun %x", sif1words);
...
__fi void dmaSIF1()
{
    ...
    if (sif1ch.chcr.MOD == CHAIN_MODE && sif1ch.qwc > 0) {
        if ((sif1ch.chcr.tag().ID == TAG_REFE) || (sif1ch.chcr.tag().ID == TAG_END) ||
