    ...
    if (cpuRegs.PERF.n.pccr.val & ((1 << (cpuRegs.CP0.n.Status.b.KSU + 2)) | (cpuRegs.CP0.n.Status.b.EXL << 1))) {
        ...
        if (PERF_ShouldCountEvent(cpuRegs.PERF.n.pccr.b.Event0)) {
            ...
            if ((cpuRegs.PERF.n.pcr0 & 0x80000000)) {
                ...
                // branch == 1 is probably not the best way to check for the delay slot, but it beats nothing!
                // (Refraction)
                /*	if( branch == 1 )
                        {
...
void MapTLB(int i)
{
    ...
    // COP0_LOG("MAP TLB %d: 0x%08X-> [0x%08X 0x%08X] S=%d G=%d ASID=%d Mask=0x%03X EntryLo0 PFN=%x EntryLo0 Cache=%x
    // EntryLo1 PFN=%x EntryLo1 Cache=%x VPN2=%x", 	i, tlb[i].VPN2, tlb[i].PFN0, tlb[i].PFN1, tlb[i].S >> 31,
    // tlb[i].G, tlb[i].ASID, 	tlb[i].Mask, tlb[i].EntryLo0 >> 6, (tlb[i].EntryLo0 & 0x38) >> 3, tlb[i].EntryLo1 >> 6,
    // (tlb[i].EntryLo1 & 0x38) >> 3, tlb[i].VPN2);
...
void UnmapTLB(int i)
{
    // Console.WriteLn("Clear TLB %d: %08x-> [%08x %08x] S=%d G=%d ASID=%d Mask= %03X",
...
void TLBR()
{
    // COP0_LOG("COP0_TLBR %d:%x,%x,%x,%x", cpuRegs.CP0.n.Index, cpuRegs.CP0.n.PageMask, cpuRegs.CP0.n.EntryHi,
...
void TLBWI()
{
    ...
    // COP0_LOG("COP0_TLBWI %d:%x,%x,%x,%x", cpuRegs.CP0.n.Index, cpuRegs.CP0.n.PageMask, cpuRegs.CP0.n.EntryHi,
    //          cpuRegs.CP0.n.EntryLo0, cpuRegs.CP0.n.EntryLo1);
...
void DI()
{
    if (cpuRegs.CP0.n.Status.b._EDI || cpuRegs.CP0.n.Status.b.EXL || cpuRegs.CP0.n.Status.b.ERL ||
