...
static __fi void IntCHackCheck()
{
    ...
    if (diff > 0)
        cpuRegs.cycle = g_nextEventCycle;
}

template <uint page> RETURNS_R128 _hwRead128(u32 mem);

template <uint page, bool intcstathack> mem32_t __fastcall _hwRead32(u32 mem)
{
    pxAssume((mem & 0x03) == 0);

    switch (page) {
        case 0x00:
            return rcntRead32<0x00>(mem);
        case 0x01:
...
template <uint page> mem32_t __fastcall hwRead32(u32 mem)
{
...
template <uint page> mem8_t __fastcall _hwRead8(u32 mem)
{
    u32 ret32 = _hwRead32<page, false>(mem & ~0x03);
    return ((u8 *)&ret32)[mem & 0x03];
}

template <uint page> mem8_t __fastcall hwRead8(u32 mem)
{
...
template <uint page> mem16_t __fastcall _hwRead16(u32 mem)
{
    pxAssume((mem & 0x01) == 0);
...
template <uint page> mem16_t __fastcall hwRead16(u32 mem)
{
...
template <uint page> static RETURNS_R64 _hwRead64(u32 mem)
{
    pxAssume((mem & 0x07) == 0);
...
template <uint page> RETURNS_R64 hwRead64(u32 mem)
{
...
template <uint page> RETURNS_R128 hwRead128(u32 mem)
{
