...
static __fi bool ProcessEETag()
{
    ...
    // SIF_LOG("SIF0 EE dest chain tag madr:%08X qwc:%04X id:%X irq:%d(%08X_%08X)", sif0ch.madr, sif0ch.qwc, ptag.ID,
...
static __fi bool ProcessIOPTag()
{
    ...
    if (sif0words > 0xFFFFF)
        DevCon.Warning("SIF0 Overrun %x", sif0words);
    ...
    if (sif0tag.IRQ || (sif0tag.ID & 4))
        sif0.iop.end = true;
    // SIF_LOG("SIF0 IOP Tag: madr=%lx, tadr=%lx, counter=%lx (%08X_%08X) Junk %d", hw_dma9.madr, hw_dma9.tadr,
    //         sif0.iop.counter, sif0words, sif0data, sif0.iop.writeJunk);
...
__fi void dmaSIF0()
{
    ...
    // if(sif0ch.chcr.MOD == CHAIN_MODE && sif0ch.qwc > 0) DevCon.Warning(L"SIF0 QWC on Chain CHCR " +
    // sif0ch.chcr.desc());
    psHu32(SBUS_F240) |= 0x2000;
    sif0.ee.busy = true;
