    ...
    for (int i = 1; i < 48; i++) {
        if (((tlb[i].EntryLo1 & 0x38) >> 3) == 0x3) {
            ...
            if ((addr >= tlb[i].PFN1) && (addr <= tlb[i].PFN1 + mask)) {
                // DevCon.Warning("Yay! Cache check cache addr=%x, mask=%x, addr+mask=%x, VPN2=%x PFN0=%x", addr, mask,
                // (addr & mask), tlb[i].VPN2, tlb[i].PFN0);
                return true;
            }
        }
...
template <typename DataType> DataType __fastcall vtlb_memRead(u32 addr)
{
    ...
    // return
    // reinterpret_cast<TemplateHelper<DataSize,false>::HandlerType*>(vtlbdata.RWFT[TemplateHelper<DataSize,false>::sidx][0][hand])(paddr,data);
...
template <typename DataType> void __fastcall vtlb_memWrite(u32 addr, DataType data)
{
    static const uint DataSize = sizeof(DataType) * 8;
...
static void GoemonTlbMissDebug()
{
    ...
    for (u32 i = 0; i < 150; i++) {
        if (tlb[i].valid == 0x1 && tlb[i].low_add != tlb[i].high_add)
            DevCon.WriteLn("GoemonTlbMissDebug: Entry %d is valid. Key %x. From V:0x%8.8x to V:0x%8.8x (P:0x%8.8x)", i,
                           tlb[i].key, tlb[i].low_add, tlb[i].high_add, tlb[i].physical_add);
        else if (tlb[i].low_add != tlb[i].high_add)
            DevCon.WriteLn("GoemonTlbMissDebug: Entry %d is invalid. Key %x. From V:0x%8.8x to V:0x%8.8x (P:0x%8.8x)",
    ...
    for (u32 i = 0; i < 150; i++) {
        if (tlb[i].valid == 0x1 && tlb[i].low_add != tlb[i].high_add) {
            ...
            if (vmv.isHandler(vaddr) && vmv.assumeHandlerGetID() == 0) {
                DevCon.WriteLn("GoemonPreloadTlb: Entry %d. Key %x. From V:0x%8.8x to P:0x%8.8x (%d pages)", i,
                               tlb[i].key, vaddr, paddr, size >> VTLB_PAGE_BITS);
    ...
    for (u32 i = 0; i < 150; i++) {
        if (tlb[i].key == key) {
            if (tlb[i].valid == 0x1) {
                u32 size  = tlb[i].high_add - tlb[i].low_add;
                u32 vaddr = tlb[i].low_add;
                DevCon.WriteLn("GoemonUnloadTlb: Entry %d. Key %x. From V:0x%8.8x to V:0x%8.8x (%d pages)", i,
                               tlb[i].key, vaddr, vaddr + size, size >> VTLB_PAGE_BITS);
...
template <typename OperandType, u32 saddr> OperandType __fastcall vtlbUnmappedVReadSm(u32 addr)
{
    vtlb_Miss(addr | saddr, 0);
    return 0;
}

template <typename OperandType, u32 saddr> u_to_r<OperandType> __vectorcall vtlbUnmappedVReadLg(u32 addr)
{
    vtlb_Miss(addr | saddr, 0);
    return rhelper<OperandType>::zero();
}

template <typename OperandType, u32 saddr> void __fastcall vtlbUnmappedVWriteSm(u32 addr, OperandType data)
{
    vtlb_Miss(addr | saddr, 1);
}

template <typename OperandType, u32 saddr> void __fastcall vtlbUnmappedVWriteLg(u32 addr, const OperandType *data)
{
    vtlb_Miss(addr | saddr, 1);
}

template <typename OperandType, u32 saddr> OperandType __fastcall vtlbUnmappedPReadSm(u32 addr)
{
    vtlb_BusError(addr | saddr, 0);
    return 0;
}

template <typename OperandType, u32 saddr> u_to_r<OperandType> __vectorcall vtlbUnmappedPReadLg(u32 addr)
{
    vtlb_BusError(addr | saddr, 0);
    return rhelper<OperandType>::zero();
}

template <typename OperandType, u32 saddr> void __fastcall vtlbUnmappedPWriteSm(u32 addr, OperandType data)
{
    vtlb_BusError(addr | saddr, 1);
}

template <typename OperandType, u32 saddr> void __fastcall vtlbUnmappedPWriteLg(u32 addr, const OperandType *data)
{
    vtlb_BusError(addr | saddr, 1);
}
...
//
__ri void vtlb_ReassignHandler(vtlbHandler rv, vtlbMemR8FP *r8, vtlbMemR16FP *r16, vtlbMemR32FP *r32, vtlbMemR64FP *r64,
                               vtlbMemR128FP *r128, vtlbMemW8FP *w8, vtlbMemW16FP *w16, vtlbMemW32FP *w32,
                               vtlbMemW64FP *w64, vtlbMemW128FP *w128)
{
...
//
__ri vtlbHandler vtlb_RegisterHandler(vtlbMemR8FP *r8, vtlbMemR16FP *r16, vtlbMemR32FP *r32, vtlbMemR64FP *r64,
                                      vtlbMemR128FP *r128, vtlbMemW8FP *w8, vtlbMemW16FP *w16, vtlbMemW32FP *w32,
                                      vtlbMemW64FP *w64, vtlbMemW128FP *w128)
{
...
#define VTLB_BuildUnmappedHandler(baseName, highBit)                                                                   \
    baseName##ReadSm<mem8_t, 0>, baseName##ReadSm<mem16_t, 0>, baseName##ReadSm<mem32_t, 0>,                           \
        baseName##ReadLg<mem64_t, 0>, baseName##ReadLg<mem128_t, 0>, baseName##WriteSm<mem8_t, 0>,                     \
        baseName##WriteSm<mem16_t, 0>, baseName##WriteSm<mem32_t, 0>, baseName##WriteLg<mem64_t, 0>,                   \
        baseName##WriteLg<mem128_t, 0>
    ...
    // there must be a way to get the full address back.Thats why i use these 2 functions and encode the hi bit directly
    // into em :)
...
{
    return L"Your system is too low on virtual resources for PCSX2 to run. This can be caused by having a small or "
           L"disabled swapfile, or by other programs that are hogging resources.";
}
// --------------------------------------------------------------------------------------
//  VtlbMemoryReserve  (implementations)
// --------------------------------------------------------------------------------------
VtlbMemoryReserve::VtlbMemoryReserve(const wxString &name, size_t size) : m_reserve(name, size)
{
