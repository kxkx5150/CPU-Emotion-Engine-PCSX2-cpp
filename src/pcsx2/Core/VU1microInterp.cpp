...
static void _vu1Exec(VURegs *VU)
{
    ...
    // VUM_LOG("VU->cycle = %d (flags st=%x;mac=%x;clip=%x,q=%f)", VU->cycle, VU->statusflag, VU->macflag, VU->clipflag,
    // VU->q.F);
    ...
    if (ptr[1] & 0x80000000)    // I Flag (Lower op is a float)
    {
        ...
        if (uregs.VFwrite) {
            if (lregs.VFwrite == uregs.VFwrite) {
                // Console.Warning("*PCSX2*: Warning, VF write to the same reg in both lower/upper cycle pc=%x",
                // VU->VI[REG_TPC].UL);
                discard = 1;
            }
        ...
        if (uregs.VIwrite & (1 << REG_CLIP_FLAG)) {
            if (lregs.VIwrite & (1 << REG_CLIP_FLAG)) {
                // Console.Warning("*PCSX2*: Warning, VI write to the same reg in both lower/upper cyclepc=%x",
                // VU->VI[REG_TPC].UL);
                discard = 1;
            }
    ...
    if (VU->ebit > 0) {
        if (VU->ebit-- == 1) {
            ...
            // We also have some code to update it in VIF Unpacks too, since in some games (Aggressive Inline) overwrite
            // the XGKick data VU currently flushes XGKICK on end, so this isn't needed, yet
            if (INSTANT_VU1)
