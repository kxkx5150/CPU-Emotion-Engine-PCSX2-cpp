...
__fi uint intcInterrupt()
{
    ...
    if ((psHu32(INTC_STAT) & psHu32(INTC_MASK)) == 0) {
        // DevCon.Warning("*PCSX2*: No valid interrupt INTC_MASK: %x INTC_STAT: %x", psHu32(INTC_MASK),
        // psHu32(INTC_STAT));
        return 0;
    }
...
{
    if (((psHu16(DMAC_STAT + 2) & psHu16(DMAC_STAT)) == 0) && (psHu16(DMAC_STAT) & 0x8000) == 0) {
    ...
    // DMA_LOG("dmacInterrupt %x", ((psHu16(DMAC_STAT + 2) & psHu16(DMAC_STAT)) | (psHu16(DMAC_STAT) & 0x8000)));
...
__ri bool hwMFIFOWrite(u32 addr, const u128 *data, uint qwc)
{
    ...
    if (u128 *dst = (u128 *)PSM(dmacRegs.rbor.ADDR)) {
        ...
        pxAssertMsg(PSM(dmacRegs.rbor.ADDR + ringsize - 1) != NULL,
                    "Scratchpad/MFIFO ringbuffer spans into invalid (unmapped) physical memory!");
...
__ri void hwMFIFOResume(u32 transferred)
{
    ...
    switch (dmacRegs.ctrl.MFD) {
        ...
        {
            // SPR_LOG("Added %x qw to mfifo, Vif CHCR %x Stalled %x done %x", transferred, vif1ch.chcr._u32,
            // vif1.vifstalled.enabled, vif1.done);
            if (vif1.inprogress & 0x10) {
                ...
                if (vif1ch.chcr.STR && !(cpuRegs.interrupt & (1 << DMAC_MFIFO_VIF)) && !vif1Regs.stat.INT) {
                    ...
                    // Need to simulate the time it takes to copy here, if the VIF resumes before the SPR has finished,
                    // it isn't happy.
...
Reason for this:- Many games  (such as clock tower 3 and FFX Videos) watched the TADR to see when a transfer has
finished, so we need to simulate this wherever we can!  Even the FFX video gets corruption and tries to fire multiple
DMA Kicks if this doesnt happen, which was the reasoning for the hacked up SPR timing we had, that is no longer
required.
