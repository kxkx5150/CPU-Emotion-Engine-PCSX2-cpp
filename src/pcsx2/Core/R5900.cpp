...
void cpuTlbMiss(u32 addr, u32 bd, u32 excode)
{
    ...
    if (Cpu != &intCpu || IsDebugBuild) {
        Console.Error("cpuTlbMiss pc:%x, cycl:%x, addr: %x, status=%x, code=%x", cpuRegs.pc, cpuRegs.cycle, addr,
                      cpuRegs.CP0.n.Status.val, excode);
    }
...
static __fi void _cpuTestInterrupts()
{
    ...
    if (cpuRegs.interrupt &
        ((1 << DMAC_VIF0) | (1 << DMAC_FROM_IPU) | (1 << DMAC_TO_IPU) | (1 << DMAC_FROM_SPR) | (1 << DMAC_TO_SPR) |
         (1 << DMAC_MFIFO_VIF) | (1 << DMAC_MFIFO_GIF) | (1 << VIF_VU0_FINISH) | (1 << VIF_VU1_FINISH))) {
...
static __fi void _cpuTestTIMR()
{
    ...
    if ((cpuRegs.CP0.n.Status.val & 0x8000) && cpuRegs.CP0.n.Count >= cpuRegs.CP0.n.Compare &&
...
{
    bool IntType =
        !!(cpuRegs.CP0.n.Status.val & Interrupt);    // Choose either INTC or DMAC, depending on what called it

    return IntType && cpuRegs.CP0.n.Status.b.EIE && cpuRegs.CP0.n.Status.b.IE && !cpuRegs.CP0.n.Status.b.EXL &&
...
__fi void _cpuEventTest_Shared()
{
    ...
    // where a DMA buffer is overwritten without waiting for the transfer to end, which causes the fonts to get all
    // messed up so to fix it, we run all the DMA's instantly when in the BIOS. Only use the lower 17 bits of the
    // cpuRegs.interrupt as the upper bits are for VU0/1 sync which can't be done in a tight loop
...
__fi void cpuTestDMACInts()
{
    ...
    if (((psHu16(0xe012) & psHu16(0xe010)) == 0) && ((psHu16(0xe010) & 0x8000) == 0))
        return;
...
// Count arguments, save their starting locations, and replace the space separators with null terminators so they're
// separate strings
    ...
    for (int i = 0; i < args_len; i++) {
        ...
        else if (wasSpace)    // then we're at a new arg
        {
            if (argc < kMaxArgs) {
                ...
                Console.WriteLn("ParseArgumentString: Discarded additional arguments beyond the maximum of %d.",
                                kMaxArgs);
                break;
            }
        }
...
void __fastcall eeloadHook()
{
    ...
    if (argc)    // calls to EELOAD *after* the first one during the startup process will come here
    {
#if DEBUG_LAUNCHARG
        Console.WriteLn(
            "eeloadHook: EELOAD was called with %d arguments according to $a0 and %d according to vargs block:", argc,
            memRead32(cpuRegs.GPR.n.a1.UD[0] - 4));
        for (int a = 0; a < argc; a++)
            Console.WriteLn("argv[%d]: %p -> %p -> '%s'", a, cpuRegs.GPR.n.a1.UL[0] + (a * 4),
                            memRead32(cpuRegs.GPR.n.a1.UD[0] + (a * 4)),
                            (char *)PSM(memRead32(cpuRegs.GPR.n.a1.UD[0] + (a * 4))));
#endif
        if (argc > 1)
            elfname = (char *)PSM(
                memRead32(cpuRegs.GPR.n.a1.UD[0] + 4));    // argv[1] in OSDSYS's invocation "EELOAD <game ELF>"

        // This code fires if the user chooses "full boot". First the Sony Computer Entertainment screen appears. This
        // is the result of an EELOAD call that does not want to accept launch arguments (but we patch it to do so in
        // eeloadHook2() in fast boot mode). Then EELOAD is called with the argument "rom0:PS2LOGO". At this point, we
        // do not need any additional tricks because EELOAD is now ready to accept launch arguments. So in full-boot
        // mode, we simply wait for PS2LOGO to be called, then we add the desired launch arguments. PS2LOGO passes those
        // on to the game itself as it calls EELOAD a third time.
        if (!EmuConfig.CurrentGameArgs.empty() && !strcmp(elfname.c_str(), "rom0:PS2LOGO")) {
            const char *argString = EmuConfig.CurrentGameArgs.c_str();
            Console.WriteLn("eeloadHook: Supplying launch argument(s) '%s' to module '%s'...", argString,
                            elfname.c_str());

            // Join all arguments by space characters so they can be processed as one string by ParseArgumentString(),
            // then add the user's launch arguments onto the end
            u32 arg_ptr = 0;
            int arg_len = 0;
            for (int a = 0; a < argc; a++) {
        ...
        // else it's presumed that the invocation is "EELOAD <game ELF> <<launch args>>", coming from PS2LOGO, and we
        // needn't do anything more
    }
#if DEBUG_LAUNCHARG
    // This code fires in full/fast boot mode when EELOAD is called the first/only time. When EELOAD is not given any
    // arguments, it calls rom0:OSDSYS by default, which displays the Sony Computer Entertainment screen. OSDSYS then
    // calls "EELOAD rom0:PS2LOGO" and we end up above.
    else
        Console.WriteLn("eeloadHook: EELOAD was called with no arguments.");
#endif

    // If "fast boot" was chosen, then on EELOAD's first call we won't yet know what the game's ELF is. Find the name
    // and write it into EELOAD's memory.
    if (g_SkipBiosHack && elfname.empty()) {
        ...
        // When fast-booting, we insert the game's ELF name into EELOAD so that the game is called instead of the
        // default call of "rom0:OSDSYS"; any launch arguments supplied by the user will be inserted into EELOAD later
        // by eeloadHook2()
        if (!elftoload.empty()) {
            // Find and save location of default/fallback call "rom0:OSDSYS"; to be used later by eeloadHook2()
            for (g_osdsys_str = EELOAD_START; g_osdsys_str < EELOAD_START + EELOAD_SIZE;
                 g_osdsys_str += 8)    // strings are 64-bit aligned
            {
...
void __fastcall eeloadHook2()
{
    ...
    const char *argString = EmuConfig.CurrentGameArgs.c_str();
    Console.WriteLn("eeloadHook2: Supplying launch argument(s) '%s' to ELF '%s'.", argString,
                    (char *)PSM(g_osdsys_str));

    // Add args string after game's ELF name that was written over "rom0:OSDSYS" by eeloadHook(). In between the ELF
    // name and args string we insert a space character so that ParseArgumentString() has one continuous string to
    // process.
