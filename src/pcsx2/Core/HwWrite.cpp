...
template <uint page> void __fastcall _hwWrite32(u32 mem, u32 value)
{
    pxAssume((mem & 0x03) == 0);
    ...
    switch (page) {
        ...
        case 0x0f: {
            switch (HELPSWITCH(mem)) {
                mcase(INTC_STAT) : psHu32(INTC_STAT) &= ~value;
                // cpuTestINTCInts();
                return;

                mcase(INTC_MASK) : psHu32(INTC_MASK) ^= (u16)value;
                cpuTestINTCInts();
                return;
                ...
                mcase(SBUS_F220) : psHu32(mem) |= value;
                return;

                mcase(SBUS_F230) : psHu32(mem) &= ~value;
                return;

                mcase(SBUS_F240) : if (value & (1 << 19))
                {
                ...
                // TODO: psx handling is done in the default case. Keep the code until we decide if we decide which
                // interface to use (sif2/Pgif dma)
#if 0
				mcase(SBUS_F300) :
                ...
                mcase(MCH_RICM)
                    :    // MCH_RICM: x:4|SA:12|x:5|SDEV:1|SOP:4|SBC:1|SDEV:5
                      if ((((value >> 16) & 0xFFF) == 0x21) && (((value >> 6) & 0xF) == 1) &&
                          (((psHu32(0xf440) >> 7) & 1) == 0))    // INIT & SRP=0
                ...
                mcase(DMAC_ENABLEW) : if (!dmacWrite32<0x0f>(DMAC_ENABLEW, value)) return;
                break;

                default:
                    // TODO: psx add the real address in a sbus mcase
                    if (((mem & 0x1FFFFFFF) >= EEMemoryMap::SBUS_PS1_Start) &&
                        ((mem & 0x1FFFFFFF) < EEMemoryMap::SBUS_PS1_End)) {
...
template <uint page> void __fastcall hwWrite32(u32 mem, u32 value)
{
...
template <uint page> void __fastcall _hwWrite16(u32 mem, u16 value)
{
    pxAssume((mem & 0x01) == 0);
#if PSX_EXTRALOGS
    if ((mem & 0x1000ff00) == 0x1000f300)
...
template <uint page> void __fastcall hwWrite16(u32 mem, u16 value)
{
    eeHwTraceLog(mem, value, false);
    _hwWrite16<page>(mem, value);
}

template <uint page> void __fastcall _hwWrite64(u32 mem, const mem64_t *srcval)
{
    pxAssume((mem & 0x07) == 0);
...
template <uint page> void __fastcall hwWrite64(u32 mem, const mem64_t *srcval)
{
    eeHwTraceLog(mem, *srcval, false);
    _hwWrite64<page>(mem, srcval);
}

template <uint page> void __fastcall _hwWrite128(u32 mem, const mem128_t *srcval)
{
    pxAssume((mem & 0x0f) == 0);
...
template <uint page> void __fastcall hwWrite128(u32 mem, const mem128_t *srcval)
{
    eeHwTraceLog(mem, *srcval, false);
