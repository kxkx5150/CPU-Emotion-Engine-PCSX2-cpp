...
ElfObject::ElfObject(const wxString &srcfile, IsoFile &isofile)
    : data(wxULongLong(isofile.getLength()).GetLo(), L"ELF headers"), proghead(NULL), secthead(NULL), filename(srcfile),
...
ElfObject::ElfObject(const wxString &srcfile, uint hdrsize)
    : data(wxULongLong(hdrsize).GetLo(), L"ELF headers"), proghead(NULL), secthead(NULL), filename(srcfile),
      header(*(ELF_HEADER *)data.GetPtr())
{
    isCdvd = false;
    checkElfSize(data.GetSizeInBytes());
...
bool ElfObject::hasProgramHeaders()
{
    return (proghead != NULL);
}
bool ElfObject::hasSectionHeaders()
{
    return (secthead != NULL);
}
bool ElfObject::hasHeaders()
{
...
{
    return "Cannot load ELF binary image.  The file may be corrupt or incomplete.\n\n";
}
...
void ElfObject::checkElfSize(s64 elfsize)
{
    ...
    if (elfsize > 0xfffffff)
        diagMsg = L"Illegal ELF file size over 2GB!";
    else if (elfsize == -1)
        diagMsg = L"ELF file does not exist!";
    else if (elfsize == 0)
        diagMsg = L"Unexpected end of ELF file.";

    if (diagMsg)
        throw Exception::BadStream(filename).SetDiagMsg(diagMsg).SetUserMsg(GetMsg_InvalidELF());
}
...
void ElfObject::loadSectionHeaders()
{
    ...
    // const u8 *sections_names = data.GetPtr(secthead[(header.e_shstrndx == 0xffff ? 0 :
    // header.e_shstrndx)].sh_offset);
...
int GetPS2ElfName(wxString &name)
{
    ...
    try {
        ...
        while (!file.eof()) {
            ...
            if (parts.lvalue.IsEmpty() && parts.rvalue.IsEmpty())
                continue;
            if (parts.rvalue.IsEmpty() &&
                file.getLength() != file.getSeekPos()) {    // Some games have a character on the last line of the file,
