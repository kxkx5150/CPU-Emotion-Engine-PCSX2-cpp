...
static void __fastcall psxDmaGeneric(u32 madr, u32 bcr, u32 chcr, u32 spuCore)
{
    ...
    chcr, madr, bcr); if (chcr & 0x40000000) DevCon.Warning("SPU 2 DMA %c Unusual bit set on 'to' direction chcr = %x
    madr = %x bcr = %x\n", dmaNum, chcr, madr, bcr); if ((chcr & 0x1) == 0) DevCon.Status("SPU 2 DMA %c loading from
    spu2 memory chcr = %x madr = %x bcr = %x\n", dmaNum, chcr, madr, bcr);*/
    ...
    if ((g_iopNextEventCycle - psxNextsCounter) > (u32)psxNextCounter) {
        // DevCon.Warning("SPU2async Setting new counter branch, old %x new %x ((%x - %x = %x) > %x delta)",
        // g_iopNextEventCycle, psxNextsCounter + psxNextCounter, g_iopNextEventCycle, psxNextsCounter,
