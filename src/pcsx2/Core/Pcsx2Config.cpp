...
const wxChar *const tbl_SpeedhackNames[] = {L"mvuFlag", L"InstantVU1"};
...
#ifdef PCSX2_CORE
    static const char *AspectRatioNames[] = {"Stretch", "4:3", "16:9",
                                             // WARNING: array must be NULL terminated to compute it size
                                             NULL};

    wrap.EnumEntry("AspectRatio", AspectRatio, AspectRatioNames, AspectRatio);

    static const char *FMVAspectRatioSwitchNames[] = {"Off", "4:3", "16:9",
                                                      // WARNING: array must be NULL terminated to compute it size
                                                      NULL};
...
const wxChar *const tbl_GamefixNames[] = {L"FpuMul",   L"FpuNegDiv", L"GoemonTlb",   L"SkipMPEG",   L"OPHFlag",
                                          L"EETiming", L"DMABusy",   L"GIFFIFO",     L"VIFFIFO",    L"VIF1Stall",
                                          L"VuAddSub", L"Ibit",      L"VUKickstart", L"VUOverflow", L"XGKick"};
...
{
    return wxString(BaseFilenames.Bios);
}

wxString Pcsx2Config::FullpathToMcd(uint slot) const
{
    return Path::Combine(appIniPath, StringUtil::UTF8StringToWxString(Mcd[slot].Filename));
}

bool Pcsx2Config::operator==(const Pcsx2Config &right) const
{
    bool equal = OpEqu(bitset) && OpEqu(Cpu) && OpEqu(GS) && OpEqu(Speedhacks) && OpEqu(Gamefixes) && OpEqu(Profiler) &&
                 OpEqu(Debugger) && OpEqu(Framerate) && OpEqu(Trace) && OpEqu(BaseFilenames) &&
