...
void BaseVUmicroCPU::ExecuteBlock(bool startUp)
{
    ...
    const int  s    = EmuConfig.Gamefixes.VUKickstartHack
                          ? 16
                          : 0;    // Kick Start Cycles (Jak needs at least 4 due to writing values after they're read
    ...
    } else             // Continue Executing
    {
        ...
        if (EmuConfig.Gamefixes.VUKickstartHack) {
            ...
            if (delta >= nextblockcycles &&
                delta > 0)    // When running behind, make sure we have enough cycles passed for the block to run
                Execute(delta);
        }
    }
}
