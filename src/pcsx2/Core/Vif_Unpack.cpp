...
template <uint idx, uint mode, bool doMask> static __ri void writeXYZW(u32 offnum, u32 &dest, u32 data)
{
    int n = 0;
    ...
    if (doMask) {
        ...
        switch (vif.cl) {
            case 0:
                n = (regs.mask >> (offnum * 2)) & 0x3;
                break;
            case 1:
                n = (regs.mask >> (8 + (offnum * 2))) & 0x3;
                break;
            case 2:
                n = (regs.mask >> (16 + (offnum * 2))) & 0x3;
                break;
            default:
                n = (regs.mask >> (24 + (offnum * 2))) & 0x3;
                break;
        }
    }
        ...
        case 0:
            switch (mode) {
                case 1:
                    ...
                    break;
                case 2:
                    dest = setVifRow(vif, offnum, vif.MaskRow._u32[offnum] + data);
                    break;
                case 3:
                    dest = setVifRow(vif, offnum, data);
                    break;
                default:
                    dest = data;
                    break;
            ...
            break;
        case 1:
            dest = vif.MaskRow._u32[offnum];
            break;
        case 2:
            dest = vif.MaskCol._u32[std::min(vif.cl, 3)];
            break;
        case 3:
            break;
    }
...
template <uint idx, uint mode, bool doMask, class T> static void __fastcall UNPACK_S(u32 *dest, const T *src)
{
    u32 data = *src;
...
template <uint idx, uint mode, bool doMask, class T> static void __fastcall UNPACK_V2(u32 *dest, const T *src)
{
...
template <uint idx, uint mode, bool doMask, class T> static void __fastcall UNPACK_V4(u32 *dest, const T *src)
{
    writeXYZW<tParam>(OFFSET_X, *(dest + 0), *(src + 0));
    writeXYZW<tParam>(OFFSET_Y, *(dest + 1), *(src + 1));
...
template <uint idx, bool doMask> static void __fastcall UNPACK_V4_5(u32 *dest, const u32 *src)
{
    u32 data = *src;
...
#define UnpackV4_5set(idx, doMask) (UNPACKFUNCTYPE) _unpk(u, 32) UNPACK_V4_5<idx, doMask>

#define UnpackModeSet(idx, mode)                                                                                       \
    UnpackFuncSet(S, idx, mode, s, 0), NULL, UnpackFuncSet(V2, idx, mode, s, 0), NULL,                                 \
        UnpackFuncSet(V4, idx, mode, s, 0), NULL, UnpackFuncSet(V4, idx, mode, s, 0), UnpackV4_5set(idx, 0),           \
                                                                                                                       \
        UnpackFuncSet(S, idx, mode, s, 1), NULL, UnpackFuncSet(V2, idx, mode, s, 1), NULL,                             \
        UnpackFuncSet(V4, idx, mode, s, 1), NULL, UnpackFuncSet(V4, idx, mode, s, 1), UnpackV4_5set(idx, 1),           \
                                                                                                                       \
        UnpackFuncSet(S, idx, mode, u, 0), NULL, UnpackFuncSet(V2, idx, mode, u, 0), NULL,                             \
        UnpackFuncSet(V4, idx, mode, u, 0), NULL, UnpackFuncSet(V4, idx, mode, u, 0), UnpackV4_5set(idx, 0),           \
                                                                                                                       \
        UnpackFuncSet(S, idx, mode, u, 1), NULL, UnpackFuncSet(V2, idx, mode, u, 1), NULL,                             \
        UnpackFuncSet(V4, idx, mode, u, 1), NULL, UnpackFuncSet(V4, idx, mode, u, 1), UnpackV4_5set(idx, 1)

__aligned16 const UNPACKFUNCTYPE VIFfuncTable[2][4][4 * 4 * 2 * 2] = {
    {{UnpackModeSet(0, 0)}, {UnpackModeSet(0, 1)}, {UnpackModeSet(0, 2)}, {UnpackModeSet(0, 3)}},

    {{UnpackModeSet(1, 0)}, {UnpackModeSet(1, 1)}, {UnpackModeSet(1, 2)}, {UnpackModeSet(1, 3)}}};
...
_vifT void vifUnpackSetup(const u32 *data)
{
    ...
    if (wl <= vifXRegs.cycle.cl) {    // Skipping write
        ...
        int n = vifXRegs.cycle.cl * (vifNum / wl) + _limit(vifNum % wl, vifXRegs.cycle.cl);
