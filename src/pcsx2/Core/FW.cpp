...
void FWwrite32(u32 addr, u32 value)
{
    switch (addr) {
        ...
        // PHY access
        case 0x1f808414:
            // If in read mode (top bit set) we read the PHY register requested then set the RRx interrupt if it's
            // enabled Im presuming we send that back to pcsx2 then. This register stores the result, plus whatever was
            // written (minus the read/write flag
