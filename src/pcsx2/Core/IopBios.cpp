...
class HostDir : public IOManDir {
  public:
    ...
    HostDir(ghc::filesystem::directory_iterator native_dir) : dir(native_dir)
    {
    }
...
struct filedesc
{
    ...
    operator bool() const
    {
        return type != FILE_FREE;
    }
    operator IOManFile *() const
    {
        return type == FILE_FILE ? file : NULL;
    }
    operator IOManDir *() const
    {
        return type == FILE_DIR ? dir : NULL;
    }
...
void RegisterLibraryEntries_DEBUG()
{
    ...
    DevCon.WriteLn(Color_Gray, "RegisterLibraryEntries: %8.8s version %x.%02x", modname.data(),
                   (unsigned)iopMemRead8(a0 + 9), (unsigned)iopMemRead8(a0 + 8));
}
}    // namespace loadcore
