...
__fi void vif0FBRST(u32 value)
{
    ...
    if (value & 0x1)    // Reset Vif.
    {
        ...
        vif0Regs.err.reset();
        vif0Regs.stat.clear_flags(VIF0_STAT_FQC | VIF0_STAT_INT | VIF0_STAT_VSS | VIF0_STAT_VIS | VIF0_STAT_VFS |
                                  VIF0_STAT_VPS);    // FQC=0
    }

    /* Fixme: Forcebreaks are pretty unknown for operation, presumption is it just stops it what its doing
              usually accompanied by a reset, but if we find a broken game which falls here, we need to see it!
       (Refraction) */
    ...
    {
        // Not completely sure about this, can't remember what game, used this, but 'draining' the VIF helped it,
        // instead of
    ...
    if (value & 0x8)    // Cancel Vif Stall.
    {
        ...
        /* Cancel stall, first check if there is a stall to cancel, and then clear VIF0_STAT VSS|VFS|VIS|INT|ER0|ER1
         * bits */
        if (vif0Regs.stat.test(VIF0_STAT_VSS | VIF0_STAT_VIS | VIF0_STAT_VFS))
            cancel = true;

        vif0Regs.stat.clear_flags(VIF0_STAT_VSS | VIF0_STAT_VFS | VIF0_STAT_VIS | VIF0_STAT_INT | VIF0_STAT_ER0 |
                                  VIF0_STAT_ER1);
        if (cancel) {
            g_vif0Cycles = 0;
...
__fi void vif1FBRST(u32 value)
{
    ...
    if (FBRST(value).RST)    // Reset Vif.
    {
        ...
        // Must Preserve Row/Col registers! (Downhill Domination for testing) - Really shouldnt be part of the
        // vifstruct.
    ...
    {
        // Not completely sure about this, can't remember what game used this, but 'draining' the VIF helped it, instead
        // of
        //   just stoppin the VIF (linuz).
    ...
    if (FBRST(value).STC)    // Cancel Vif Stall.
    {
        ...
        /* Cancel stall, first check if there is a stall to cancel, and then clear VIF1_STAT VSS|VFS|VIS|INT|ER0|ER1
         * bits */
        if (vif1Regs.stat.test(VIF1_STAT_VSS | VIF1_STAT_VIS | VIF1_STAT_VFS)) {
            cancel = true;
        }

        vif1Regs.stat.clear_flags(VIF1_STAT_VSS | VIF1_STAT_VFS | VIF1_STAT_VIS | VIF1_STAT_INT | VIF1_STAT_ER0 |
                                  VIF1_STAT_ER1);
...
__fi void vif1STAT(u32 value)
{
    ...
    if ((vif1Regs.stat.FDR) ^ ((tVIF_STAT &)value).FDR) {
        ...
        if (vif1Regs.stat.test(VIF1_STAT_INT | VIF1_STAT_VSS | VIF1_STAT_VIS | VIF1_STAT_VFS)) {
            DbgCon.WriteLn("changing dir when vif1 fifo stalled done = %x qwc = %x stat = %x", vif1.done, vif1ch.qwc,
                           vif1Regs.stat._u32);
            isStalled = true;
        }
    ...
    if (vif1Regs.stat.FDR)    // Vif transferring to memory.
    {
        ...
        // One of the registers told about this is TRXREG which tells us how much data is going to transfer (th x tw) in
        // words As far as the GS is concerned, the transfer starts as soon as TRXDIR is accessed, which is why fatal
        // frame was expecting data, the GS should already be sending it over (buffering in the FIFO)
