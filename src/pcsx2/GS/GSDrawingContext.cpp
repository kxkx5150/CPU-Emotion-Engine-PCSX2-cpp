...
static int findmax(int tl, int br, int limit, int wm, int minuv, int maxuv)
{
    ...
    if (wm == CLAMP_CLAMP) {
        ...
        if (tl < 0)
            ...
            uv = std::min(uv, minuv) |
                 maxuv;    // (any & mask) cannot be larger than mask, select br if that is smaller (not br & mask
                           // because there might be a larger value between tl and br when &'ed with the mask)
    }
...
GIFRegTEX0 GSDrawingContext::GetSizeFixedTEX0(const GSVector4 &st, bool linear, bool mipmap)
{
    ...
    // find the optimal value for TW/TH by analyzing vertex trace and clamping values, extending only for region modes
    // where uv may be outside
    ...
    if (tw + th >= 19)    // smaller sizes aren't worth, they just create multiple entries in the textue cache and the
                          // saved memory is less
    {
        tw = reduce(uv.x, tw);
        th = reduce(uv.y, th);
    }
    ...
    if ((theApp.GetCurrentRendererType() == GSRendererType::OGL_SW) && ((int)TEX0.TW != tw || (int)TEX0.TH != th)) {
        GL_DBG("FixedTEX0 %05x %d %d tw %d=>%d th %d=>%d st (%.0f,%.0f,%.0f,%.0f) uvmax %d,%d wm %d,%d (%d,%d,%d,%d)",
               (int)TEX0.TBP0, (int)TEX0.TBW, (int)TEX0.PSM, (int)TEX0.TW, tw, (int)TEX0.TH, th, uvf.x, uvf.y, uvf.z,
               uvf.w, uv.x, uv.y, wms, wmt, minu, maxu, minv, maxv);
    }
...
void GSDrawingContext::ComputeFixedTEX0(const GSVector4 &st)
{
    ...
    // therefore we remove the reduce optimization and we don't handle bilinear filtering which might create wrong
    // interpolation at the border.
    ...
    if ((tw != (int)TEX0.TW) || (th != (int)TEX0.TH)) {
        ...
        GL_DBG("FixedTEX0 TW %d=>%d, TH %d=>%d wm %d,%d", (int)stack.TEX0.TW, (int)TEX0.TW, (int)stack.TEX0.TH,
               (int)TEX0.TH, (int)CLAMP.WMS, (int)CLAMP.WMT);
    }
}
