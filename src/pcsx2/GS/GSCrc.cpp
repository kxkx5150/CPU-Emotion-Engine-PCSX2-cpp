...
CRC::Game CRC::m_games[] = {
    // Note: IDs 0x7ACF7E03, 0x7D4EA48F, 0x37C53760 - shouldn't be added as it's from the multiloaders when packing
    // games.
    ...
    {0x8192A241, StarOcean3, JP,
     0},    // NTSC JP special directors cut limited extra sugar on top edition (the special one :p)
...
// The exclusions list is a comma separated list of: the word "all" and/or CRCs in standard hex notation (0x and 8
// digits with leading 0's if required). The list is case insensitive and order insensitive. E.g. Disable all CRC hacks:
// CrcHacksExclusions=all E.g. Disable hacks for these CRCs:   CrcHacksExclusions=0x0F0C4A9C, 0x0EE5646B, 0x7ACF7E03
            ...
            if (!IsCrcExcluded(exclusions, m_games[i].crc)) {
                if (m_map[m_games[i].crc]) {
                    printf("[FIXME] GS: Duplicate CRC: 0x%08X: (game-id/region-id) %d/%d overrides %d/%d\n",
                           m_games[i].crc, m_games[i].title, m_games[i].region, m_map[m_games[i].crc]->title,
                           m_map[m_games[i].crc]->region);
                    crcDups++;
                }
