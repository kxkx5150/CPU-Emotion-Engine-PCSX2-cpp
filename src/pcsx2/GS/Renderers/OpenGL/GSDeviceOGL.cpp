...
GSDeviceOGL::GSDeviceOGL()
    : m_force_texture_clear(0), m_fbo(0), m_fbo_read(0), m_apitrace(0), m_palette_ss(0), m_shader(NULL)
{
...
{
    if (format == 0)
        format = (type == GSTexture::DepthStencil || type == GSTexture::SparseDepthStencil) ? GL_DEPTH32F_STENCIL8
                                                                                            : GL_RGBA8;
...
bool GSDeviceOGL::Create(const WindowInfo &wi)
{
    ...
    // ****************************************************************
    {
        ...
        if (!m_vertex_stream_buffer || !m_index_stream_buffer || !m_vertex_uniform_stream_buffer ||
...
void GSDeviceOGL::DrawIndexedPrimitive()
{
    if (!m_disable_hw_gl_draw) {
        glDrawElementsBaseVertex(m_draw_topology, static_cast<u32>(m_index.count), GL_UNSIGNED_INT,
                                 reinterpret_cast<void *>(static_cast<u32>(m_index.start) * sizeof(u32)),
...
void GSDeviceOGL::DrawIndexedPrimitive(int offset, int count)
{
    ...
    if (!m_disable_hw_gl_draw) {
        glDrawElementsBaseVertex(
            m_draw_topology, count, GL_UNSIGNED_INT,
...
GSDepthStencilOGL *GSDeviceOGL::CreateDepthStencil(OMDepthStencilSelector dssel)
{
    ...
    if (dssel.ztst != ZTST_ALWAYS || dssel.zwe) {
        static const GLenum ztst[] = {GL_NEVER, GL_ALWAYS, GL_GEQUAL, GL_GREATER};
        dss->EnableDepth();
...
void GSDeviceOGL::SelfShaderTestRun(const std::string &dir, const std::string &file, const PSSelector &sel,
                                    int &nb_shader)
{
#ifdef __unix__
...
void GSDeviceOGL::SelfShaderTestPrint(const std::string &test, int &nb_shader)
{
    fprintf(stderr, "%-25s\t\t%d shaders:\t%d instructions (M %4.2f)\t%d registers (M %4.2f)\n", test.c_str(),
            nb_shader, m_shader_inst, (float)m_shader_inst / (float)nb_shader, m_shader_reg,
            (float)m_shader_reg / (float)nb_shader);
    m_shader_inst = 0;
    m_shader_reg  = 0;
    nb_shader     = 0;
}
    ...
    for (int colclip = 0; colclip < 2; colclip++) {
        for (int fmt = 0; fmt < 3; fmt++) {
            for (int i = 0; i < 3; i++) {
                ...
                sel.dfmt     = fmt;
                std::string file =
                    format("Shader_Blend_%d_%d_%d_%d__Cclip_%d__Dfmt_%d.glsl.asm", i, ib, i, i, colclip, fmt);
                ...
                for (int aem = 0; aem < 2; aem++) {
                    for (int wms = 1; wms < 4; wms++) {
                        for (int wmt = 1; wmt < 4; wmt++) {
                            ...
                            sel.wmt       = wmt;
                            std::string file =
                                format("Shader_Ltf_%d__Aem_%d__TFmt_%d__Wms_%d__Wmt_%d__DepthFmt_%d.glsl.asm", ltf, aem,
                                       fmt, wms, wmt, depth);
                            SelfShaderTestRun(test, file, sel, nb_shader);
                        }
                    }
...
void GSDeviceOGL::CopyRect(GSTexture *sTex, GSTexture *dTex, const GSVector4i &r)
{
    ...
    glCopyImageSubData(sid, GL_TEXTURE_2D, 0, r.x, r.y, 0, did, GL_TEXTURE_2D, 0, 0, 0, 0, r.width(), r.height(), 1);
}
void GSDeviceOGL::StretchRect(GSTexture *sTex, const GSVector4 &sRect, GSTexture *dTex, const GSVector4 &dRect,
                              int shader, bool linear)
{
    StretchRect(sTex, sRect, dTex, dRect, m_convert.ps[shader], linear);
}
void GSDeviceOGL::StretchRect(GSTexture *sTex, const GSVector4 &sRect, GSTexture *dTex, const GSVector4 &dRect,
                              GLuint ps, bool linear)
{
...
void GSDeviceOGL::StretchRect(GSTexture *sTex, const GSVector4 &sRect, GSTexture *dTex, const GSVector4 &dRect,
                              GLuint ps, int bs, OMColorMaskSelector cms, bool linear)
{
    if (!sTex || !dTex) {
        ASSERT(0);
        return;
    }
    const bool draw_in_depth =
        (ps == m_convert.ps[ShaderConvert_RGBA8_TO_FLOAT32] || ps == m_convert.ps[ShaderConvert_RGBA8_TO_FLOAT24] ||
...
void GSDeviceOGL::DoMerge(GSTexture *sTex[3], GSVector4 *sRect, GSTexture *dTex, GSVector4 *dRect,
...
void GSDeviceOGL::SetupCB(const VSConstantBuffer *vs_cb, const PSConstantBuffer *ps_cb)
{
    ...
    if (m_vs_cb_cache.Update(vs_cb)) {
        WriteToStreamBuffer(m_vertex_uniform_stream_buffer.get(), g_vs_cb_index, m_uniform_buffer_alignment, vs_cb,
                            sizeof(VSConstantBuffer));
    }
    if (m_ps_cb_cache.Update(ps_cb)) {
        WriteToStreamBuffer(m_fragment_uniform_stream_buffer.get(), g_ps_cb_index, m_uniform_buffer_alignment, ps_cb,
...
void GSDeviceOGL::DebugOutputToFile(GLenum gl_source, GLenum gl_type, GLuint id, GLenum gl_severity, GLsizei gl_length,
                                    const GLchar *gl_message, const void *userParam)
{
    ...
    if (GSState::s_n == 0) {
        ...
        const int status = sscanf(message.c_str(), "type: %d, local: %d, gpr: %d, inst: %d, bytes: %d", &t, &local,
                                  &gpr, &inst, &byte);
        if (status == 5) {
            m_shader_inst += inst;
            m_shader_reg += gpr;
