...
void APIENTRY TextureSubImage(GLuint texture, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height,
...
void APIENTRY CopyTextureSubImage(GLuint texture, GLint level, GLint xoffset, GLint yoffset, GLint x, GLint y,
...
void check_gl_supported_extension()
{
    ...
    // Extra
    {
        ...
        // Mandatory for the advance HW renderer effect. Unfortunately Mesa LLVMPIPE/SWR renderers doesn't support this
        // extension. Rendering might be corrupted but it could be good enough for test/virtual machine.
    ...
    if (vendor_id_intel &&
        (!GLExtension::Has("GL_ARB_texture_barrier") || !GLExtension::Has("GL_ARB_direct_state_access"))) {
...
static void check_sparse_compatibility()
{
    ...
    found_compatible_sparse_depth =
        !vendor_id_amd && is_sparse2_compatible("GL_DEPTH32F_STENCIL8", GL_DEPTH32F_STENCIL8, 128, 128);
