...
GSRendererOGL::GSRendererOGL() : GSRendererHW(new GSTextureCacheOGL(this))
{
...
void GSRendererOGL::SetupIA(const float &sx, const float &sy)
{
    ...
    GLenum     t = 0;
    const bool unscale_pt_ln =
        m_userHacks_enabled_unscale_ptln && (GetUpscaleMultiplier() != 1) && GLLoader::found_geometry_shader;
    switch (m_vt.m_primclass) {
        ...
        case GS_SPRITE_CLASS:
            ...
            //
            // Note3: Some GPUs (Happens on GT 750m, not on Intel 5200) don't properly divide by large floats (e.g.
            // FLT_MAX/FLT_MAX == 0) Lines2Sprites predivides by Q, avoiding this issue, so always use it if
            // m_vt.m_accurate_stq
            ...
            if (GLLoader::found_geometry_shader && !m_vt.m_accurate_stq &&
                (m_vertex.next > 32 || GLLoader::in_replayer))    // <=> 16 sprites (based on Shadow Hearts)
            {
                m_gs_sel.sprite = 1;
                t               = GL_LINES;
            } else {
...
void GSRendererOGL::EmulateChannelShuffle(GSTexture **rt, const GSTextureCache::Source *tex)
{
    ...
    if (m_channel_shuffle) {
        if (m_game.title == CRC::GT4 || m_game.title == CRC::GT3 || m_game.title == CRC::GTConcept ||
            m_game.title == CRC::TouristTrophy) {
...
void GSRendererOGL::EmulateBlending(bool &DATE_GL42, bool &DATE_GL45)
{
    ...
    const bool impossible_or_free_blend =
        (blend_flag & (BLEND_NO_REC | BLEND_A_MAX | BLEND_ACCU))    // Blend doesn't requires the costly barrier
    ...
    if (m_env.COLCLAMP.CLAMP == 0) {
        ...
        GL_DBG("COLCLIP Info (Blending: %d/%d/%d/%d, SW FBMASK: %d, OVERLAP: %d)", ALPHA.A, ALPHA.B, ALPHA.C, ALPHA.D,
               sw_fbmask_colclip, m_prim_overlap);
        if (free_colclip) {
...
{
    GSDeviceOGL *dev = (GSDeviceOGL *)m_dev;
    // Warning fetch the texture PSM format rather than the context format. The latter could have been corrected in the
    // texture cache for depth.
    ...
    // So of course, GTA set the linear mode, but sampling is done at texel center so it is equivalent to nearest
    // sampling
    ...
    if (complex_wms_wmt) {
        ps_cb.MskFix =
            GSVector4i(m_context->CLAMP.MINU, m_context->CLAMP.MINV, m_context->CLAMP.MAXU, m_context->CLAMP.MAXV);
    ...
    if (shader_emulated_sampler) {
        ...
        // Enable aniso only for triangles. Sprites are flat so aniso is likely useless (it would save perf for others
        // primitives).
...
void GSRendererOGL::DrawPrims(GSTexture *rt, GSTexture *ds, GSTextureCache::Source *tex)
{
    ...
    GL_PUSH("GL Draw from %d (area %d,%d => %d,%d) in %d (Depth %d) (area %d,%d => %d,%d)",
            tex && tex->m_texture ? tex->m_texture->GetID() : -1, area_in.x, area_in.y, area_in.z, area_in.w,
            rt ? rt->GetID() : -1, ds ? ds->GetID() : -1, area_out.x, area_out.y, area_out.z, area_out.w);
#endif
    ...
    if (ate_first_pass & ate_second_pass) {
        ...
        const bool commutative_depth =
            (m_om_dssel.ztst == ZTST_GEQUAL && m_vt.m_eq.z) || (m_om_dssel.ztst == ZTST_ALWAYS);
    ...
    const GSVector4i scissor =
        GSVector4i(GSVector4(rtscale).xyxy() * hacked_scissor).rintersect(GSVector4i(rtsize).zwxy());
    SetupIA(sx, sy);
...
bool GSRendererOGL::IsDummyTexture() const
{
    ...
    return (m_context->FRAME.Block() == m_context->TEX0.TBP0) && PRIM->TME && m_sw_blending &&
           m_vt.m_primclass == GS_TRIANGLE_CLASS && (m_context->FRAME.FBMSK == 0x00FFFFFF);
}
