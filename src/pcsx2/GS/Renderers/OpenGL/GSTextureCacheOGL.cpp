...
GSTextureCacheOGL::GSTextureCacheOGL(GSRenderer *r) : GSTextureCache(r)
{
}
...
void GSTextureCacheOGL::Read(Target *t, const GSVector4i &r)
{
    ...
    GL_PERF("TC: Read Back Target: %d (0x%x)[fmt: 0x%x]. Size %dx%d", t->m_texture->GetID(), TEX0.TBP0, TEX0.PSM,
            r.width(), r.height());

    GSVector4 src =
        GSVector4(r) * GSVector4(t->m_texture->GetScale()).xyxy() / GSVector4(t->m_texture->GetSize()).xyxy();

    if (GSTexture *offscreen =
            m_renderer->m_dev->CopyOffscreen(t->m_texture, src, r.width(), r.height(), fmt, ps_shader)) {
        GSTexture::GSMap m;
