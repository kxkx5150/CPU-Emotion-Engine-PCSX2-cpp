...
void Sync()
{
    ...
    if (segment_current != segment_next) {
        ...
        if (m_size > m_seg_size) {
            fprintf(stderr,
                    "BUG: PBO Map size %u is bigger than a single segment %u. Crossing more than one fence is not "
                    "supported yet, texture data may be corrupted.\n",
                    m_size, m_seg_size);
...
GSTextureOGL::GSTextureOGL(int type, int w, int h, int format, GLuint fbo_read, bool mipmap)
    : m_clean(false), m_generate_mipmap(true), m_local_buffer(nullptr), m_r_x(0), m_r_y(0), m_r_w(0), m_r_h(0),
      m_layer(0)
{
    ...
    if (m_sparse) {
        ...
        if (m_size != old_size) {
            fprintf(stderr, "Sparse texture size (%dx%d) isn't a multiple of gpu page size (%dx%d)\n", old_size.x,
                    old_size.y, m_gpu_page_size.x, m_gpu_page_size.y);
        }
    ...
    if ((GLState::available_vram < 0) && (every_512 % 512 == 0)) {
        fprintf(stderr,
                "Available VRAM is very low (%lld), a crash is expected! Enable conservative buffer allocation or "
                "reduce upscaling!\n",
...
{
    glClearTexSubImage(m_texture_id, GL_TEX_LEVEL_0, area.x, area.y, 0, area.width(), area.height(), 1, m_int_format,
                       m_int_type, data);
}
...
bool GSTextureOGL::Map(GSMap &m, const GSVector4i *_r, int layer)
{
    ...
    if (m_type == GSTexture::Offscreen) {
        ...
        glGetTextureSubImage(m_texture_id, GL_TEX_LEVEL_0, r.x, r.y, 0, r.width(), r.height(), 1, m_int_format,
                             m_int_type, m_size.x * m_size.y * 4, m_local_buffer);
#else
...
void GSTextureOGL::CommitPages(const GSVector2i &region, bool commit)
{
    ...
    if (commit) {
        if (m_committed_size.x == 0) {
            ...
            GL_INS("CommitPages extend %dx%d to %dx%d of %u", m_committed_size.x, m_committed_size.y, region.x,
                   region.y, m_texture_id);
            int w = region.x - m_committed_size.x;
            int h = region.y - m_committed_size.y;
            // Extend width
            glTexturePageCommitmentEXT(m_texture_id, GL_TEX_LEVEL_0, m_committed_size.x, 0, 0, w, m_committed_size.y, 1,
                                       commit);
            // Extend height
        ...
        glTexturePageCommitmentEXT(m_texture_id, GL_TEX_LEVEL_0, 0, 0, 0, m_committed_size.x, m_committed_size.y, 1,
                                   commit);
