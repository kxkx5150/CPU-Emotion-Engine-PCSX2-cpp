...
GSShaderOGL::GSShaderOGL(bool debug) : m_pipeline(0), m_debug_shader(debug)
{
...
{
    printf("Delete %zu Shaders, %zu Programs, %zu Pipelines\n", m_shad_to_delete.size(), m_prog_to_delete.size(),
           m_pipe_to_delete.size());
...
GLuint GSShaderOGL::Compile(const std::string &glsl_file, const std::string &entry, GLenum type,
                            const char *glsl_h_code, const std::string &macro_sel)
{
...
GLuint GSShaderOGL::CompileShader(const std::string &glsl_file, const std::string &entry, GLenum type,
                                  const char *glsl_h_code, const std::string &macro_sel)
{
    ASSERT(glsl_h_code != NULL);
