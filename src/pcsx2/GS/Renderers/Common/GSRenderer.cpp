...
GSRenderer::GSRenderer()
    : m_shader(0), m_shift_key(false), m_control_key(false), m_texture_shuffle(false), m_real_size(0, 0), m_dev(NULL)
{
...
bool GSRenderer::Merge(int field)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (en[i]) {
            ...
            // printf("[%d]: %d %d %d %d, %d %d %d %d\n", i, fr[i].x,fr[i].y,fr[i].z,fr[i].w ,
            // dr[i].x,dr[i].y,dr[i].z,dr[i].w);
        }
    }
    ...
    GL_PUSH("Renderer Merge %d (0: enabled %d 0x%x, 1: enabled %d 0x%x)", s_n, en[0], m_regs->DISP[0].DISPFB.Block(),
            en[1], m_regs->DISP[1].DISPFB.Block());

    // try to avoid fullscreen blur, could be nice on tv but on a monitor it's like double vision, hurts my eyes
    // (persona 4, guitar hero)
    //
    // NOTE: probably the technique explained in graphtip.pdf (Antialiasing by Supersampling / 4. Reading Odd/Even Scan
    // Lines Separately with the PCRTC then Blending)

    bool samesrc = en[0] && en[1] && m_regs->DISP[0].DISPFB.FBP == m_regs->DISP[1].DISPFB.FBP &&
    ...
    if (samesrc /*&& m_regs->PMODE.SLBG == 0 && m_regs->PMODE.MMOD == 1 && m_regs->PMODE.ALP == 0x80*/) {
        ...
        //
        // These kinds of cases are now fixed by the more generic frame_diff code below, as the code here was too
        // specific and has become obsolete. NOTE: Persona 4 and God Of War are not rare exceptions, many games have the
        // same(or very similar) offsets.

        int topDiff = fr[0].top - fr[1].top;
        if (dr[0].eq(dr[1]) && (fr[0].eq(fr[1] + GSVector4i(0, topDiff, 0, topDiff)) ||
                                fr[1].eq(fr[0] + GSVector4i(0, topDiff, 0, topDiff)))) {
            // dq5:
            //
            // fr[0] = 0 1 512 445
            // fr[1] = 0 0 512 444
    ...
    for (int i = 0; i < 2; i++) {
        ...
        src_hw[i] =
            (GSVector4(r) + GSVector4(0, y_offset[i], 0, y_offset[i])) * scale / GSVector4(tex[i]->GetSize()).xyxy();
        ...
        // Though ignore cases where baseline and display rectangle offsets only differ by 1 pixel, causes blurring and
        // wrong resolution output on FFXII
        if (display_diff.x > 2) {
            off.x = tex[i]->GetScale().x * display_diff.x;
        }
        // If the DX offset is too small then consider the status of frame memory offsets, prevents blurring on Tenchu:
        // Fatal Shadows, Worms 3D
    ...
    if (tex[0] || tex[1]) {
        ...
        GSVector4 c =
            GSVector4((int)m_regs->BGCOLOR.R, (int)m_regs->BGCOLOR.G, (int)m_regs->BGCOLOR.B, (int)m_regs->PMODE.ALP) /
            255;
...
GSVector4i GSRenderer::ComputeDrawRectangle(int width, int height) const
{
    ...
    return GSVector4i(static_cast<int>(std::floor(target_x)), static_cast<int>(std::floor(target_y)),
...
void GSRenderer::VSync(int field)
{
    ...
    if ((m_perfmon.GetFrame() & 0x1f) == 0) {
...
#ifdef GSTITLEINFO_API_FORCE_VERBOSE
        {
            ...
            static const char *aspect_ratio_names[static_cast<int>(AspectRatioType::MaxCount)] = {"Stretch", "4:3",
                                                                                                  "16:9"};

            std::string s2 = m_regs->SMODE2.INT
                                 ? (std::string("Interlaced ") + (m_regs->SMODE2.FFMD ? "(frame)" : "(field)"))
                                 : "Progressive";

            s = format("%lld | %d x %d | %.2f fps (%d%%) | %s - %s | %s | %d S/%d P/%d D | %d%% CPU | %.2f | %.2f",
                       m_perfmon.GetFrame(), GetInternalResolution().x, GetInternalResolution().y, fps,
                       (int)(100.0 * fps / GetTvRefreshRate()), s2.c_str(),
                       theApp.m_gs_interlace[m_interlace].name.c_str(),
                       aspect_ratio_names[static_cast<int>(EmuConfig.GS.AspectRatio)],
                       (int)m_perfmon.Get(GSPerfMon::SyncPoint), (int)m_perfmon.Get(GSPerfMon::Prim),
                       (int)m_perfmon.Get(GSPerfMon::Draw), m_perfmon.CPU(), m_perfmon.Get(GSPerfMon::Swizzle) / 1024,
...
#else
        {
            ...
            s = format("%dx%d | %s", GetInternalResolution().x, GetInternalResolution().y,
                       theApp.m_gs_interlace[m_interlace].name.c_str());
        }
#endif
        ...
        switch (e.key) {
            case VK_F5:
                ...
                printf("GS: Set deinterlace mode to %d (%s).\n", m_interlace,
                       theApp.m_gs_interlace.at(m_interlace).name.c_str());
                return;
            case VK_DELETE:
                m_aa1 = !m_aa1;
