...
void GSOsdManager::LoadFont()
{
    ...
    if (error) {
        ...
        if (theApp.LoadResource(IDR_FONT_ROBOTO, resource_data_buffer))
            error_load_res = FT_New_Memory_Face(m_library, (const FT_Byte *)resource_data_buffer.data(),
                                                resource_data_buffer.size(), 0, &m_face);
...
GSOsdManager::GSOsdManager() : m_atlas_h(0), m_atlas_w(0), m_max_width(0), m_onscreen_messages(0), m_texture_dirty(true)
{
...
size_t GSOsdManager::Size()
{
    ...
    if (m_log_enabled) {
        ...
        for (auto it = m_log.begin(); it != m_log.end(); ++it) {
            ...
            float ratio = (elapsed - std::chrono::seconds(m_log_timeout / 2)).count() /
                          std::chrono::seconds(m_log_timeout / 2).count();
...
size_t GSOsdManager::GeneratePrimitives(GSVertexPT1 *dst, size_t count)
{
    ...
    if (m_monitor_enabled) {
        ...
        for (const auto &pair : m_monitor) {
            ...
            float x = 1.0f - 8 * (2.0f / m_real_size.x) - first_max - m_char_info[' '].ax * (2.0f / m_real_size.x) -
                      second_max;
