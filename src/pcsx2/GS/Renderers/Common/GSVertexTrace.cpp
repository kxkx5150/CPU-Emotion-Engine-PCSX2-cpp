...
#define InitUpdate2(P, IIP, TME)                                                                                       \
    InitUpdate3(P, IIP, TME, 0, 0) InitUpdate3(P, IIP, TME, 0, 1) InitUpdate3(P, IIP, TME, 1, 0)                       \
        InitUpdate3(P, IIP, TME, 1, 1)

#define InitUpdate(P) InitUpdate2(P, 0, 0) InitUpdate2(P, 0, 1) InitUpdate2(P, 1, 0) InitUpdate2(P, 1, 1)
...
void GSVertexTrace::Update(const void *vertex, const uint32 *index, int v_count, int i_count, GS_PRIM_CLASS primclass)
{
    ...
    m_eq.value =
        (m_min.c == m_max.c).mask() | ((m_min.p == m_max.p).mask() << 16) | ((m_min.t == m_max.t).mask() << 20);
