...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME && (fi.FBP == 0x03400 || fi.FBP == 0x02e00) && fi.FPSM == fi.TPSM &&
            fi.TBP0 == 0x03f00 && fi.TPSM == PSM_PSMCT32) {
...
bool GSC_DeathByDegreesTekkenNinaWilliams(const GSFrameInfo &fi, int &skip)
{
    ...
    if (skip == 0) {
        ...
        if ((Aggressive || !s_nativeres) && fi.TME && (fi.FBP | fi.TBP0 | fi.FPSM | fi.TPSM) &&
            fi.FBMSK == 0x00FFFFFF) {
...
{
    if (skip == 0) {
        if (Aggressive && fi.TME && fi.TPSM == PSM_PSMCT24 && fi.FBP == 0x1300 &&
            (fi.TBP0 == 0x0F00 || fi.TBP0 == 0x1300 || fi.TBP0 == 0x2b00))    // || fi.FBP == 0x0100
        {
            // Ghosting when upscaling, HPO helps but isn't perfect.
            skip = 1;    // global haze/halo
        } else if ((Aggressive || !s_nativeres) && fi.TME && fi.TPSM == PSM_PSMCT24 &&
                   (fi.FBP == 0x0100 || fi.FBP == 0x2100) &&
                   (fi.TBP0 == 0x2b00 || fi.TBP0 == 0x2e80 || fi.TBP0 == 0x3100))    // 480P 2e80, interlaced 3100
        {
            // Upscaling issue.
            // Don't enable hack on native res if crc is below aggressive.
...
{
    if (skip == 0) {
        if (fi.TME && (fi.FBP == 0x00000 || fi.FBP == 0x008c0 || fi.FBP == 0x00a00) &&
            (fi.TBP0 == 0x00000 || fi.TBP0 == 0x008c0 || fi.TBP0 == 0x00a00) && fi.FBP == fi.TBP0 &&
            fi.FPSM == PSM_PSMCT32 && fi.FPSM == fi.TPSM) {
            return false;    // allowed
        }

        if (fi.TME && (fi.FBP == 0x01e40 || fi.FBP == 0x02200) && fi.FPSM == PSM_PSMZ24 &&
            (fi.TBP0 == 0x01180 || fi.TBP0 == 0x01400) && fi.TPSM == PSM_PSMZ24) {
            skip = 42;
        }
    } else {
        if (fi.TME && (fi.FBP == 0x00000 || fi.FBP == 0x008c0 || fi.FBP == 0x00a00) && fi.FPSM == PSM_PSMCT32 &&
            fi.TBP0 == 0x03c00 && fi.TPSM == PSM_PSMCT32) {
            skip = 0;
...
bool GSC_SacredBlaze(const GSFrameInfo &fi, int &skip)
{
    ...
    if (skip == 0) {
        if (fi.TME && (fi.FBP == 0x0000 || fi.FBP == 0x0e00) && (fi.TBP0 == 0x2880 || fi.TBP0 == 0x2a80) &&
            fi.FPSM == fi.TPSM && fi.TPSM == PSM_PSMCT32 && fi.FBMSK == 0x0) {
            skip = 1;
        }
    }
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME && fi.FBP == 0x00700 && fi.FPSM == PSM_PSMZ24 &&
            fi.TBP0 == 0x01180 && fi.TPSM == PSM_PSMZ24) {
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME && fi.TBP0 == 0x2BC0 ||
            (fi.FBP == 0 || fi.FBP == 0x1180) && (fi.FPSM | fi.TPSM) == 0) {
            // Don't enable hack on native res if crc is below aggressive.
...
{
    if (skip == 0) {
        if (!fi.TME && (fi.FBP == 0x0 || fi.FBP == 0x1180) && (fi.TBP0 != 0x3fc0 && fi.TBP0 != 0x3c9a && fi.TBP0 != 0x3dec /*fi.TBP0 ==0x38d0 || fi.TBP0==0x3912 ||fi.TBP0==0x3bdc ||fi.TBP0==0x3ab3 ||fi.TBP0<=0x3a92*/) &&
            fi.FPSM == PSM_PSMCT32 && (fi.TPSM == PSM_PSMT8 || fi.TPSM == PSM_PSMT4) &&
            (fi.FBMSK == 0x00FFFFFF || !fi.FBMSK)) {
            skip = 0;    // 3dec 3fc0 3c9a
        }
        if (!fi.TME && (fi.FBP | fi.TBP0) != 0 && (fi.FBP | fi.TBP0) != 0x1180 && (fi.FBP | fi.TBP0) != 0x3be0 &&
            (fi.FBP | fi.TBP0) != 0x3c80 && fi.TBP0 != 0x3c9a && (fi.FBP | fi.TBP0) != 0x3d80 && fi.TBP0 != 0x3dec &&
            fi.FPSM == PSM_PSMCT32 && (fi.FBMSK == 0)) {
            skip = 0;    // 3dec 3fc0 3c9a
        }
        if (!fi.TME && (fi.FBP | fi.TBP0) != 0 && (fi.FBP | fi.TBP0) != 0x1180 && (fi.FBP | fi.TBP0) != 0x3be0 &&
            (fi.FBP | fi.TBP0) != 0x3c80 && (fi.FBP | fi.TBP0) != 0x3d80 && fi.TBP0 != 0x3c9a && fi.TBP0 != 0x3de &&
            fi.FPSM == PSM_PSMCT32 && (fi.FBMSK == 0)) {
...
bool GSC_ShadowofRome(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        if (fi.FBP && fi.TPSM == PSM_PSMT8H && (fi.FBMSK == 0x00FFFFFF)) {
            ...
            // Cause a grey transparent wall (D3D) and a transparent vertical grey line (all renders) on the left side
            // of the screen. Blur effect maybe ?
            skip = 1;
        } else if (fi.TME && fi.FPSM == PSM_PSMCT32 && (fi.TBP0 == 0x0160 || fi.TBP0 == 0x01e0 || fi.TBP0 <= 0x0800) &&
                   fi.TPSM == PSM_PSMT8) {
            skip = 1;    // Speedhack ?
        }
    }
...
bool GSC_LordOfTheRingsThirdAge(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        ...
        if (fi.TME && (fi.FBP == 0x0 || fi.FBP == 0x00e00 || fi.FBP == 0x01000) && fi.FPSM == PSM_PSMCT32 &&
            fi.TBP0 == 0x03000 && fi.TPSM == PSM_PSMCT24) {
            skip = 1;
        }
    }
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME &&
            (fi.FBP == 0x02d60 || fi.FBP == 0x02d80 || fi.FBP == 0x02ea0 || fi.FBP == 0x03620 || fi.FBP == 0x03640) &&
            fi.FPSM == fi.TPSM && fi.TBP0 == 0x00000 && fi.TPSM == PSM_PSMCT32) {
            // Don't enable hack on native res if crc is below aggressive.
            // Fixes/removes ghosting/blur effect and white lines appearing in stages: Moonfit Wilderness, Acid Rain -
            // caused by upscaling. Downside is it also removes the channel effect which is fixed on OpenGL. Let's
            // enable this hack for Aggressive only since it's an upscaling issue for both renders.
            skip = 95;
        } else if (fi.TME &&
                   (fi.FBP == 0x02bc0 || fi.FBP == 0x02be0 || fi.FBP == 0x02d00 || fi.FBP == 0x03480 ||
                    fi.FBP == 0x034a0) &&
                   fi.FPSM == fi.TPSM && fi.TBP0 == 0x00000 && fi.TPSM == PSM_PSMCT32) {
            // The moving display effect(flames) is not emulated properly in the entire screen so let's remove the
            // effect in the stage: Burning Temple. Related to half screen bottom issue. Fixes black lines in the stage:
            // Burning Temple - caused by upscaling. Note the black lines can also be fixed with Merge Sprite hack.
            skip = 2;
        }
    }
...
bool GSC_TombRaiderLegend(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        // ||fi.TBP0 ==0x2F00
        if (fi.TME && fi.FBP == 0x01000 && fi.FPSM == fi.TPSM && fi.TPSM == PSM_PSMCT32 &&
            (fi.TBP0 == 0x2b60 || fi.TBP0 == 0x2b80 || fi.TBP0 == 0x2E60 || fi.TBP0 == 0x3020 || fi.TBP0 == 0x3200 ||
             fi.TBP0 == 0x3320)) {
            skip = 1;    // Garbage TC
...
bool GSC_TombRaiderUnderWorld(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        if (fi.TME && fi.FBP == 0x01000 && fi.FPSM == fi.TPSM && fi.TPSM == PSM_PSMCT32 &&
            (fi.TBP0 == 0x2B60 /*|| fi.TBP0 == 0x2EFF || fi.TBP0 ==0x2F00 || fi.TBP0 == 0x3020*/ ||
             fi.TBP0 >= 0x2C01 && fi.TBP0 != 0x3029 && fi.TBP0 != 0x302d)) {
            skip = 1;    // Garbage TC
...
bool GSC_BurnoutGames(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        if (fi.TME &&
            (fi.FBP == 0x01dc0 || fi.FBP == 0x01c00 || fi.FBP == 0x01f00 || fi.FBP == 0x01d40 || fi.FBP == 0x02200 ||
             fi.FBP == 0x02000) &&
            fi.FPSM == fi.TPSM &&
            (fi.TBP0 == 0x01dc0 || fi.TBP0 == 0x01c00 || fi.TBP0 == 0x01f00 || fi.TBP0 == 0x01d40 ||
             fi.TBP0 == 0x02200 || fi.TBP0 == 0x02000) &&
            fi.TPSM == PSM_PSMCT32) {
...
{
    if (skip == 0) {
        if (fi.TME && (fi.FBP > 0x01d00 && fi.FBP <= 0x02a00) && fi.FPSM == PSM_PSMCT32 &&
            (fi.FBP >= 0x01600 && fi.FBP < 0x03260) && fi.TPSM == PSM_PSMT8H) {
            // Vram usage.
...
bool GSC_TalesOfLegendia(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        ...
        if (fi.TME && fi.FBP == 0x1c00 && (fi.TBP0 == 0x2e80 || fi.TBP0 == 0x2d80) && fi.TPSM == 0 &&
            fi.FBMSK == 0xff000000) {
            skip = 1;    // Ghosting
        }
...
{
    if (skip == 0) {
        if (!fi.TME && (fi.FBP == 0x0 || fi.FBP == 0x00700 || fi.FBP == 0x00800) && fi.FPSM == PSM_PSMCT32 &&
            fi.FBMSK == 0x00FFFFFF) {
...
bool GSC_ZettaiZetsumeiToshi2(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        ...
        if (fi.TME && fi.TPSM == PSM_PSMT8 && (fi.FBP == 0x1180 || fi.FBP == 0) &&
            (fi.TBP0 != 0x3764 && fi.TBP0 != 0x370f)) {
            skip = 0;
        }
...
bool GSC_SakuraWarsSoLongMyLove(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        ...
        } else if (fi.TME == 0 && fi.FBP == fi.TBP0 && (fi.TBP0 == 0x1200 || fi.TBP0 == 0x1180 || fi.TBP0 == 0) &&
                   fi.FBMSK == 0x00FFFFFF) {
            skip = 3;    // Remove darkness
        } else if (fi.TME && (fi.FBP == 0 || fi.FBP == 0x1180) && fi.FPSM == PSM_PSMCT32 && fi.TBP0 == 0x3F3F &&
                   fi.TPSM == PSM_PSMT8) {
            skip = 1;    // Floodlight
        }
    }
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME && (fi.TBP0 == 0x0700 || fi.TBP0 == 0x0a80) &&
            (fi.TPSM == PSM_PSMCT32 || fi.TPSM == PSM_PSMCT24)) {
            // Don't enable hack on native res if crc is below aggressive.
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME && fi.FBP == 0x2a00 && fi.FPSM == PSM_PSMZ24 && fi.TBP0 == 0x1c00 &&
            fi.TPSM == PSM_PSMZ24) {
            // Don't enable hack on native res if crc is below aggressive.
            skip = 5;    // blur
        }
...
{
    if (skip == 0) {
        if (fi.TME && fi.FPSM == PSM_PSMCT32 && (fi.TBP0 == 0x2bc0 || fi.TBP0 <= 0x0200) &&
            (fi.FBMSK == 0xFF000000 || fi.FBMSK == 0x00FFFFFF)) {
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && fi.TME == 0 && (fi.FBP == 0x1500) &&
            (fi.TBP0 == 0x2c97 || fi.TBP0 == 0x2ace || fi.TBP0 == 0x03d0 || fi.TBP0 == 0x2448) &&
            (fi.FBMSK == 0x0000)) {
            // Don't enable hack on native res if crc is below aggressive.
...
{
    if (skip == 0) {
        if (fi.TME && fi.FBP == 0x0000 && fi.TBP0 == 0x3980 && fi.FPSM == fi.TPSM && fi.TPSM == PSM_PSMCT32 &&
            fi.FBMSK == 0x0) {
            skip = 1;    // Black shadow
        }
    }
...
{
    if (skip == 0) {
        if (!fi.TME && !(fi.FBP == 0x0 || fi.FBP == 0x00800 || fi.FBP == 0x008c0 || fi.FBP == 0x03e00) &&
            fi.FPSM == PSM_PSMCT32 && (fi.TBP0 == 0x0 || fi.TBP0 == 0x01800) && fi.TPSM == PSM_PSMCT32) {
    ...
    if (skip == 0) {
        ...
        if (fi.TME && fi.TPSM == PSM_PSMCT16S) {
            if (fi.FBP == 0x1180) {
                ...
                skip = 100;    // deletes most others(too high deletes the buggy sea completely;c, too low causes
                               // glitches to be visible)
            } else if (Aggressive && fi.FBP != 0)    // Aggressive CRC
            {
                skip = 19;    // "speedhack", makes the game very light, vaporized water can disappear when not looked
                              // at directly, possibly some interface still, other value to try: 6 breaks menu
                              // background, possibly nothing(?) during gameplay, but it's slower, hence not much of a
                              // speedhack anymore
            }
        }
    }
...
{
    if (skip == 0) {
        if ((Aggressive || !s_nativeres) && !fi.TME && (fi.FBP == 0x1c20 || fi.FBP == 0x1e20 || fi.FBP == 0x1620) &&
            (fi.TBP0 == 0xe00 || fi.TBP0 == 0x1000 || fi.TBP0 == 0x800) && fi.TPSM == PSM_PSMZ24 &&
            fi.FPSM == PSM_PSMCT32
...
{
    // Removes clouds for a good speed boost, removes both 3D clouds(invisible with Hardware renderers, but cause
    // slowdown) and 2D background clouds. Removes blur from player airplane. This hack also removes rockets, shows
    // explosions(invisible without CRC hack) as garbage data, causes flickering issues with the HUD, and in some
    // (night) missions removes the HUD altogether.
...
bool GSC_GodOfWar(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        if (fi.TME && fi.FBP == 0x00000 && fi.FPSM == PSM_PSMCT16 && fi.TBP0 == 0x00000 && fi.TPSM == PSM_PSMCT16 &&
            ...
            // Removes shadows.
            skip = 1000;
        } else if (fi.TME && fi.FBP == 0x00000 && fi.FPSM == PSM_PSMCT32 && fi.TBP0 == 0x00000 &&
                   fi.TPSM == PSM_PSMCT32 && fi.FBMSK == 0xff000000) {
...
{
    if (skip == 0) {
        if (fi.TME && fi.FBP == 0x001000 && (fi.TBP0 == 0 || fi.TBP0 == 0x0800) && fi.TPSM == PSM_PSMT8H &&
            fi.FBMSK == 0x00FFFFFF) {
        ...
        } else if (fi.TME && (fi.TBP0 == 0x1400 || fi.TBP0 == 0x1000 || fi.TBP0 == 0x1200) &&
                   (fi.TPSM == PSM_PSMCT32 || fi.TPSM == PSM_PSMCT24)) {
...
bool GSC_XenosagaE3(const GSFrameInfo &fi, int &skip)
{
    if (skip == 0) {
        if (fi.TPSM == PSM_PSMT8H && fi.FBMSK >= 0xEFFFFFFF) {
            ...
            if (fi.TME) {
                // depth textures (bully, mgs3s1 intro, Front Mission 5)
                if ((fi.TPSM == PSM_PSMZ32 || fi.TPSM == PSM_PSMZ24 || fi.TPSM == PSM_PSMZ16 ||
                     fi.TPSM == PSM_PSMZ16S) ||
                    // General, often problematic post processing
                    (GSUtil::HasSharedBits(fi.FBP, fi.FPSM, fi.TBP0, fi.TPSM))) {
                    skip = 1;
