...
GSTextureCache::GSTextureCache(GSRenderer *r) : m_renderer(r), m_palette_map(r)
{
...
GSTextureCache::Source *GSTextureCache::LookupDepthSource(const GIFRegTEX0 &TEX0, const GIFRegTEXA &TEXA,
                                                          const GSVector4i &r, bool palette)
{
    if (!m_can_convert_depth) {
        ...
        if (m_renderer->m_game.title == CRC::JackieChanAdv || m_renderer->m_game.title == CRC::SVCChaos) {
            // JackieChan and SVCChaos cause regressions when skipping the draw calls when depth is disabled/not
            // supported. This way we make sure there are no regressions on D3D as well.
    ...
    if (!dst) {
        ...
        for (auto t : m_dst[RenderTarget]) {
            ...
            if (!t->m_age && t->m_used && t->m_dirty.empty() &&
    ...
    if (dst) {
        GL_CACHE("TC depth: dst %s hit: %d (0x%x, %s)", to_string(dst->m_type),
                 dst->m_texture ? dst->m_texture->GetID() : 0, TEX0.TBP0, psm_str(psm));
        ...
        if (m_renderer->m_game.title == CRC::JackieChanAdv || m_renderer->m_game.title == CRC::SVCChaos) {
            // JackieChan and SVCChaos cause regressions when skipping the draw calls so we reuse the old code for these
            // two.
            return LookupSource(TEX0, TEXA, r);
        } else {
...
GSTextureCache::Source *GSTextureCache::LookupSource(const GIFRegTEX0 &TEX0, const GIFRegTEXA &TEXA,
                                                     const GSVector4i &r)
{
    const GSLocalMemory::psm_t &psm_s = GSLocalMemory::m_psm[TEX0.PSM];
...
#endif
    {
        ...
        // (Simply not doing this code at all makes a lot of previsouly missing stuff show (but breaks pretty much
        // everything else.)
        ...
        for (auto t : m_dst[RenderTarget]) {
            if (t->m_used && t->m_dirty.empty()) {
                ...
                if (GSUtil::HasSharedBits(bp, psm, t->m_TEX0.TBP0, t_psm)) {
                    // It is a complex to convert the code in shader. As a reference, let's do it on the CPU, it will be
                    // slow but 1/ it just works :) 2/ even with upscaling 3/ for both Direct3D and OpenGL
                    if (m_cpu_fb_conversion && (psm == PSM_PSMT4 || psm == PSM_PSMT8))
                        // Forces 4-bit and 8-bit frame buffer conversion to be done on the CPU instead of the GPU, but
                        // performance will be slower. There is no dedicated shader to handle 4-bit conversion (Stuntman
                        // has been confirmed to use 4-bit). Direct3D10/11 and OpenGL support 8-bit fb conversion but
                        // don't render some corner cases properly (Harry Potter games). The hack can fix glitches in
                        // some games.
                        Read(t, t->m_valid);
                    else
                        dst = t;
                    ...
                    break;
                } else if ((t->m_TEX0.TBW >= 16) &&
                           GSUtil::HasSharedBits(bp, psm, t->m_TEX0.TBP0 + t->m_TEX0.TBW * 0x10, t->m_TEX0.PSM)) {
                    ...
                    break;
                } else if (texture_inside_rt && psm == PSM_PSMCT32 && t->m_TEX0.PSM == psm && t->m_TEX0.TBP0 < bp &&
                           t->m_end_block >= bp) {
                        ...
                        for (int candidate_y_offset = 0; candidate_y_offset < t->m_valid.w; ++candidate_y_offset) {
                            ...
                            if (bp == candidate_bp && bp_end <= t->m_end_block) {
                                ...
                                while (m_texture_inside_rt_cache.size() > m_texture_inside_rt_cache_size) {
                                    GL_PERF("TC tex in rt: Size of cache %d too big, clearing it.",
                                ...
                                GL_CACHE("TC tex in rt: Cached HIT element (size %d), BW: %d, PSM %s, rt 0x%x <%d,%d> "
                                         "+ off <%d,%d> -> 0x%x <%d,%d> (END: 0x%x)",
                                         m_texture_inside_rt_cache.size(), bw, psm_str(psm), t->m_TEX0.TBP0,
                                         t->m_valid.z, t->m_valid.w, x_offset, y_offset, bp, tw, th, bp_end);
                                break;
                            }
                        }
                        if (dst != nullptr)
                    ...
                    GL_CACHE("TC tex in rt: Cached MISS element (size %d), BW: %d, PSM %s, rt 0x%x <%d,%d> -/-> 0x%x "
                             "<%d,%d> (END: 0x%x)",
                             m_texture_inside_rt_cache.size(), bw, psm_str(psm), t->m_TEX0.TBP0, t->m_valid.z,
                             t->m_valid.w, bp, tw, th, bp_end);
                    m_texture_inside_rt_cache.emplace_back(entry);
                }
            }
        }
    ...
    if (src == NULL) {
        ...
        if (dst) {
            GL_CACHE("TC: dst %s hit (%s): %d (0x%x, %s)", to_string(dst->m_type), half_right ? "half" : "full",
                     dst->m_texture ? dst->m_texture->GetID() : 0, TEX0.TBP0, psm_str(TEX0.PSM));
        } else {
        ...
        GL_CACHE("TC: src hit: %d (0x%x, 0x%x, %s)", src->m_texture ? src->m_texture->GetID() : 0, TEX0.TBP0,
                 psm_s.pal > 0 ? TEX0.CBP : 0, psm_str(TEX0.PSM));
    }
                                                     ...
                                                     uint32 fbmask)
{
    ...
    if (dst) {
        GL_CACHE("TC: Lookup Target(%s) %dx%d, hit: %d (0x%x, %s)", to_string(type), w, h, dst->m_texture->GetID(), bp,
                 psm_str(TEX0.PSM));
        ...
        if (dst_match) {
            ...
            if (type == DepthStencil) {
                GL_CACHE("TC: Lookup Target(Depth) %dx%d, hit Color (0x%x, %s was %s)", w, h, bp, psm_str(TEX0.PSM),
                         psm_str(dst_match->m_TEX0.PSM));
                shader = (fmt_16_bits) ? ShaderConvert_RGB5A1_TO_FLOAT16 : ShaderConvert_RGBA8_TO_FLOAT32 + psm_s.fmt;
            } else {
                GL_CACHE("TC: Lookup Target(Color) %dx%d, hit Depth (0x%x, %s was %s)", w, h, bp, psm_str(TEX0.PSM),
                         psm_str(dst_match->m_TEX0.PSM));
...
GSTextureCache::Target *GSTextureCache::LookupTarget(const GIFRegTEX0 &TEX0, int w, int h, int real_h)
{
    ...
    for (auto t : m_dst[RenderTarget]) {
        if (bp == t->m_TEX0.TBP0 && t->m_end_block >= bp) {
            ...
            GL_CACHE("TC: Lookup Frame %dx%d, perfect hit: %d (0x%x -> 0x%x %s)", w, h, dst->m_texture->GetID(), bp,
                     t->m_end_block, psm_str(TEX0.PSM));
    ...
    if (dst == NULL) {
        for (auto t : m_dst[RenderTarget]) {
            if (t->m_TEX0.TBP0 < bp && bp <= t->m_end_block) {
                ...
                GL_CACHE("TC: Lookup Frame %dx%d, inclusive hit: %d (0x%x, took 0x%x -> 0x%x %s)", w, h,
                         t->m_texture->GetID(), bp, t->m_TEX0.TBP0, t->m_end_block, psm_str(TEX0.PSM));
    ...
    if (dst == NULL) {
        for (auto t : m_dst[RenderTarget]) {
            if (bp == t->m_TEX0.TBP0) {
                ...
                GL_CACHE("TC: Lookup Frame %dx%d, empty hit: %d (0x%x -> 0x%x %s)", w, h, dst->m_texture->GetID(), bp,
                         t->m_end_block, psm_str(TEX0.PSM));
    ...
    if (dst == NULL) {
        ...
        m_renderer->m_dev->ClearRenderTarget(
            dst->m_texture, 0);    // new frame buffers after reset should be cleared, don't display memory garbage
...
void GSTextureCache::InvalidateVideoMem(GSOffset *off, const GSVector4i &rect, bool target)
{
    ...
    if (!target) {
        ...
        if (m_renderer->m_game.title == CRC::HauntingGround) {
            uint32 end_block =
                GSLocalMemory::m_psm[psm].bn(rect.z - 1, rect.w - 1, bp, bw);    // Valid only for color formats
    ...
    for (int type = 0; type < 2; type++) {
        ...
        for (auto i = list.begin(); i != list.end();) {
            ...
            if (GSUtil::HasSharedBits(bp, psm, t->m_TEX0.TBP0, t->m_TEX0.PSM)) {
                if (!found && GSUtil::HasCompatibleBits(psm, t->m_TEX0.PSM)) {
                    GL_CACHE("TC: Dirty Target(%s) %d (0x%x) r(%d,%d,%d,%d)", to_string(type),
                             t->m_texture ? t->m_texture->GetID() : 0, t->m_TEX0.TBP0, r.x, r.y, r.z, r.w);
            ...
            if (GSUtil::HasSharedBits(psm, t->m_TEX0.PSM)) {
                if (bp < t->m_TEX0.TBP0) {
                    ...
                    if (rowsize > 0 && offset % rowsize == 0) {
                        ...
                        if (r.bottom > y) {
                            ...
                            t->m_dirty.push_back(
                                GSDirtyRect(GSVector4i(r.left, r.top - y, r.right, r.bottom - y), psm));
                            t->m_TEX0.TBW = bw;
                            continue;
                        }
                    }
                ...
                //
                // Ben 10 Alien Force : Vilgax Attacks uses a small temporary target for multiple textures (different
                // bw) It is too complex to handle, and purpose of the code was to handle FMV (large bw). So let's skip
                // small (128 pixels) target
                if (bw > 2 && t->m_TEX0.TBW == bw && t->Inside(bp, bw, psm, rect) &&
                    GSUtil::HasCompatibleBits(psm, t->m_TEX0.PSM)) {
                    ...
                    if (rowsize > 0 && offset % rowsize == 0) {
                        ...
                        GL_CACHE("TC: Dirty in the middle of Target(%s) %d (0x%x->0x%x) pos(%d,%d => %d,%d) bw:%u",
                                 to_string(type), t->m_texture ? t->m_texture->GetID() : 0, t->m_TEX0.TBP0,
                                 t->m_end_block, r.left, r.top + y, r.right, r.bottom + y, bw);
...
void GSTextureCache::InvalidateLocalMem(GSOffset *off, const GSVector4i &r)
{
    ...
    for (auto t : m_dst[RenderTarget]) {
        if (t->m_TEX0.PSM != PSM_PSMZ32 && t->m_TEX0.PSM != PSM_PSMZ24 && t->m_TEX0.PSM != PSM_PSMZ16 &&
            t->m_TEX0.PSM != PSM_PSMZ16S) {
            if (GSUtil::HasSharedBits(bp, psm, t->m_TEX0.TBP0, t->m_TEX0.PSM)) {
    ...
    //	if (t->m_TEX0.PSM != PSM_PSMZ32 && t->m_TEX0.PSM != PSM_PSMZ24 && t->m_TEX0.PSM != PSM_PSMZ16 && t->m_TEX0.PSM
    //!= PSM_PSMZ16S)
    //	{
    //		if (GSUtil::HasSharedBits(bp, psm, t->m_TEX0.TBP0, t->m_TEX0.PSM))
    //		{
    ...
    //			{
    //				// ffx-2 riku changing to her default (shoots some reflecting glass at the end), 16-bit rt read as
    //32-bit 				Read(t, GSVector4i(r.left, r.top, r.right, r.top + (r.bottom - r.top) * 2).rintersect(t->m_valid)); 				return;
    //			}
    //			else
    //			{
    //				if (psm == PSM_PSMT4HH && t->m_TEX0.PSM == PSM_PSMCT32)
    //				{
    //					// Silent Hill Origins shadows: Read 8 bit using only the HIGH bits (4 bit) texture as 32
    //bit. 					Read(t, r.rintersect(t->m_valid)); 					return;
    //				}
    //				else
    //				{
    //					//printf("Trashing render target. We have a %d type texture and we are trying to write into a %d type
    //texture\n", t->m_TEX0.PSM, psm); 					m_dst[RenderTarget].erase(j); 					delete t;
    //				}
    //			}
    //		}

    //		// Grandia3, FFX, FFX-2 pause menus. t->m_TEX0.TBP0 magic number checks because otherwise kills xs2
    //videos 		if ((GSUtil::HasSharedBits(psm, t->m_TEX0.PSM) && (bp > t->m_TEX0.TBP0))
...
void GSTextureCache::InvalidateVideoMemSubTarget(GSTextureCache::Target *rt)
{
    ...
    for (auto i = list.begin(); i != list.end();) {
        ...
        if ((t->m_TEX0.TBP0 > rt->m_TEX0.TBP0) && (t->m_end_block < rt->m_end_block) &&
            (t->m_TEX0.TBW == rt->m_TEX0.TBW) && (t->m_TEX0.TBP0 < t->m_end_block)) {
            GL_INS("InvalidateVideoMemSubTarget: rt 0x%x -> 0x%x, sub rt 0x%x -> 0x%x", rt->m_TEX0.TBP0,
                   rt->m_end_block, t->m_TEX0.TBP0, t->m_end_block);
...
void GSTextureCache::IncAge()
{
    ...
    maxage = 400;    // ffx intro scene changes leave the old image untouched for a couple of frames and only then start
                     // using it
...
GSTextureCache::Source *GSTextureCache::CreateSource(const GIFRegTEX0 &TEX0, const GIFRegTEXA &TEXA, Target *dst,
                                                     bool half_right, int x_offset, int y_offset)
{
    ...
    if (dst && (x_offset != 0 || y_offset != 0)) {
        ...
        // do not round here!!! if edge becomes a black pixel and addressing mode is clamp => everything outside the
        // clamped area turns into black (kh2 shadows)
        ...
        if (dst->m_TEX0.TBW != TEX0.TBW)    // && dst->m_TEX0.PSM == TEX0.PSM
        {
            ...
            // ASSERT(dst->m_TEX0.TBW > TEX0.TBW); // otherwise scale.x need to be reduced to make the larger texture
            // fit (TODO)
        ...
        if ((sRect == dRect).alltrue() && !shader) {
            if (half_right) {
                ...
                m_renderer->m_dev->CopyRect(
                    sTex, dTex, GSVector4i(0, 0, w, h));    // <= likely wrong dstsize.x could be bigger than w
            }
        } else {
...
GSTextureCache::Surface::Surface(GSRenderer *r, uint8 *temp)
    : m_renderer(r), m_texture(NULL), m_age(0), m_temp(temp), m_32_bits_fmt(false), m_shared_texture(false),
      m_end_block(0)
{
    m_TEX0.TBP0 = 0x3fff;
}
...
GSTextureCache::Source::Source(GSRenderer *r, const GIFRegTEX0 &TEX0, const GIFRegTEXA &TEXA, uint8 *temp,
                               bool dummy_container)
    : Surface(r, temp), m_palette_obj(nullptr), m_palette(nullptr), m_valid_rect(0, 0), m_target(false),
      m_complete(false), m_p2t(NULL), m_from_target(NULL), m_from_target_TEX0(TEX0)
{
    m_TEX0 = TEX0;
    m_TEXA = TEXA;
...
GSTextureCache::Target::Target(GSRenderer *r, const GIFRegTEX0 &TEX0, uint8 *temp, bool depth_supported)
    : Surface(r, temp), m_type(-1), m_used(false), m_depth_supported(depth_supported)
{
    m_TEX0 = TEX0;
...
void GSTextureCache::Target::Update()
{
    ...
    if (m_type == RenderTarget) {
        ...
        // FIXME linear or not?
        m_renderer->m_dev->StretchRect(t, m_texture, GSVector4(r) * GSVector4(m_texture->GetScale()).xyxy(),
                                       ShaderConvert_RGBA8_TO_FLOAT32);
    }
...
void GSTextureCache::Target::UpdateValidity(const GSVector4i &rect)
{
    ...
    m_end_block = GSLocalMemory::m_psm[m_TEX0.PSM].bn(m_valid.z - 1, m_valid.w - 1, m_TEX0.TBP0,
                                                      m_TEX0.TBW);    // Valid only for color formats

    // GL_CACHE("UpdateValidity (0x%x->0x%x) from R:%d,%d Valid: %d,%d", m_TEX0.TBP0, m_end_block, rect.z, rect.w,
    // m_valid.z, m_valid.w);
}
...
void GSTextureCache::SourceMap::RemoveAt(Source *s)
{
    ...
    GL_CACHE("TC: Remove Src Texture: %d (0x%x)", s->m_texture ? s->m_texture->GetID() : 0, s->m_TEX0.TBP0);
...
GSTextureCache::Palette::Palette(const GSRenderer *renderer, uint16 pal, bool need_gs_texture)
    : m_pal(pal), m_tex_palette(nullptr), m_renderer(renderer)
{
...
{
    if (!m_tex_palette) {
        // A palette texture is always created with dimensions 256x1 (also in the case that m_pal is 16, thus a 16x1
        // texture would be enough to store the CLUT data) because the coordinates that the shader uses for sampling
        // such texture are always normalized by 255. This is because indexes are stored as normalized values of an RGBA
        // texture (e.g. index 15 will be read as (15/255), and therefore will read texel 15/255 * texture size).
...
// 1) The clut can be an array of 16 or 256 uint32 (depending on the pal parameter) and in order to speed up the
// computation of the hash
//    the array is hashed in blocks of 16 uint32, so for clut of size 16 uint32 the hashing is computed in one pass and
//    for clut of 256 uint32 it is computed in 16 passes,
// 2) The clut can contain many 0s, so as a way to increase the spread of hashing values for small changes in the input
// clut the hashing function
//    is using addition in combination with logical XOR operator; The addition constants are large prime numbers, which
//    may help in achieving what intended.
...
std::shared_ptr<GSTextureCache::Palette> GSTextureCache::PaletteMap::LookupPalette(uint16 pal, bool need_gs_texture)
{
    ...
    if (map.size() > MAX_SIZE) {
        ...
        GL_INS("WARNING, %u-bit PaletteMap (Size %u): Max size %u exceeded, clearing unused palettes.",
               pal * sizeof(uint32), map.size(), MAX_SIZE);

        uint32 current_size = map.size();

        for (auto it = map.begin(); it != map.end();) {
            // If the palette is unused, there is only one shared pointers holding a reference to the unused Palette
            // object, and this shared pointer is the one stored in the map itself
            if (it->second.use_count() <= 1) {
                // Palette is unused
                it = map.erase(it);    // Erase element from map
                                       // The palette object should now be gone as the shared pointer to the object in
                                       // the map is deleted
            } else {
                ++it;
            }
        ...
        if (cleared_palette_count == 0) {
            GL_INS("ERROR, %u-bit PaletteMap (Size %u): Max size %u exceeded, could not clear any palette, negative "
                   "performance impact.",
                   pal * sizeof(uint32), map.size(), MAX_SIZE);
        } else {
            map.reserve(MAX_SIZE);    // Ensure map capacity is not modified by the clearing
            GL_INS("INFO, %u-bit PaletteMap (Size %u): Cleared %u palettes.", pal * sizeof(uint32), map.size(),
                   cleared_palette_count);
        }
    }
...
{
    for (auto &map : m_maps) {
        map.clear();    // Clear all the nodes of the map, deleting Palette objects managed by shared pointers as they
                        // should be unused elsewhere
        map.reserve(MAX_SIZE);    // Ensure map capacity is not modified by the clearing
    }
}
