...
#define ASSERT_BLOCK(r, w, h)                                                                                          \
    ASSERT((r).width() >= (w) && (r).height() >= (h) && !((r).left & ((w)-1)) && !((r).top & ((h)-1)) &&               \
           !((r).right & ((w)-1)) && !((r).bottom & ((h)-1)));
...
GSLocalMemory::GSLocalMemory() : m_clut(this)
{
    ...
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSGPU24].rowOffset[i] = rowOffset16;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMCT16].rowOffset[i] = rowOffset16;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMCT16S].rowOffset[i] = rowOffset16S;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMT8].rowOffset[i] = rowOffset8[((i + 2) >> 2) & 1];
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMT4].rowOffset[i] = rowOffset4[((i + 2) >> 2) & 1];
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMZ32].rowOffset[i] = rowOffset32Z;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMZ24].rowOffset[i] = rowOffset32Z;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMZ16].rowOffset[i] = rowOffset16Z;
    for (int i = 0; i < 8; i++)
        m_psm[PSM_PSMZ16S].rowOffset[i] = rowOffset16SZ;
...
static bool cmp_vec2x(const GSVector2i &a, const GSVector2i &b)
{
    return a.x < b.x;
}
...
std::vector<GSVector2i> *GSLocalMemory::GetPage2TileMap(const GIFRegTEX0 &TEX0)
{
    ...
    std::unordered_map<uint32, std::unordered_set<uint32>>
        tmp;    // key = page, value = y:x, 7 bits each, max 128x128 tiles for the worst case (1024x1024 32bpp 8x8
                // blocks)
    ...
    // combine the lower 5 bits of the address into a 9:5 pointer:mask form, so the "valid bits" can be tested against
    // an uint32 array
...
void GSLocalMemory::WriteImageColumn(int l, int r, int y, int h, const uint8 *src, int srcpitch,
                                     const GIFRegBITBLTBUF &BITBLTBUF)
{
...
template <int psm, int bsx, int bsy, int alignment>
void GSLocalMemory::WriteImageBlock(int l, int r, int y, int h, const uint8 *src, int srcpitch,
                                    const GIFRegBITBLTBUF &BITBLTBUF)
{
    uint32 bp = BITBLTBUF.DBP;
    uint32 bw = BITBLTBUF.DBW;
...
void GSLocalMemory::WriteImageLeftRight(int l, int r, int y, int h, const uint8 *src, int srcpitch,
                                        const GIFRegBITBLTBUF &BITBLTBUF)
{
    uint32 bp = BITBLTBUF.DBP;
    uint32 bw = BITBLTBUF.DBW;
    ...
    for (; h > 0; y++, h--, src += srcpitch) {
        for (int x = l; x < r; x++) {
            switch (psm) {
                case PSM_PSMCT32:
                    WritePixel32(x, y, *(uint32 *)&src[x * 4], bp, bw);
                    break;
                case PSM_PSMCT16:
                    WritePixel16(x, y, *(uint16 *)&src[x * 2], bp, bw);
                    break;
                case PSM_PSMCT16S:
                    WritePixel16S(x, y, *(uint16 *)&src[x * 2], bp, bw);
                    break;
                case PSM_PSMT8:
                ...
                case PSM_PSMT4:
                    ...
                    break;
                case PSM_PSMZ32:
                    WritePixel32Z(x, y, *(uint32 *)&src[x * 4], bp, bw);
                    break;
                case PSM_PSMZ16:
                    WritePixel16Z(x, y, *(uint16 *)&src[x * 2], bp, bw);
                    break;
                case PSM_PSMZ16S:
                    WritePixel16SZ(x, y, *(uint16 *)&src[x * 2], bp, bw);
                    break;
                // TODO
                default:
                    __assume(0);
...
void GSLocalMemory::WriteImageTopBottom(int l, int r, int y, int h, const uint8 *src, int srcpitch,
                                        const GIFRegBITBLTBUF &BITBLTBUF)
{
...
template <int psm, int bsx, int bsy, int trbpp>
void GSLocalMemory::WriteImage(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                               GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImage24(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                 GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImage8H(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                 GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImage4HL(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                  GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImage4HH(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                  GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImage24Z(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                  GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (TRXREG.RRW == 0)
        return;
...
void GSLocalMemory::WriteImageX(int &tx, int &ty, const uint8 *src, int len, GIFRegBITBLTBUF &BITBLTBUF,
                                GIFRegTRXPOS &TRXPOS, GIFRegTRXREG &TRXREG)
{
    if (len <= 0)
        return;
...
void GSLocalMemory::ReadImageX(int &tx, int &ty, uint8 *dst, int len, GIFRegBITBLTBUF &BITBLTBUF, GIFRegTRXPOS &TRXPOS,
                               GIFRegTRXREG &TRXREG) const
{
    if (len <= 0)
        return;
...
void GSLocalMemory::ReadTexture32(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
...
void GSLocalMemory::ReadTexture24(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                  const GIFRegTEXA &TEXA)
{
    if (TEXA.AEM) {
        FOREACH_BLOCK_START(r, 8, 8, 32)
...
void GSLocalMemory::ReadTextureGPU24(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                     const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture16(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                  const GIFRegTEXA &TEXA)
{
    if (TEXA.AEM) {
...
void GSLocalMemory::ReadTexture8(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                 const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture4(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                 const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture8H(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                  const GIFRegTEXA &TEXA)
{
    const uint32 *pal = m_clut;
...
void GSLocalMemory::ReadTexture4HL(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                   const GIFRegTEXA &TEXA)
{
    const uint32 *pal = m_clut;
...
void GSLocalMemory::ReadTexture4HH(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                   const GIFRegTEXA &TEXA)
{
    const uint32 *pal = m_clut;
...
void GSLocalMemory::ReadTexture(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                const GIFRegTEXA &TEXA)
{
    ...
    if (r.width() < psm.bs.x || r.height() < psm.bs.y || (r.left & (psm.bs.x - 1)) || (r.top & (psm.bs.y - 1)) ||
...
void GSLocalMemory::ReadTexture8P(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                  const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture4P(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                  const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture8HP(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                   const GIFRegTEXA &TEXA)
{
...
void GSLocalMemory::ReadTexture4HLP(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                    const GIFRegTEXA &TEXA)
{
    FOREACH_BLOCK_START(r, 8, 8, 8)
    {
...
void GSLocalMemory::ReadTexture4HHP(const GSOffset *RESTRICT off, const GSVector4i &r, uint8 *dst, int dstpitch,
                                    const GIFRegTEXA &TEXA)
{
    FOREACH_BLOCK_START(r, 8, 8, 8)
    {
