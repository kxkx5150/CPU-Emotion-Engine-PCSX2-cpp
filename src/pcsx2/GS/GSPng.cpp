...
bool SaveFile(const std::string &file, const Format fmt, const uint8 *const image, uint8 *const row, const int width,
              const int height, const int pitch, const int compression, const bool rb_swapped = false,
              const bool first_image = false)
{
    ...
    try {
        ...
        png_set_IHDR(png_ptr, info_ptr, width, height, channel_bit_depth, type, PNG_INTERLACE_NONE,
...
bool Save(GSPng::Format fmt, const std::string &file, uint8 *image, int w, int h, int pitch, int compression,
          bool rb_swapped)
{
...
Transaction::Transaction(GSPng::Format fmt, const std::string &file, const uint8 *image, int w, int h, int pitch,
                         int compression)
