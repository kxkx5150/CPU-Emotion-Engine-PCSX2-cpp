...
int GSState::s_n = 0;
GSState::GSState()
    : m_version(7), m_gsc(NULL), m_skip(0), m_skip_offset(0), m_q(1.0f), m_vt(this), m_regs(NULL), m_crc(0),
      m_options(0), m_frameskip(0)
{
    // m_nativeres seems to be a hack. Unfortunately it impacts draw call number which make debug painful in the
    // replayer. Let's keep it disabled to ease debug.
...
void GSState::Reset()
{
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        m_env.CTXT[i].offset.fb =
            m_mem.GetOffset(m_env.CTXT[i].FRAME.Block(), m_env.CTXT[i].FRAME.FBW, m_env.CTXT[i].FRAME.PSM);
        m_env.CTXT[i].offset.zb =
            m_mem.GetOffset(m_env.CTXT[i].ZBUF.Block(), m_env.CTXT[i].FRAME.FBW, m_env.CTXT[i].ZBUF.PSM);
        m_env.CTXT[i].offset.tex =
            m_mem.GetOffset(m_env.CTXT[i].TEX0.TBP0, m_env.CTXT[i].TEX0.TBW, m_env.CTXT[i].TEX0.PSM);
...
void GSState::ResetHandlers()
{
    ...
    m_fpGIFPackedRegHandlers[GIF_REG_UV] =
        m_userhacks_wildhack ? &GSState::GIFPackedRegHandlerUV_Hack : &GSState::GIFPackedRegHandlerUV;
...
#define SetHandlerXYZ(P, auto_flush)                                                                                   \
    ...
    m_fpGIFRegHandlers[GIF_A_D_REG_UV] =
        m_userhacks_wildhack ? &GSState::GIFRegHandlerUV_Hack : &GSState::GIFRegHandlerUV;
    ...
    switch (Colorburst) {
        case 0:
            if (isinterlaced() && PLL_Divider == 22)
                ...
                return GSVideoMode::SDTV_480P;    // TODO: 576P will also be reported as 480P, find some way to
                                                  // differeniate.
            else
...
// unsupported value with respect to the current video mode via the DISP registers, the following function handles such
// cases. NOTE: This function is totally hacky as there are no documents related to saturation of output dimensions,
// function is generally just based on technical and intellectual guesses.
void GSState::SaturateOutputSize(GSVector4i &r)
{
    ...
    //  Limit games to standard NTSC resolutions. games with 512X512 (PAL resolution) on NTSC video mode produces black
    //  border on the bottom. 512 X 448 is the resolution generally used by NTSC, saturating the height value seems to
    //  get rid of the black borders. Though it's quite a bad hack as it affects binaries which are patched to run on a
    //  non-native video mode.
    ...
    const bool saturate =
        m_NTSC_Saturation && is_ntsc && (interlaced_field || single_frame_output) && unsupported_output_size;
    if (saturate)
        r.bottom = r.top + 448;
}
    ...
    if (i == -1) {
        if (IsEnabled(0) && IsEnabled(1)) {
            ...
            const bool can_be_merged =
                intersect.width() == 0 || intersect.height() == 0 || intersect.xyxy().eq(combined.xyxy());
            if (can_be_merged)
                return combined;
...
{
    if (GSUtil::GetPrimClass(m_env.PRIM.PRIM) ==
        GSUtil::GetPrimClass(prim & 7))    // NOTE: assume strips/fans are converted to lists
    {
...
void GSState::GIFRegHandlerRGBAQ(const GIFReg *RESTRICT r)
{
    ...
    GSVector4i q     = rgbaq.blend8(GSVector4i::cast(GSVector4::m_one), rgbaq == GSVector4i::zero())
                       .yyyy();    // see GIFPackedRegHandlerSTQ
...
template <uint32 prim, uint32 adc, bool auto_flush> void GSState::GIFRegHandlerXYZF2(const GIFReg *RESTRICT r)
{
...
template <uint32 prim, uint32 adc, bool auto_flush> void GSState::GIFRegHandlerXYZ2(const GIFReg *RESTRICT r)
{
...
template <int i> void GSState::ApplyTEX0(GIFRegTEX0 &TEX0)
{
    ...
    // even if TEX0 did not change, a new palette may have been uploaded and will overwrite the currently queued for
    // drawing
    const bool wt = m_mem.m_clut.WriteTest(TEX0, m_env.TEXCLUT);
    // clut loading already covered with WriteTest, for drawing only have to check CPSM and CSA (MGS3 intro skybox would
    // be drawn piece by piece without this)
...
template <int i> void GSState::GIFRegHandlerTEX0(const GIFReg *RESTRICT r)
{
    ...
    if (m_env.CTXT[i].TEX1.MTBA) {
        ...
        // NOTE 2: Mipmap levels are tightly packed, if (tbw << 6) > (1 << tw) then the left-over space to the right is
        // used. (common for PSM_PSMT4) NOTE 3: Non-rectangular textures are treated as rectangular when calculating the
        // occupied space (height is extended, not sure about width)
        uint32       bp  = TEX0.TBP0;
        uint32       bw  = TEX0.TBW;
        uint32       w   = 1u << TEX0.TW;
        uint32       h   = 1u << TEX0.TH;
...
void GSState::FlushPrim()
{
    if (m_index.tail > 0) {
        ...
        // If the PSM format of Z is invalid, but it is masked (no write) and ZTST is set to ALWAYS pass (no test, just
        // allow) we can ignore the Z format, since it won't be used in the draw (Star Ocean 3 transitions)
        const bool ignoreZ = m_context->ZBUF.ZMSK && m_context->TEST.ZTST == 1;
        if (GSLocalMemory::m_psm[m_context->FRAME.PSM].fmt >= 3 ||
            (GSLocalMemory::m_psm[m_context->ZBUF.PSM].fmt >= 3 && !ignoreZ)) {
            Console.Warning("GS: Possible invalid draw, Frame PSM %x ZPSM %x", m_context->FRAME.PSM,
                            m_context->ZBUF.PSM);
        }
...
void GSState::Write(const uint8 *mem, int len)
{
    ...
    if (!m_tr.Update(w, h, psm.trbpp, len))
        return;
    GL_CACHE("Write! ...  => 0x%x W:%d F:%s (DIR %d%d), dPos(%d %d) size(%d %d)", blit.DBP, blit.DBW,
             psm_str(blit.DPSM), m_env.TRXPOS.DIRX, m_env.TRXPOS.DIRY, m_env.TRXPOS.DSAX, m_env.TRXPOS.DSAY, w, h);
...
void GSState::Read(uint8 *mem, int len)
{
    ...
    //     std::string s =
    //         m_dump_root + format("%05d_read_%05x_%d_%d_%d_%d_%d_%d.bmp", s_n, (int)m_env.BITBLTBUF.SBP,
    //                              (int)m_env.BITBLTBUF.SBW, (int)m_env.BITBLTBUF.SPSM, r.left, r.top, r.right,
    //                              r.bottom);
...
void GSState::Move()
{
    // ffxii uses this to move the top/bottom of the scrolling menus offscreen and then blends them back over the text
    // to create a shading effect guitar hero copies the far end of the board to do a similar blend too
    ...
    GL_CACHE("Move! 0x%x W:%d F:%s => 0x%x W:%d F:%s (DIR %d%d), sPos(%d %d) dPos(%d %d) size(%d %d)",
             m_env.BITBLTBUF.SBP, m_env.BITBLTBUF.SBW, psm_str(m_env.BITBLTBUF.SPSM), m_env.BITBLTBUF.DBP,
             m_env.BITBLTBUF.DBW, psm_str(m_env.BITBLTBUF.DPSM), m_env.TRXPOS.DIRX, m_env.TRXPOS.DIRY, sx, sy, dx, dy,
             w, h);
    InvalidateLocalMem(m_env.BITBLTBUF, GSVector4i(sx, sy, sx + w, sy + h));
    ...
    // TODO: unroll inner loops (width has special size requirement, must be multiples of 1 << n, depending on the
    // format)
...
template <int index> void GSState::Transfer(const uint8 *mem, uint32 size)
{
    ...
    while (size > 0) {
        if (path.nloop == 0) {
            ...
            size--;
            // eeuser 7.2.2. GIFtag:
            // "... when NLOOP is 0, the GIF does not output anything, and values other than the EOP field are
            // disregarded."
            if (path.nloop > 0) {
                m_q = 1.0f;
                        ...
                        switch (path.type) {
                            case GIFPath::TYPE_UNKNOWN: {
                                ...
                                do {
                                    ...
                                    mem += sizeof(GIFPackedReg);
                                    reg = reg & ((int)(reg - path.nreg) >>
                                                 31);    // resets reg back to 0 when it becomes equal to path.nreg
                                } while (--total > 0);
                            } break;
                            case GIFPath::TYPE_ADONLY:    // very common
                                do {
                                    (this->*m_fpGIFRegHandlers[((GIFPackedReg *)mem)->A_D.ADDR & 0x7F])(
                                        &((GIFPackedReg *)mem)->r);
                                    mem += sizeof(GIFPackedReg);
                                } while (--total > 0);
                                break;
...
void GSState::GrowVertexBuffer()
{
    ...
    uint32      *index    = (uint32 *)_aligned_malloc(sizeof(uint32) * maxcount * 3,
                                                      32);    // worst case is slightly less than vertex number * 3
    if (vertex == NULL || index == NULL) {
        ...
        Console.Error("GS: failed to allocate %zu bytes for verticles and %zu for indices.", vert_byte_count,
                      idx_byte_count);
        throw GSError();
    }
    ...
    m_vertex.maxcount =
        maxcount - 3;    // -3 to have some space at the end of the buffer before DrawingKick can grow it
    m_index.buff = index;
}
template <uint32 prim, bool auto_flush> __forceinline void GSState::VertexKick(uint32 skip)
{
    ...
    // callers should write XYZUVF to m_v.m[1] in one piece to have this load store-forwarded, either by the cpu or the
    // compiler when this function is inlined
    ...
    if (m < n)
        return;
    if (skip == 0 && (prim != GS_TRIANGLEFAN ||
                      m <= 4))    // m_vertex.xy only knows about the last 4 vertices, head could be far behind for fan
    {
...
bool GSState::TryAlphaTest(uint32 &fm, uint32 &zm)
{
    ...
    switch (m_context->TEST.AFAIL) {
        ...
        case AFAIL_RGB_ONLY:
            if (zm == 0xFFFFFFFF &&
                ((fm & 0xFF000000) == 0xFF000000 || GSLocalMemory::m_psm[m_context->FRAME.PSM].fmt == 1))
                return true;
            break;
        default:
            __assume(0);
