...
template <typename Func> static void EnumFilters(IGraphBuilder *filterGraph, Func &&f)
{
...
template <typename Func> static void EnumPins(IBaseFilter *baseFilter, Func &&f)
{
...
// GSSource
//
interface __declspec(uuid("59C193BB-C520-41F3-BC1D-E245B80A86FA")) IGSSource : public IUnknown {
    STDMETHOD(DeliverNewSegment)() PURE;
    STDMETHOD(DeliverFrame)(const void *bits, int pitch, bool rgba) PURE;
    STDMETHOD(DeliverEOS)() PURE;
};

class __declspec(uuid("F8BB6F4F-0965-4ED4-BA74-C6A01E6E6C77")) GSSource : public CBaseFilter,
                                                                          private CCritSec,
                                                                          public IGSSource {
    GSVector2i     m_size;
    ...
    {
        return riid == __uuidof(IGSSource) ? GetInterface((IGSSource *)this, ppv)
    ...
    class GSSourceOutputPin : public CBaseOutputPin {
      ...
      public:
        GSSourceOutputPin(const GSVector2i &size, REFERENCE_TIME atpf, CBaseFilter *pFilter, CCritSec *pLock,
                          HRESULT &hr, int colorspace)
            : CBaseOutputPin("GSSourceOutputPin", pFilter, pLock, &hr, L"Output"), m_size(size)
        {
            CMediaType mt;
  ...
  public:
    GSSource(int w, int h, float fps, IUnknown *pUnk, HRESULT &hr, int colorspace)
        : CBaseFilter("GSSource", pUnk, this, __uuidof(this), &hr), m_output(NULL), m_size(w, h),
          m_atpf((REFERENCE_TIME)(10000000.0f / fps)), m_now(0)
    {
...
GSCapture::GSCapture() : m_capturing(false), m_frame(0), m_out_dir("/tmp/GS_Capture")    // FIXME Later add an option
{
...
bool GSCapture::BeginCapture(float fps, GSVector2i recommendedResolution, float aspect, std::string &filename)
{
    ...
    if (FAILED(cgb->SetFiltergraph(graph.get())) ||
        FAILED(cgb->SetOutputFileName(&MEDIASUBTYPE_Avi,
                                      std::wstring(dlg.m_filename.begin(), dlg.m_filename.end()).c_str(), mux.put(),
                                      nullptr))) {
        return false;
    }
    ...
    // starts the reference count from 0. Should this ever change and GSSource ends up with a refcount of 1 after
    // constructing, change this to `.attach(new GSSource(...))`.
    ...
    if (dlg.m_enc == 0) {
        if (FAILED(graph->AddFilter(src.get(), L"Source")))
            return false;
        if (FAILED(graph->ConnectDirect(GetFirstPin(src.get(), PINDIR_OUTPUT).get(),
                                        GetFirstPin(mux.get(), PINDIR_INPUT).get(), nullptr)))
            return false;
    } else {
        ...
        if (FAILED(graph->ConnectDirect(GetFirstPin(src.get(), PINDIR_OUTPUT).get(),
                                        GetFirstPin(dlg.m_enc.get(), PINDIR_INPUT).get(), nullptr)) ||
            FAILED(graph->ConnectDirect(GetFirstPin(dlg.m_enc.get(), PINDIR_OUTPUT).get(),
                                        GetFirstPin(mux.get(), PINDIR_INPUT).get(), nullptr))) {
            return false;
        }
    }
...
bool GSCapture::DeliverFrame(const void *bits, int pitch, bool rgba)
{
    ...
    m_workers[m_frame % m_threads]->Push(std::make_shared<GSPng::Transaction>(
        GSPng::RGB_PNG, out_file, static_cast<const uint8 *>(bits), m_size.x, m_size.y, pitch, m_compression_level));
