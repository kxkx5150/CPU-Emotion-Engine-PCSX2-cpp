...
DHCP_Server::DHCP_Server(std::function<void()> receivedcallback) : callback{receivedcallback}
{
}
...
bool DHCP_Server::Send(UDP_Packet *payload)
{
    ...
    for (size_t i = 0; i < dhcp.options.size(); i++) {
        switch (dhcp.options[i]->GetCode()) {
            ...
            case 6:
                ...
                if ((((DHCPopDNS *)dhcp.options[i])->dnsServers.size() > 0 &&
                     dns1 != ((DHCPopDNS *)dhcp.options[i])->dnsServers[0]) ||
                    (((DHCPopDNS *)dhcp.options[i])->dnsServers.size() > 1 &&
                     dns2 != ((DHCPopDNS *)dhcp.options[i])->dnsServers[1]))
    ...
    retPay->op = 2, retPay->hardwareType = hType, retPay->hardwareAddressLength = hLen, retPay->transactionID = xID,
