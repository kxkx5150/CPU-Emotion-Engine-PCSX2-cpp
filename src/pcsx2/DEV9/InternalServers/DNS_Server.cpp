...
DNS_Server::DNS_Server(std::function<void()> receivedcallback) : callback{receivedcallback}
{
#ifdef _WIN32
...
void DNS_Server::GetHost(std::string url, DNS_State *state)
{
    ...
    int ret = GetAddrInfoEx(converted_string.data(), nullptr, NS_ALL, 0, &hints, (ADDRINFOEX **)&data->result, nullptr,
                            &data->overlapped, &DNS_Server::GetAddrInfoExCallback, &data->cancelHandle);
