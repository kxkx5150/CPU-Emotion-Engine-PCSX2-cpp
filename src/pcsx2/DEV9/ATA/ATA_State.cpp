...
u16 ATA::Read16(u32 addr)
{
    switch (addr) {
        case ATA_R_DATA:
            return ATAreadPIO();
        case ATA_R_ERROR:
            // DevCon.WriteLn("DEV9: *ATA_R_ERROR 16bit read at address %x, value %x, Active %s", addr, regError,
            // (GetSelectedDevice() == 0) ? "True" : "False");
            if (GetSelectedDevice() != 0)
                return 0;
            return regError;
        case ATA_R_NSECTOR:
            // DevCon.WriteLn("DEV9: *ATA_R_NSECTOR 16bit read at address %x, value %x, Active %s", addr, nsector,
            // (GetSelectedDevice() == 0) ? "True" : "False");
            if (GetSelectedDevice() != 0)
                return 0;
        ...
        case ATA_R_SECTOR:
            // DevCon.WriteLn("DEV9: *ATA_R_NSECTOR 16bit read at address %x, value %x, Active %s", addr, regSector,
            // (GetSelectedDevice() == 0) ? "True" : "False");
            if (GetSelectedDevice() != 0)
                return 0;
            if (!regControlHOBRead)
                ...
                return regSectorHOB;
        case ATA_R_LCYL:
            // DevCon.WriteLn("DEV9: *ATA_R_LCYL 16bit read at address %x, value %x, Active %s", addr, regLcyl,
            // (GetSelectedDevice() == 0) ? "True" : "False");
            if (GetSelectedDevice() != 0)
                return 0;
            if (!regControlHOBRead)
                ...
                return regLcylHOB;
        case ATA_R_HCYL:
            // DevCon.WriteLn("DEV9: *ATA_R_HCYL 16bit read at address % x, value % x, Active %s", addr, regHcyl,
            // (GetSelectedDevice() == 0) ? " True " : " False ");
            if (GetSelectedDevice() != 0)
                return 0;
            if (!regControlHOBRead)
                ...
                return regHcylHOB;
        case ATA_R_SELECT:
            // DevCon.WriteLn("DEV9: *ATA_R_SELECT 16bit read at address % x, value % x, Active %s", addr, regSelect,
            // (GetSelectedDevice() == 0) ? " True " : " False ");
            return regSelect;
        case ATA_R_STATUS:
        ...
        case ATA_R_ALT_STATUS:
            // DevCon.WriteLn("DEV9: *ATA_R_ALT_STATUS 16bit read at address % x, value % x, Active %s", addr,
            // regStatus, (GetSelectedDevice() == 0) ? " True " : " False "); raise IRQ?
            if (GetSelectedDevice() != 0)
                return 0;
            return regStatus;
        default:
...
void ATA::Async(uint cycles)
{
    ...
    if ((regStatus & (ATA_STAT_BUSY | ATA_STAT_DRQ)) == 0 || awaitFlush || (waitingCmd != nullptr)) {
        {
            std::lock_guard ioSignallock(ioMutex);
            if (ioRead || ioWrite)
                // IO Running
...
s64 ATA::HDD_GetLBA()
{
    if ((regSelect & 0x40) != 0) {
        if (!lba48) {
            return (regSector | (regLcyl << 8) | (regHcyl << 16) | ((regSelect & 0x0f) << 24));
        } else {
            return ((s64)regHcylHOB << 40) | ((s64)regLcylHOB << 32) | ((s64)regSectorHOB << 24) |
                   ((s64)regHcyl << 16) | ((s64)regLcyl << 8) | regSector;
        }
    } else {
