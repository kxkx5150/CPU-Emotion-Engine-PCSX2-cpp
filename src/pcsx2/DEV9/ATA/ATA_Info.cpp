...
void ATA::CreateHDDinfo(int sizeMb)
{
    ...
    WriteUInt16(
        identifyData, &index,
        (u16)((1 << 14) | (1 << 13) | (1 << 12) /*| (1 << 8)*/ | ((lba48Supported ? 1 : 0) << 10)));    // word 83
    ...
    WriteUInt16(identifyData, &index,
                (u16)((fetSmartEnabled << 0) | (fetSecurityEnabled << 1) | (fetWriteCacheEnabled << 5) |
                      (fetHostProtectedAreaEnabled << 10) | (1 << 14)));    // Fixed      //word 85
    ...
    // 92
    // Hardware reset result. The contents of bits (12:0) of this word shall change only during the execution of a
    // hardware reset.
    /*
     * bit 0: 1
