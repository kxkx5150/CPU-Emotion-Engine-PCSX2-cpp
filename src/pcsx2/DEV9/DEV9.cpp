...
#ifdef _WIN32
    hEeprom =
        CreateFile(L"eeprom.dat", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_WRITE_THROUGH, NULL);
...
void DEV9write16(u32 addr, u16 value)
{
    ...
    switch (addr) {
        case SPD_R_INTR_MASK:
            // DevCon.WriteLn("DEV9: SPD_R_INTR_MASK 16bit write %x	, checking for masked/unmasked interrupts",
            // value);
...
void DEV9readDMA8Mem(u32 *pMem, int size)
{
    ...
    if (dev9.dma_ctrl & SPD_DMA_TO_SMAP)
        ...
        if (dev9.xfr_ctrl & SPD_XFR_DMAEN && !(dev9.xfr_ctrl & SPD_XFR_WRITE)) {
            HDDWriteFIFO();
            IOPReadFIFO(size);
