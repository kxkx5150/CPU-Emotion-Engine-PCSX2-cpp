...
u32 FLASHread32(u32 addr, int size)
{
    ...
    switch (addr) {
        case FLASH_R_DATA:
            ...
            counter += size;
            DevCon.WriteLn("DEV9: *FLASH DATA %dbit read 0x%08lX %s", size * 8, value,
                           (ctrl & FLASH_PP_READ) ? "READ_ENABLE" : "READ_DISABLE");
            if (cmd == SM_CMD_READ3) {
...
void FLASHwrite32(u32 addr, u32 value, int size)
{
    ...
    switch (addr & 0x1FFFFFFF) {
        case FLASH_R_DATA:
            ...
            DevCon.WriteLn("DEV9: *FLASH DATA %dbit write 0x%08lX %s", size * 8, value,
                           (ctrl & FLASH_PP_WRITE) ? "WRITE_ENABLE" : "WRITE_DISABLE");
        ...
        case FLASH_R_CMD:
            if (!(ctrl & FLASH_PP_READY)) {
                if ((value != SM_CMD_GETSTATUS) && (value != SM_CMD_RESET)) {
                    DevCon.WriteLn("DEV9: *FLASH CMD %dbit write %s ILLEGAL in busy mode - IGNORED", size * 8,
                                   getCmdName(value));
                    break;
                }
            }
            if (cmd == SM_CMD_WRITEDATA) {
                if ((value != SM_CMD_PROGRAMPAGE) && (value != SM_CMD_RESET)) {
                    DevCon.WriteLn("DEV9: *FLASH CMD %dbit write %s ILLEGAL after WRITEDATA cmd - IGNORED", size * 8,
                                   getCmdName(value));
        ...
        case FLASH_R_ADDR:
            ...
            if (!(value & 0x100)) {    // address is complete
                ...
                addrbyte = 0;    // address reset
                {
                    ...
                    DevCon.WriteLn("DEV9: *FLASH ADDR = 0x%08lX (%d:%d:%d) (addrbyte=%d) FINAL", address, blocks, pages,
                                   bytes, addrbyte);
                }
            }
            break;
        ...
        default:
            DevCon.WriteLn("DEV9: *FLASH Unkwnown %dbit write at address 0x%08lX= 0x%08lX IGNORED", size * 8, addr,
                           value);
            break;
    }
}
...
static unsigned char xor_table[256] = {
    ,
