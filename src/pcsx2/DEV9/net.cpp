...
void ReconfigureLiveNet(ConfigDEV9 *oldConfig)
{
    ...
    if (config.ethEnable) {
        if (oldConfig->ethEnable) {
            ...
            if (strcmp(oldConfig->Eth, config.Eth) != 0 || oldConfig->EthApi != config.EthApi) {
                TermNet();
                InitNet();
                return;
            } else
...
const wxChar *NetApiToWxString(NetApi api)
{
    switch (api) {
        case NetApi::PCAP_Bridged:
            return wxString("PCAP Bridged");
        case NetApi::PCAP_Switched:
            return wxString("PCAP Switched");
        case NetApi::TAP:
            return wxString("TAP");
        default:
            return wxString("UNK");
        ...
        if (frame.protocol == (u16)EtherType::IPv4) {
            ...
            if (ippkt.protocol == (u16)IP_Type::UDP) {
                ...
                if (udppkt.destinationPort == 53) {
                    Console.WriteLn("DEV9: DNS: Packet Sent To %i.%i.%i.%i", ippkt.destinationIP.bytes[0],
                                    ippkt.destinationIP.bytes[1], ippkt.destinationIP.bytes[2],
                                    ippkt.destinationIP.bytes[3]);
        ...
        if (frame.protocol == (u16)EtherType::IPv4) {
            ...
            if (ippkt.protocol == (u16)IP_Type::UDP) {
                ...
                if (udppkt.sourcePort == 53) {
                    Console.WriteLn("DEV9: DNS: Packet Sent From %i.%i.%i.%i", ippkt.sourceIP.bytes[0],
                                    ippkt.sourceIP.bytes[1], ippkt.sourceIP.bytes[2], ippkt.sourceIP.bytes[3]);
