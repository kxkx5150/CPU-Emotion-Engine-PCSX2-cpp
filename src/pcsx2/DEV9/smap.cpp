...
// this can return a false positive, but its not problem since it may say it cant recv while it can (no harm done, just
// delay on packets)
bool rx_fifo_can_rx()
{
    ...
    int space  = sizeof(dev9.rxfifo) - ((dev9.rxfifo_wr_ptr - rd_ptr) & 16383);
    ...
    if (addr >= SMAP_BD_TX_BASE && addr < (SMAP_BD_TX_BASE + SMAP_BD_SIZE)) {
        ...
        */
        ...
        case 0: // ctrl_stat
                ...
                if(value&0x8000)
                {
                        DevCon.WriteLn("DEV9:  * * PACKET READ COMPLETE:   rd_ptr=%d, wr_ptr=%d",
...
void smap_readDMA8Mem(u32 *pMem, int size)
{
    if (dev9Ru16(SMAP_R_RXFIFO_CTRL) & SMAP_RXFIFO_DMAEN) {
        ...
        DevCon.WriteLn("DEV9:  * * SMAP DMA READ START: rd_ptr=%d, wr_ptr=%d", dev9Ru32(SMAP_R_RXFIFO_RD_PTR),
                       dev9.rxfifo_wr_ptr);
        while (size > 0) {
        ...
        DevCon.WriteLn("DEV9:  * * SMAP DMA READ END:   rd_ptr=%d, wr_ptr=%d", dev9Ru32(SMAP_R_RXFIFO_RD_PTR),
                       dev9.rxfifo_wr_ptr);
...
void smap_writeDMA8Mem(u32 *pMem, int size)
{
    if (dev9Ru16(SMAP_R_TXFIFO_CTRL) & SMAP_TXFIFO_DMAEN) {
        ...
        DevCon.WriteLn("DEV9:  * * SMAP DMA WRITE START: wr_ptr=%d, rd_ptr=%d", dev9Ru32(SMAP_R_TXFIFO_WR_PTR),
                       dev9.txfifo_rd_ptr);
        while (size > 0) {
            int value = *pMem;
        ...
        DevCon.WriteLn("DEV9:  * * SMAP DMA WRITE END:   wr_ptr=%d, rd_ptr=%d", dev9Ru32(SMAP_R_TXFIFO_WR_PTR),
                       dev9.txfifo_rd_ptr);
