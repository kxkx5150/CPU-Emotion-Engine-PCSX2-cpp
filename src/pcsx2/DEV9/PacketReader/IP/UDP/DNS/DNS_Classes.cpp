...
DNS_QuestionEntry::DNS_QuestionEntry(const std::string &qName, u16 qType, u16 qClass)
    : name{qName}, entryType{qType}, entryClass{qClass}
{
}
...
DNS_ResponseEntry::DNS_ResponseEntry(const std::string &rName, u16 rType, u16 rClass, const std::vector<u8> &rData,
                                     u32 rTTL)
    : DNS_QuestionEntry(rName, rType, rClass), timeToLive{rTTL}, data{rData}
{
}

DNS_ResponseEntry::DNS_ResponseEntry(u8 *buffer, int *offset) : DNS_QuestionEntry(buffer, offset)
{
    u16 dataLen;
