...
int IPU1dma()
{
    ...
    // IPU_LOG("IPU1 DMA Called QWC %x Finished %d In Progress %d tadr %x", ipu1ch.qwc, IPU1Status.DMAFinished,
    //         IPU1Status.InProgress, ipu1ch.tadr);

    switch (IPU1Status.DMAMode) {
        case DMA_MODE_NORMAL: {
            // IPU_LOG("Processing Normal QWC left %x Finished %d In Progress %d", ipu1ch.qwc, IPU1Status.DMAFinished,
            //         IPU1Status.InProgress);
        ...
        case DMA_MODE_CHAIN: {
            ...
            {
                // IPU_LOG("Processing Chain QWC left %x Finished %d In Progress %d", ipu1ch.qwc,
            ...
            if (!IPU1Status.InProgress &&
                !IPU1Status.DMAFinished)    // No transfer is ready to go so we need to set one up
            {
                ...
                // IPU_LOG("dmaIPU1 dmaChain %8.8x_%8.8x size=%d, addr=%lx, fifosize=%x", ptag[1]._u32, ptag[0]._u32,
                //         ipu1ch.qwc, ipu1ch.madr, 8 - g_BP.IFC);

                if (ipu1ch.chcr.TIE && ptag->IRQ)    // Tag Interrupt is set, so schedule the end/interrupt
                    IPU1Status.DMAFinished = true;

                // IPU_LOG("Processing Start Chain QWC left %x Finished %d In Progress %d", ipu1ch.qwc,
                //         IPU1Status.DMAFinished, IPU1Status.InProgress);
                totalqwc += IPU1chain();
    ...
    // IPU_LOG("Completed Call IPU1 DMA QWC Remaining %x Finished %d In Progress %d tadr %x", ipu1ch.qwc,
    //         IPU1Status.DMAFinished, IPU1Status.InProgress, ipu1ch.tadr);
    return totalqwc;
}
...
void IPU0dma()
{
    ...
    // IPU_LOG("dmaIPU0 chcr = %lx, madr = %lx, qwc  = %lx", ipu0ch.chcr._u32, ipu0ch.madr, ipu0ch.qwc);
...
{
    // IPU_LOG("IPU1DMAStart QWC %x, MADR %x, CHCR %x, TADR %x", ipu1ch.qwc, ipu1ch.madr, ipu1ch.chcr._u32,
    // ipu1ch.tadr);
    ...
    if (ipu1ch.chcr.MOD == CHAIN_MODE)    // Chain Mode
    {
        ...
        if (ipu1ch.qwc == 0) {
            ...
            IPU1Status.InProgress = true;
            if ((ipu1ch.chcr.tag().ID == TAG_REFE) || (ipu1ch.chcr.tag().ID == TAG_END) ||
