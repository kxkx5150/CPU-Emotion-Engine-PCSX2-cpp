...
__fi RETURNS_R64 ipuRead64(u32 mem)
{
    ...
    switch (mem) {
        ...
        ipucase(IPU_CTRL) : DevCon.Warning("reading 64bit IPU ctrl");
        break;

        ipucase(IPU_BP) : DevCon.Warning("reading 64bit IPU top");
        break;
...
void ipuSoftReset()
{
    ...
    ipuRegs.cmd.BUSY = 0;
    ipuRegs.cmd.DATA =
        0;    // required for Enthusia - Professional Racing after fix, or will freeze at start of next video.
...
__fi bool ipuWrite64(u32 mem, u64 value)
{
    ...
    switch (mem) {
        // ipucase(IPU_CMD) : IPU_LOG("write64: IPU_CMD=0x%08X", value);
...
static __fi bool ipuVDEC(u32 val)
{
    ...
    switch (ipu_cmd.pos[0]) {
            ...
            switch ((val >> 26) & 3) {
                ...
                case 1:    // Macroblock Type
                    ...
                    decoder.coding_type =
                        ipuRegs.ctrl.PCT > 0
                            ? ipuRegs.ctrl.PCT
                            : 1;    // Kaiketsu Zorro Mezase doesn't set a Picture type, seems happy with I
        ...
        case 1:
            ...
            // IPU_LOG("VDEC command data 0x%x(0x%x). Skip 0x%X bits/Table=%d (%s), pct %d", ipuRegs.cmd.DATA,
            //         ipuRegs.cmd.DATA >> 16, val & 0x3f, (val >> 26) & 3,
            //         (val >> 26) & 1 ? ((val >> 26) & 2 ? "DMV" : "MBT") : (((val >> 26) & 2 ? "MC" : "MBAI")),
            //         ipuRegs.ctrl.PCT);
...
static bool ipuSETIQ(u32 val)
{
    if ((val >> 27) & 1) {
        ...
        // for (uint i = 0; i < 8; i++) {
        //     IPU_LOG("%02X %02X %02X %02X %02X %02X %02X %02X", niq[i * 8 + 0], niq[i * 8 + 1], niq[i * 8 + 2],
        //             niq[i * 8 + 3], niq[i * 8 + 4], niq[i * 8 + 5], niq[i * 8 + 6], niq[i * 8 + 7]);
        // }
    } else {
        u8(&iq)[64] = decoder.iq;
        ...
        // for (uint i = 0; i < 8; i++) {
        //     IPU_LOG("%02X %02X %02X %02X %02X %02X %02X %02X", iq[i * 8 + 0], iq[i * 8 + 1], iq[i * 8 + 2],
        //             iq[i * 8 + 3], iq[i * 8 + 4], iq[i * 8 + 5], iq[i * 8 + 6], iq[i * 8 + 7]);
        // }
    }
...
static bool ipuSETVQ(u32 val)
{
    ...
    //         "%02d:%02d:%02d %02d:%02d:%02d %02d:%02d:%02d %02d:%02d:%02d\n"
    //         "%02d:%02d:%02d %02d:%02d:%02d %02d:%02d:%02d %02d:%02d:%02d",
    //         vqclut[0].r, vqclut[0].g, vqclut[0].b, vqclut[1].r, vqclut[1].g, vqclut[1].b, vqclut[2].r, vqclut[2].g,
    //         vqclut[2].b, vqclut[3].r, vqclut[3].g, vqclut[3].b, vqclut[4].r, vqclut[4].g, vqclut[4].b, vqclut[5].r,
    //         vqclut[5].g, vqclut[5].b, vqclut[6].r, vqclut[6].g, vqclut[6].b, vqclut[7].r, vqclut[7].g, vqclut[7].b,
    //         vqclut[8].r, vqclut[8].g, vqclut[8].b, vqclut[9].r, vqclut[9].g, vqclut[9].b, vqclut[10].r, vqclut[10].g,
    //         vqclut[10].b, vqclut[11].r, vqclut[11].g, vqclut[11].b, vqclut[12].r, vqclut[12].g, vqclut[12].b,
    //         vqclut[13].r, vqclut[13].g, vqclut[13].b, vqclut[14].r, vqclut[14].g, vqclut[14].b, vqclut[15].r,
    //         vqclut[15].g, vqclut[15].b);
...
u8 getBits64(u8 *address, bool advance)
{
    ...
    if (uint shift = (g_BP.BP & 7)) {
        u64 mask = (0xff >> shift);
        mask     = mask | (mask << 8) | (mask << 16) | (mask << 24) | (mask << 32) | (mask << 40) | (mask << 48) |
               (mask << 56);
...
__fi void IPUCMD_WRITE(u32 val)
{
    ...
    switch (ipu_cmd.CMD) {
        ...
        case SCE_IPU_FDEC:
            // IPU_LOG("FDEC command. Skip 0x%X bits, FIFO 0x%X qwords, BP 0x%X, CHCR 0x%x", val & 0x3f, g_BP.IFC,
            // g_BP.BP,
            //         ipu1ch.chcr._u32);
