...
VifUnpackSSE_Dynarec::VifUnpackSSE_Dynarec(const nVifStruct &vif_, const nVifBlock &vifBlock_) : v(vif_), vB(vifBlock_)
{
...
__fi void VifUnpackSSE_Dynarec::SetMasks(int cS) const
{
    ...
    u32 m3 = ((m0 & 0xaaaaaaaa) >> 1) & ~m0;    // all the upper bits, so our example 0x01010000 & 0xFCFDFEFF =
                                                // 0x00010000 just the cols (shifted right for maskmerge)
    ...
    if (m3 && doMask) {
        ...
        if ((cS >= 2) && (m3 & 0x0000ff00))
            xPSHUF.D(xmmCol1, xmmCol0, _v1);
        if ((cS >= 3) && (m3 & 0x00ff0000))
            xPSHUF.D(xmmCol2, xmmCol0, _v2);
        if ((cS >= 4) && (m3 & 0xff000000))
            xPSHUF.D(xmmCol3, xmmCol0, _v3);
        if ((cS >= 1) && (m3 & 0x000000ff))
            xPSHUF.D(xmmCol0, xmmCol0, _v0);
    }
...
void VifUnpackSSE_Dynarec::doMaskWrite(const xRegisterSSE &regX) const
{
    ...
    u32 m3 =
        ((m0 & 0xaa) >> 1) & ~m0;    // all the upper bits (cols shifted right) cancelling out any write protects 0x10
...
void VifUnpackSSE_Dynarec::CompileRoutine()
{
    ...
    // MSKPATH3_LOG("Compiling new block, unpack number %x, mode %x, masking %x, vNum %x", upkNum, doMode, doMask,
    // vNum);
    ...
    while (vNum) {
        ...
        if (UnpkNoOfIterations == 0)
            ShiftDisplacementWindow(srcIndirect,
                                    arg2reg);    // Don't need to do this otherwise as we arent reading the source.
...
_vifT __fi nVifBlock *dVifCompile(nVifBlock &block, bool isFill)
{
    ...
    if (v.recWritePtr > (v.recReserve->GetPtrEnd() - _256kb)) {
        DevCon.WriteLn(L"nVif Recompiler Cache Reset! [%ls > %ls]", pxsPtr(v.recWritePtr),
                       pxsPtr(v.recReserve->GetPtrEnd()));
        recReset(idx);
    }
...
_vifT __fi void dVifUnpack(const u8 *data, bool isFill)
{
    ...
    u32 key1 = ((u32)vifRegs.cycle.wl << 24) | ((u32)vifRegs.cycle.cl << 16) | ((u32)(vif.start_aligned & 0xFF) << 8) |
               ((u32)vifRegs.mode & 0xFF);
    if ((upkType & 0xf) != 9)
        key1 &= 0xFFFF01FF;
    ...
    {    // Execute the block
        ...
        if (likely((startmem + b->length) <= endmem)) {
            ...
            // VIF_LOG("Running Interpreter Block: nVif%x - VU Mem Ptr Overflow; falling back to interpreter. Start = %x
            // "
            //         "End = %x num = %x, wl = %x, cl = %x",
