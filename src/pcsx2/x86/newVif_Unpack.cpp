...
// ----------------------------------------------------------------------------
template <int idx, bool doMode, bool isFill> __ri void __fastcall _nVifUnpackLoop(const u8 *data);
...
_vifT int nVifUnpack(const u8 *data)
{
    ...
    } else    // Partial Transfer
    {
        ...
        if (!isFill) {
            ...
            vifRegs.num =
                vifRegs.num - (((dataSize / vifRegs.cycle.cl) * (vifRegs.cycle.wl - vifRegs.cycle.cl)) + dataSize);
        }
    }
...
template <int idx, bool doMode, bool isFill> __ri void __fastcall _nVifUnpackLoop(const u8 *data)
{
    ...
    // DevCon.WriteLn("[%d][%d][%d][num=%d][upk=%d][cl=%d][bl=%d][skip=%d]", isFill, doMask, doMode, vifRegs.num,
    // upkNum, vif.cl, blockSize, skipSize);
