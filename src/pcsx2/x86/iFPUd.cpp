...
struct FPUd_Globals
{
    ...
    u32 pos_inf[4], neg_inf[4], one_exp[4];
...
static const __aligned16 u32 minmax_mask[8] = {
    0xffffffff, 0x80000000, 0, 0, 0, 0x40000000, 0, 0,
};
...
void recRSQRThelper1(int regd,
                     int regt)    // Preforms the RSQRT function when regd <- Fs and regt <- Ft (Sets correct flags)
{
    u8  *pjmp1, *pjmp2;
    u8  *qjmp1, *qjmp2;
    u32 *pjmp32;
...
void recRSQRThelper2(int regd,
                     int regt)    // Preforms the RSQRT function when regd <- Fs and regt <- Ft (Doesn't set flags)
{
    xAND.PS(xRegisterSSE(regt), ptr[&s_const.pos[0]]);    // Make regt Positive
