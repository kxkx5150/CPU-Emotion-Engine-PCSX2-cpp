...
static const size_t           recLutSize =
    (Ps2MemSize::MainRam + Ps2MemSize::Rom + Ps2MemSize::Rom1 + Ps2MemSize::Rom2) * wordsize / 4;
...
{
    pxAssertDev(DispatcherReg != NULL,
                "Dynamically generated dispatchers are required prior to generating EnterRecompiledCode!");
...
static void recThrowHardwareDeficiency(const wxChar *extFail)
{
    throw Exception::HardwareDeficiency()
        ...
        .SetUserMsg(pxsFmt(
            wxString("Extensions not found.  The R5900-32 recompiler requires a host CPU with SSE2 extensions.")));
}
...
static void recReserveCache()
{
    ...
    while (!recMem->IsOk()) {
        if (recMem->Reserve(GetVmMemory().MainMemory(), HostMemoryMap::EErecOffset, m_ConfiguredCacheReserve * _1mb) !=
            NULL)
            break;
...
// Note: scaleblockcycles() scales s_nBlockCycles respective to the EECycleRate value for manipulating the cycles of
// current block recompiling. s_nBlockCycles is 3 bit fixed point.  Divide by 8 when done! Scaling blocks under 40
// cycles seems to produce countless problem, so let's try to avoid them.
...
void recompileNextInstruction(int delayslot)
{
    ...
    if (delayslot) {
        ...
        // Gregory tested this in 2017 using the ps2autotests suite and remarked "So far we return 1 (even with this
        // PR), and the HW 2. Original PR and discussion at https://github.com/PCSX2/pcsx2/pull/1783 so we don't forget
        // this information.
    ...
    if (_Opcode_ == 022)    // COP2
    {
        ...
        else if ((cpuRegs.code & 0x7FC) == 0x3BC)                                    // DIV/RSQRT/SQRT/WAITQ
        {
            ...
            for (u32 p = pc; cycles > 0 && p < s_nEndBlock; p += 4, cycles--) {
                ...
                if ((_Opcode_ == 022) &&
                    (cpuRegs.code & 0x7FC) == 0x3BC)    // WaitQ or another DIV op hit (stalled), we're safe
                    break;
            ...
            for (u32 p = pc; s != 0 && p < s_nEndBlock && all_count < 10 && cop2m_count < 5 && cop2o_count < 4;
                 p += 4) {
                // I am so sorry.
                cpuRegs.code = memRead32(p);
    ...
    switch (PageType) {
        ...
        case ProtMode_Manual:
            ...
            if (!contains_thread_stack && manual_counter[inpage_ptr >> 12] <= 3) {
                ...
                // eeRecPerfLog.Write(
                //     "Manual block @ %08X : size =%3d  page/offs = 0x%05X/0x%03X  inpgsz = %d  clearcnt = %d",
                //     startpc, size, inpage_ptr >> 12, inpage_ptr & 0xfff, inpage_sz, manual_counter[inpage_ptr >>
                //     12]);
            } else {
                // eeRecPerfLog.Write("Uncounted Manual block @ 0x%08X : size =%3d page/offs = 0x%05X/0x%03X  inpgsz =
                // %d",
...
static void __fastcall recRecompile(const u32 startpc)
{
    ...
    if (g_eeloadMain && HWADDR(startpc) == HWADDR(g_eeloadMain)) {
        ...
        if (g_SkipBiosHack) {
            // There are four known versions of EELOAD, identifiable by the location of the 'jal' to the EELOAD function
            // which calls ExecPS2(). The function itself is at the same address in all BIOSs after v1.00-v1.10.
            ...
            if ((typeBexecjump >> 26 == 3) || (typeCexecjump >> 26 == 3) ||
                (typeDexecjump >> 26 == 3))    // JAL to 0x822B8
                g_eeloadExec = EELOAD_START + 0x2B8;
            else if (typeAexecjump >> 26 == 3)    // JAL to 0x82170
                g_eeloadExec = EELOAD_START + 0x170;
            else    // There might be other types of EELOAD, because these models' BIOSs have not been examined: 18000,
                    // 3500x, 3700x, 5500x, and 7900x. However, all BIOS versions have been examined except for v1.01
                    // and v1.10.
                Console.WriteLn("recRecompile: Could not enable launch arguments for fast boot mode; unidentified BIOS "
                                "version! Please report this to the PCSX2 developers.");
        }
    ...
    if (EmuConfig.Gamefixes.GoemonTlbHack) {
        if (pc == 0x33ad48 || pc == 0x35060c) {
            ...
            // Game will unmap some virtual addresses. If a constant address were hardcoded in the block, we would be in
            // a bad situation.
            eeRecNeedsReset = true;
...
StartRecomp:
    ...
    if (HWADDR(pc) <= Ps2MemSize::MainRam) {
        ...
        while (oldBlock = recBlocks[i--]) {
            ...
            if (memcmp(&recRAMCopy[oldBlock->startpc / 4], PSM(oldBlock->startpc), oldBlock->size * 4)) {
