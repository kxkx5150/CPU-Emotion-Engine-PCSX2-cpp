...
class iAllocRegSSE {
  ...
  public:
    iAllocRegSSE() : m_reg(xmm0), m_free(!!_hasFreeXMMreg())
    {
        if (m_free)
...
/*
        ...
        else
        {
                ...
                return
 reinterpret_cast<TemplateHelper<DataSize,false>::HandlerType*>(RWFT[TemplateHelper<DataSize,false>::sidx][0][hand])(paddr,data);
        }
...
int vtlb_DynGenRead64(u32 bits, int gpr)
{
    ...
    int reg =
        gpr == -1 ? _allocTempXMMreg(XMMT_INT, 0) : _allocGPRtoXMMreg(0, gpr, MODE_WRITE);    // Handler returns in xmm0
...
int vtlb_DynGenRead64_Const(u32 bits, u32 addr_const, int gpr)
{
    ...
    if (!vmv.isHandler(addr_const)) {
        ...
        reg = gpr == -1 ? _allocTempXMMreg(XMMT_INT, 0)
                        : _allocGPRtoXMMreg(0, gpr, MODE_WRITE);    // Handler returns in xmm0
