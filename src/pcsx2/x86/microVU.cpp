...
static __fi void mVUthrowHardwareDeficiency(const wxChar *extFail, int vuIndex)
{
    throw Exception::HardwareDeficiency()
        ...
        .SetUserMsg(
            pxsFmt(wxString("%s Extensions not found.  microVU requires a host CPU with SSE2 extensions."), extFail));
}
...
void mVUreserveCache(microVU &mVU)
{
    ...
    mVU.cache = mVU.index ? (u8 *)mVU.cache_reserve->Reserve(GetVmMemory().MainMemory(), HostMemoryMap::mVU1recOffset,
                                                             mVU.cacheSize * _1mb)
                          : (u8 *)mVU.cache_reserve->Reserve(GetVmMemory().MainMemory(), HostMemoryMap::mVU0recOffset,
                                                             mVU.cacheSize * _1mb);
...
__ri microProgram *mVUcreateProg(microVU &mVU, int startPC)
{
    ...
    DevCon.WriteLn(c, "microVU%d: Cached Prog = [%03d] [PC=%04x] [List=%02d] (Cache=%3.3f%%) [%3.1fmb]", mVU.index,
                   prog->idx, startPC * 8, mVU.prog.prog[startPC]->size() + 1, cachePerc, cacheUsed);
    return prog;
}
    ...
    if (!quick.prog)    // If null, we need to search for new program
    {
        ...
        for (; it != list->end(); ++it) {
            ...
            if (b) {
                ...
                // Sanity check, in case for some reason the program compilation aborted half way through (JALR for
                // example)
...
void recMicroVU0::Vsync() noexcept
{
    mVUvsyncUpdate(microVU0);
}
void recMicroVU1::Vsync() noexcept
{
