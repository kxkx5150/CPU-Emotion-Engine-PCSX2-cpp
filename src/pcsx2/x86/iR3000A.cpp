...
#define PSXREC_CLEARM(mem) (((mem) < g_psxMaxRecMem && (psxRecLUT[(mem) >> 16] + (mem))) ? psxRecClearMem(mem) : 4)
...
static void iIopDumpBlock(int startpc, u8 *ptr)
{
    ...
    //     int status = std::system(wxsFormat(L"objdump -D -b binary -mi386 -M intel --no-show-raw-insn %s >> %s; rm
    //     %s",
...
// rd = rs op rt
void psxRecompileCodeConst0(R3000AFNPTR constcode, R3000AFNPTR_INFO constscode, R3000AFNPTR_INFO consttcode,
                            R3000AFNPTR_INFO noconstcode)
{
    if (!_Rd_)
        return;
...
void psxRecompileCodeConst3(R3000AFNPTR constcode, R3000AFNPTR_INFO constscode, R3000AFNPTR_INFO consttcode,
                            R3000AFNPTR_INFO noconstcode, int LOHI)
{
    _deleteX86reg(X86TYPE_PSX, _Rs_, 1);
    _deleteX86reg(X86TYPE_PSX, _Rt_, 1);
...
static void recReserveCache()
{
    ...
    while (!recMem->IsOk()) {
        if (recMem->Reserve(GetVmMemory().MainMemory(), HostMemoryMap::IOPrecOffset, m_ConfiguredCacheReserve * _1mb) !=
            NULL)
            break;
...
R3000Acpu psxRec = {recReserve,         recResetIOP,       recExecute, recExecuteBlock, recClearIOP, recShutdown,

                    recGetCacheReserve, recSetCacheReserve};
