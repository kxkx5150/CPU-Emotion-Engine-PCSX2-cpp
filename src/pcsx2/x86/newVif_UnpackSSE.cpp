...
static const __aligned16 u32 SSEXYZWMask[4][4] = {{0xffffffff, 0xffffffff, 0xffffffff, 0x00000000},
...
VifUnpackSSE_Base::VifUnpackSSE_Base()
    : usn(false), doMask(false), UnpkLoopIteration(0), UnpkNoOfIterations(0), IsAligned(0), dstIndirect(arg1reg),
      srcIndirect(arg2reg), workReg(xmm1), destReg(xmm0)
{
}
...
void VifUnpackSSE_Base::xUPK_V3_16() const
{
    ...
    if ((UnpkLoopIteration & 0x1) == 0 && result == 0) {
        xAND.PS(destReg,
                ptr128[SSEXYZWMask[0]]);    // zero last word on QW boundary if whole 32bit word is used - tested on ps2
    }
}
...
void VifUnpackSSE_Init()
{
    ...
    DevCon.Indent().WriteLn(L"Reserved buffer    : %u bytes @ %ls\n"
