...
void JoystickInfo::EnumerateJoysticks(std::vector<std::unique_ptr<Device>> &vjoysticks)
{
    ...
    if ((SDL_WasInit(0) & flag) != flag) {
        ...
        {    // Support as much Joystick as possible
            GBytes *bytes = g_resource_lookup_data(PAD_res_get_resource(), "/PAD/res/game_controller_db.txt",
...
JoystickInfo::JoystickInfo(int id) : Device(), m_controller(nullptr), m_haptic(nullptr), m_unique_id(0)
{
    ...
    fprintf(stdout, "PAD: controller (%s) detected%s, GUID:%s\n", m_device_name.c_str(),
            m_haptic ? " with rumble support" : "", guid);
