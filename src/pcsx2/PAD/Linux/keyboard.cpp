...
// Mac keyboard input code is based on Dolphin's
// Source/Core/InputCommon/ControllerInterface/Quartz/QuartzKeyboardAndMouse.mm Copyright 2016 Dolphin Emulator Project
    ...
    for (u32 pad = 0; pad < GAMEPAD_NUMBER; pad++) {
        ...
        for (const auto &key : map) {
            ...
            if (key.first >> 16 == 0) {
                ...
                state =
                    CGEventSourceButtonState(kCGEventSourceStateHIDSystemState, (CGMouseButton)(key.first & 0xFFFF));
            }
            if (state)
...
void AnalyzeKeyEvent(HostKeyEvent &evt)
{
    ...
    switch (static_cast<int>(evt.type)) {
        case KeyPress:
            ...
            //     if (!s_grab_input) {
            //         s_grab_input = true;
            //         XGrabPointer(GSdsp, GSwin, True, ButtonPressMask, GrabModeAsync, GrabModeAsync, GSwin, None,
            //                      CurrentTime);
...
void UpdateKeyboardInput()
{
    ...
    //             break;
    //         default:;    // evt.key = (int)XLookupKeysym(&E.xkey, 0);
    //     }
