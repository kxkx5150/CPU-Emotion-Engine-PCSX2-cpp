...
static __ri void DmaExec(void (*func)(), u32 mem, u32 value)
{
    ...
    if (reg.chcr.STR) {
        ...
        if (chcr.STR == 0) {
            // DevCon.Warning(L"32bit Force Stopping %s (Current CHCR %x) while DMA active", ChcrName(mem),
            // reg.chcr._u32, chcr._u32);
            reg.chcr.STR = 0;
        ...
        // else DevCon.Warning(L"32bit Attempted to change %s CHCR (Currently %x) with %x while DMA active, ignoring QWC
        // = %x", ChcrName(mem), reg.chcr._u32, chcr._u32, reg.qwc);
        return;
    }

    // if(reg.chcr.TAG != chcr.TAG && chcr.MOD == CHAIN_MODE) DevCon.Warning(L"32bit CHCR Tag on %s changed to %x from
    // %x QWC = %x Channel Not Active", ChcrName(mem), chcr.TAG, reg.chcr.TAG, reg.qwc);

    reg.chcr.set(value);

    // Final Fantasy XII sets the DMA Mode to 3 which doesn't exist. On some channels (like SPR) this will break logic
    // completely. so lets assume they mean chain.
    if (reg.chcr.MOD == 0x3) {
    ...
    // As tested on hardware, if NORMAL mode is started with 0 QWC it will actually transfer 1 QWC then underflows and
    // transfer another 0xFFFF QWC's The easiest way to handle this is to just say 0x10000 QWC
    ...
    if (reg.chcr.STR && dmacRegs.ctrl.DMAE && !psHu8(DMAC_ENABLER + 2)) {
        ...
        QueuedDMA._u16 |=
            (1 << ChannelNumber(
                 mem));    // Queue the DMA up to be started then the DMA's are Enabled and or the Suspend is lifted
    }                      // else QueuedDMA._u16 &~= (1 << ChannelNumber(mem)); //
}

template <uint page> __fi u32 dmacRead32(u32 mem)
{
...
template <uint page> __fi bool dmacWrite32(u32 mem, mem32_t &value)
{
