...
void handleGp1Command(u32 cmd)
{
    ...
    switch (cmdNr) {
        case 2:    // Acknowledge GPU IRQ
            ackGpuIrq1();
            break;
        case 4:    // DMA Direction / Data Request. The PS1DRV doesn't care of this... Should we do something on pgif
                   // ctrl?
...
u32 getUpdPgpuStatReg()
{
    ...
    // The PS1 program will not send the DMA direction command (GP1(04h)) and will not start DMA until this bit (27)
    // becomes set.
...
{
    // if (((addr != PGIF_CTRL) || (addr != PGPU_STAT) || ((addr == PGIF_CTRL) && (getUpdPgifCtrlReg() != data))) &&
    // (addr != PGPU_STAT))
    REG_LOG("PGIF write 0x%08X = 0x%08X  0x%08X  EEpc= %08X  IOPpc= %08X ", addr, data, getUpdPgifCtrlReg(), cpuRegs.pc,
            psxRegs.pc);
    ...
    switch (addr) {
        ...
        case PGPU_DAT_FIFO:
            ringBufPut(&rb_gp0, &data);
            // 	Console.WriteLn( "\n\r PGIF REVERSE !!! DATA write 0x%08X = 0x%08X  IF_CTRL= %08X   PGPU_STAT= %08X
...
u32 PGIFr(int addr)
{
    ...
    //	if (addr != PGPU_STAT)
    //		REG_LOG("PGIF read %08X = %08X  GPU_ST %08X  IF_STAT %08X IOPpc %08X EEpc= %08X ", addr, data,
    // getUpdPgpuStatReg(), getUpdPgifCtrlReg(), psxRegs.pc, cpuRegs.pc);
...
void PGIFrQword(u32 addr, void *dat)
{
    ...
    if (addr == PGPU_CMD_FIFO) {
        ...
        Console.WriteLn("Data = %08X %08X %08X %08X ", *(u32 *)(data + 0), *(u32 *)(data + 1), *(u32 *)(data + 2),
                        *(u32 *)(data + 3));
    }
}
...
void PGIFwQword(u32 addr, void *dat)
{
    ...
    Console.WriteLn("PGIF QW write  0x%08X = 0x%08X %08X %08X %08X ", addr, *(u32 *)(data + 0), *(u32 *)(data + 1),
...
void fillFifoOnDrain()
{
    ...
    // This is done here in a loop, rather than recursively in each function, because a very large buffer causes stack
    // oveflow.
    while ((rb_gp0.count < ((rb_gp0.size) - PGIF_DAT_RB_LEAVE_FREE)) &&
           ((dma.state.to_gpu_active) || (dma.state.ll_active))) {
        drainPgpuDmaLl();
        drainPgpuDmaNrToGpu();
    }
    // Clear bit as DMA will be run - normally it should be cleared only once the current request finishes, but the IOP
    // won't notice anything anyway. WARNING: Current implementation assume that GPU->IOP DMA uses this flag, so we only
    // clear it here if the mode is not GPU->IOP.
...
void drainPgpuDmaLl()
{
    ...
    // so the result is that this function loops indefinitely, because of some links pointing back to themselves,
    // forming a loop. The solution is to only start DMA once the GP1(04h) - DMA Direction / Data Request command has
    // been set to the value 0x2 (CPU->GPU DMA)
    ...
    if (dma.ll_dma.current_word >= dma.ll_dma.total_words) {
        if (dma.ll_dma.next_address == DMA_LL_END_CODE) {
            ...
            dma.ll_dma.current_word      = 0;
            dma.ll_dma.total_words =
                (data >> 24) & 0xFF;    // Current length of packet and future address of header word.
...
void drainPgpuDmaNrToIop()
{
    ...
    if (dma.normal.current_word < dma.normal.total_words) {
        ...
        dma.normal.address += 4;
        dma.normal.current_word++;
        // dmaRegs.madr.address += 4; //It is unclear if this should be done exactly so... // kozarovv: WHY EVEN DO THAT
        // AGAIN?
        if ((dma.normal.current_word % dmaRegs.bcr.bit.block_size) == 0) {
            // decrease block amount only if full block size were drained.
            dmaRegs.bcr.bit.block_amount -= 1;
        }
        // PGPU_DMA_LOG("GPU->IOP ba: %x , cw: %x , tw: %x" ,dmaRegs.bcr.bit.block_amount, dma.normal.current_word,
        // dma.normal.total_words);
    }
    if (dma.normal.current_word >= dma.normal.total_words) {
        dma.state.to_iop_active = 0;
        dmaRegs.chcr.bits.BUSY  = 0;
...
void processPgpuDma()
{
    ...
    // PGPU_DMA_LOG("Starting GPU DMA! CHCR %08X  BCR %08X  MADR %08X ", dmaRegs.chcr.get(), dmaRegs.bcr.get(),
    // dmaRegs.madr.address);
    ...
    if (dmaRegs.chcr.bits.TSM == 2) {
        ...
        if (dmaRegs.chcr.bits.DIR) {
            ...
            dma.ll_dma.next_address =
                (dmaRegs.madr.address &
                 0x00FFFFFF);    // The address in IOP RAM where to load the first header word from
            dma.ll_dma.current_word = 0;
            dma.ll_dma.total_words  = 0;
