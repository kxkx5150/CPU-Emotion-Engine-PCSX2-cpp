...
//////////////////////////////////////////////////////////////////////////////////////////
//
template <typename T> static __fi void _generic_write(u32 addr, T val)
{
...
void __fastcall iopHwWrite8_generic(u32 addr, mem8_t val)
{
    _generic_write<mem8_t>(addr, val);
}
void __fastcall iopHwWrite16_generic(u32 addr, mem16_t val)
{
    _generic_write<mem16_t>(addr, val);
}
void __fastcall iopHwWrite32_generic(u32 addr, mem32_t val)
{
    _generic_write<mem32_t>(addr, val);
}

//////////////////////////////////////////////////////////////////////////////////////////
//
template <typename T> static __fi T _generic_read(u32 addr)
{
    // int bitsize = (sizeof(T) == 1) ? 8 : ( (sizeof(T) == 2) ? 16 : 32 );
...
mem8_t __fastcall iopHwRead8_generic(u32 addr)
{
    return _generic_read<mem8_t>(addr);
}
mem16_t __fastcall iopHwRead16_generic(u32 addr)
{
    return _generic_read<mem16_t>(addr);
}
mem32_t __fastcall iopHwRead32_generic(u32 addr)
{
    return _generic_read<mem32_t>(addr);
}
...
void __fastcall iopHwWrite8_Page1(u32 addr, mem8_t val)
{
    ...
    switch (masked_addr) {
        mcase(HW_SIO_DATA) : sioWrite8(val);
        break;

        // for use of serial port ignore for now
        // case 0x50: serial_write8( val ); break;

        mcase(HW_DEV9_DATA) : DEV9write8(addr, val);
        break;

        mcase(HW_CDR_DATA0) : cdrWrite0(val);
        break;
        mcase(HW_CDR_DATA1) : cdrWrite1(val);
        break;
        mcase(HW_CDR_DATA2) : cdrWrite2(val);
        break;
        mcase(HW_CDR_DATA3) : cdrWrite3(val);
        break;
...
//
template <typename T> static __fi void _HwWrite_16or32_Page1(u32 addr, T val)
{
    // all addresses are assumed to be prefixed with 0x1f801xxx:
    pxAssert((addr >> 12) == 0x1f801);

    // all addresses should be aligned to the data operand size:
    pxAssert((sizeof(T) == 2 && (addr & 1) == 0) || (sizeof(T) == 4 && (addr & 3) == 0));
    ...
    //
        ...
        switch (masked_addr) {
            // ------------------------------------------------------------------------
            mcase(HW_SIO_DATA) : sioWrite8(val & 0xFF);
            ...
            mcase(HW_SIO_MODE) : sio.ModeReg = (u16)val;
            if (sizeof(T) == 4) {
            ...
            mcase(HW_SIO_BAUD) : sio.BaudReg = (u16)val;
            break;
            ...
            mcase(HW_IREG + 2) : psxHu(addr) &= val;
            break;

            mcase(HW_IMASK) : psxHu(addr) = val;
            iopTestIntc();
            break;

            mcase(HW_IMASK + 2) : psxHu(addr) = val;
            iopTestIntc();
            break;

            mcase(HW_ICTRL) : psxHu(addr) = val;
            iopTestIntc();
            break;

            mcase(HW_ICTRL + 2) : psxHu(addr) = val;
            iopTestIntc();
            break;
            ...
            break;
            mcase(0x1f8010ac) : DevCon.Warning("SIF2 IOP TADR?? write");
            psxHu(addr) = val;
            break;
            ...
            mcase(0x1f8010f4) :
            {
                ...
                if (((newtmp >> 15) & 0x1) ||
                    (((newtmp >> 23) & 0x1) == 0x1 && (((newtmp & 0x7F000000) >> 8) & (newtmp & 0x7F0000)) != 0)) {
                    newtmp |= 0x80000000;
                } else {
                    newtmp &= ~0x80000000;
                }
            ...
            mcase(HW_DEV9_DATA) : DEV9write16(addr, val);
            psxHu(addr) = val;
            break;
...
void __fastcall iopHwWrite32_Page8(u32 addr, mem32_t val)
{
    ...
    if (masked_addr >= 0x200) {
        if (masked_addr < 0x240) {
            ...
            if (masked_addr & 4)
                sio2_setSend2(parm, val);
            else
                sio2_setSend1(parm, val);
        } else if (masked_addr <= 0x280) {
            switch (masked_addr) {
                mcase(HW_SIO2_CTRL) : sio2_setCtrl(val);
                break;
                mcase(0x1f808278) : sio2_set8278(val);
                break;
                mcase(0x1f80827C) : sio2_set827C(val);
                break;
                mcase(HW_SIO2_INTR) : sio2_setIntr(val);
                break;
