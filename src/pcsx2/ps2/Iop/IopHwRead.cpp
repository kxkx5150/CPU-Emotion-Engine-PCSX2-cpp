...
mem8_t __fastcall iopHwRead8_Page1(u32 addr)
{
    ...
    switch (masked_addr) {
        ...
        mcase(HW_DEV9_DATA) : ret = DEV9read8(addr);
        break;

        mcase(HW_CDR_DATA0) : ret = cdrRead0();
        break;
        mcase(HW_CDR_DATA1) : ret = cdrRead1();
        break;
        mcase(HW_CDR_DATA2) : ret = cdrRead2();
        break;
        mcase(HW_CDR_DATA3) : ret = cdrRead3();
        break;
...
//////////////////////////////////////////////////////////////////////////////////////////
//
template <typename T> static __fi T _HwRead_16or32_Page1(u32 addr)
{
    // all addresses are assumed to be prefixed with 0x1f801xxx:
    pxAssume((addr >> 12) == 0x1f801);

    // all addresses should be aligned to the data operand size:
    pxAssume((sizeof(T) == 2 && (addr & 1) == 0) || (sizeof(T) == 4 && (addr & 3) == 0));
    ...
    //
        ...
        switch (masked_addr) {
            // ------------------------------------------------------------------------
            mcase(HW_SIO_DATA) : ret = sioRead8();
            ret |= sioRead8() << 8;
            if (sizeof(T) == 4) {
                ret |= sioRead8() << 16;
                ret |= sioRead8() << 24;
            ...
            mcase(HW_SIO_CTRL) : ret = sio.CtrlReg;
            break;

            mcase(HW_SIO_BAUD) : ret = sio.BaudReg;
            break;
            ...
            mcase(HW_ICTRL) : ret = psxHu32(0x1078);
            psxHu32(0x1078)       = 0;
            break;

            mcase(HW_ICTRL + 2) : ret = psxHu16(0x107a);
            ...
            //
            mcase(0x1f8010ac) : ret = psxHu32(addr);
            DevCon.Warning("SIF2 IOP TADR?? read");
            break;

            mcase(HW_PS1_GPU_DATA) : ret = psxGPUr(addr);
            break;

            mcase(HW_PS1_GPU_STATUS) : ret = psxGPUr(addr);
            break;
            ...
            mcase(0x1f80146e) : ret = DEV9read16(addr);
            break;
...
mem32_t __fastcall iopHwRead32_Page8(u32 addr)
{
    ...
    if (masked_addr >= 0x200) {
        if (masked_addr < 0x240) {
            ...
            switch (masked_addr) {
                mcase(HW_SIO2_CTRL) : ret = sio2_getCtrl();
                break;
                mcase(HW_SIO2_RECV1) : ret = sio2_getRecv1();
                break;
                mcase(HW_SIO2_RECV2) : ret = sio2_getRecv2();
                break;
                mcase(HW_SIO2_RECV3) : ret = sio2_getRecv3();
                break;
                mcase(0x1f808278) : ret = sio2_get8278();
                break;
                mcase(0x1f80827C) : ret = sio2_get827C();
                break;
                mcase(HW_SIO2_INTR) : ret = sio2_getIntr();
                break;
