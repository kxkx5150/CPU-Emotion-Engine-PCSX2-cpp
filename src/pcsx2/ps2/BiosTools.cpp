...
static void LoadBiosVersion(pxInputStream &fp, u32 &version, wxString &description, u32 &region, wxString &zoneStr)
{
    ...
    if (i == 512 * 1024) {
        throw Exception::BadStream(fp.GetStreamName())
            ...
            .SetUserMsg(wxString("The selected BIOS file is not a valid PS2 BIOS.  Please re-configure."));
    }
    ...
    while (strlen(rd.fileName) > 0) {
        if (strcmp(rd.fileName, "ROMVER") == 0) {
            ...
            result.Write("%-7s v%s.%s(%c%c/%c%c/%c%c%c%c)  %s", zone, vermaj, vermin, romver[12], romver[13],    // day
...
template <size_t _size> void ChecksumIt(u32 &result, const u8 (&srcdata)[_size])
{
    pxAssume((_size & 3) == 0);
...
//
template <size_t _size> static void LoadExtraRom(const wxChar *ext, u8 (&dest)[_size])
{
    wxString Bios1;
    s64      filesize = 0;
    ...
    try {
        ...
        if (EmuConfig.BaseFilenames.Bios.empty())
            throw Exception::FileNotFound(Bios)
                ...
                .SetUserMsg(wxString("The PS2 BIOS could not be loaded.  The BIOS has not been configured, or the "
        ...
        if (filesize <= 0) {
            throw Exception::FileNotFound(Bios)
                ...
                .SetUserMsg(wxString("The configured BIOS file does not exist.  Please re-configure."));
        }
        ...
        Console.SetTitle(pxsFmt(L"Running BIOS (%s v%u.%u)", WX_STR(biosZone), BiosVersion >> 8, BiosVersion & 0xff));
        ...
        throw Exception::BiosLoadFailed(ex.StreamName).SetDiagMsg(ex.DiagMsg()).SetUserMsg(ex.UserMsg());
    }
}
