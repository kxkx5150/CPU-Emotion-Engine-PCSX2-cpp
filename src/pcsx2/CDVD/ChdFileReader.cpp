...
bool ChdFileReader::Open2(const wxString &fileName)
{
    ...
    while (CHDERR_REQUIRES_PARENT == (error = chd_open(chds[chd_depth].c_str(), CHD_OPEN_READ, NULL, &child))) {
        ...
        if (chd_read_header(chds[chd_depth].c_str(), &header) != CHDERR_NONE) {
            Console.Error(L"CDVD: chd_open chd_read_header error: %s: %s", chd_error_string(error),
                          WX_STR(chds[chd_depth]));
            return false;
        }
        ...
        if (dir.IsOpened()) {
            ...
            bool     cont =
                dir.GetFirst(&parent_fileName, wxString("*.") + wxfilename.GetExt(), wxDIR_FILES | wxDIR_HIDDEN);
