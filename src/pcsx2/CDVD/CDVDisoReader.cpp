...
static void FindLayer1Start()
{
    ...
    uint      blockresult =
        buffer[offset + 80] + (buffer[offset + 81] << 8) + (buffer[offset + 82] << 16) + (buffer[offset + 83] << 24);
...
s32 CALLBACK ISOgetTOC(void *toc)
{
    ...
    } else if ((type == CDVD_TYPE_CDDA) || (type == CDVD_TYPE_PS2CDDA) || (type == CDVD_TYPE_PS2CD) ||
               (type == CDVD_TYPE_PSCDDA) || (type == CDVD_TYPE_PSCD)) {
        // cd toc
...
CDVD_API CDVDapi_Iso = {
    ...
    ISOopen,       ISOreadTrack,   ISOgetBuffer, ISOreadSubQ, ISOgetTN, ISOgetTD, ISOgetTOC, ISOgetDiskType,
