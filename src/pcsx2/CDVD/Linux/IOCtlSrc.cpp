...
IOCtlSrc::IOCtlSrc(decltype(m_filename) filename) : m_filename(filename)
{
    if (!Reopen())
...
bool IOCtlSrc::ReadCDInfo()
{
    ...
    for (u8 n = header.cdth_trk0; n <= header.cdth_trk1; ++n) {
        ...
        if (ioctl(m_device, CDROMREADTOCENTRY, &entry) != -1)
            m_toc.push_back({static_cast<u32>(entry.cdte_addr.lba), entry.cdte_track, entry.cdte_adr, entry.cdte_ctrl});
    }
