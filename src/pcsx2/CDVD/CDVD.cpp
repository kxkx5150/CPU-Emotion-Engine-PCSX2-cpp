...
static void cdvdNVM(u8 *buffer, int offset, size_t bytes, bool read)
{
    ...
    if (ret != bytes)
        Console.Error(L"Failed to %s %s. Did only %zu/%zu bytes", read ? L"read from" : L"write to", WX_STR(fname), ret,
                      bytes);
}
...
s32 cdvdReadConfig(u8 *config)
{
    ...
    // check if block index is in bounds
        ...
        return 1;
    else if (((cdvd.COffset == 0) && (cdvd.CBlockIndex >= 4)) || ((cdvd.COffset == 1) && (cdvd.CBlockIndex >= 2)) ||
...
static __fi void _reloadElfInfo(wxString elfpath)
{
    ...
    Console.WriteLn(Color_StrongBlue, L"ELF (%s) Game CRC = 0x%08X, EntryPoint = 0x%08X", WX_STR(elfpath), ElfCRC,
                    ElfEntry);
...
void cdvdReloadElfInfo(wxString elfoverride)
{
    ...
    try {
        ...
        if (discType == 1) {
            ...
            // To support different PSX titles in the console title and for savestates, this code bypasses all the
            // detection, simply using the exe name, stripped of problematic characters.
            wxString fname = elfpath.AfterLast('\\').AfterLast(
                ':');    // Also catch elf paths which lack a backslash, and only have a colon.
...
void cdvdReadKey(u8, u16, u32 arg2, u8 *key)
{
    ...
    if (!DiscSerial.IsEmpty()) {
        ...
        letters = (s32)((DiscSerial[3].GetValue() & 0x7F) << 0) | (s32)((DiscSerial[2].GetValue() & 0x7F) << 7) |
                  (s32)((DiscSerial[1].GetValue() & 0x7F) << 14) | (s32)((DiscSerial[0].GetValue() & 0x7F) << 21);
    }
    ...
    DevCon.WriteLn("CDVD.KEY = %02X,%02X,%02X,%02X,%02X,%02X,%02X", cdvd.Key[0], cdvd.Key[1], cdvd.Key[2], cdvd.Key[3],
                   cdvd.Key[4], cdvd.Key[14], cdvd.Key[15]);
}
...
{
    if (cdvd.Type == CDVD_TYPE_DETCTDVDS || cdvd.Type == CDVD_TYPE_DETCTDVDD || cdvd.Type == CDVD_TYPE_PS2DVD ||
        cdvd.Type == CDVD_TYPE_DVDV)
        return true;
    else
        return false;
}
...
static uint cdvdRotationalLatency(CDVD_MODE_TYPE mode)
{
    ...
    if (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) {
        ...
        // CLV adjusts its speed based on where it is on the disc, so we can take the max RPM and use the sector to work
        // it out
        ...
        float rotationPerSecond =
            (((mode == MODE_CDROM) ? CD_MAX_ROTATION_X1 : DVD_MAX_ROTATION_X1) * cdvd.Speed * sectorSpeed) / 60;
        float msPerRotation = 1000.0f / rotationPerSecond;
        // DevCon.Warning("Rotations per second %f, msPerRotation cycles per ms %f total cycles per ms %d cycles per
        // rotation %d", rotationPerSecond, msPerRotation, (u32)(PSXCLK / 1000), (u32)((PSXCLK / 1000) *
        // msPerRotation));
...
static uint cdvdBlockReadTime(CDVD_MODE_TYPE mode)
{
    ...
    if (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) {
        ...
        return (PSXCLK /
                ((((mode == MODE_CDROM) ? CD_SECTORS_PERSECOND : DVD_SECTORS_PERSECOND) * cdvd.Speed) * sectorSpeed));
        // return ((PSXCLK * cdvd.BlockSize) / ((float)(((mode == MODE_CDROM) ? PSX_CD_READSPEED : PSX_DVD_READSPEED) *
        // cdvd.Speed) * sectorSpeed));
    }
...
__fi void cdvdActionInterrupt()
{
    switch (cdvd.Action) {
        ...
        case cdvdAction_Break:
            ...
            cdvd.Status =
                CDVD_STATUS_PAUSE;    // Break stops the command in progress it doesn't stop the drive. Formula 2001
            cdvd.RErr = 0;
            break;
    }
    cdvd.Action   = cdvdAction_None;
...
{
    // Console.WriteLn("cdvdReadInterrupt %x %x %x %x %x", cpuRegs.interrupt, cdvd.Readed, cdvd.Reading, cdvd.nSectors,
    // (HW_DMA3_BCR_H16 * HW_DMA3_BCR_L16) *4);
    ...
    if (cdvd.nSectors > 0 && cdvd.nextSectorsBuffered) {
        ...
        if (--cdvd.nSectors <= 0) {
            ...
            //  Star Ocean (1.1 Japan) expects the DMA to end and interrupt at least 128 or more cycles before the CDVD
            //  command ends. However the time required seems to increase slowly, so delaying the end of the command is
            //  not the solution.
            // cdvd.Status = CDVD_STATUS_PAUSE; // Needed here but could be smth else than Pause (rama)
            //  All done! :D
            return;
...
static uint cdvdStartSeek(uint newsector, CDVD_MODE_TYPE mode)
{
    ...
    // So In the case where it's seeking to data it will be Spinning (0x2) not reading (0x8) and Seeking (0x10, but
    // because seeking is also spinning 0x2 is also set)) Update - Apparently all that was rubbish and some games don't
    // like it. WRC was the one in this scenario which hated SEEK |ZPAUSE, so just putting it back to pause for now. We
    // should really run some tests for this behaviour.
    ...
    if (!cdvd.Spinning) {
        ...
        if (delta >= tbl_FastSeekDelta[mode]) {
            // Full Seek
            // CDVD_LOG("CdSeek Begin > to sector %d, from %d - delta=%d [FULL]", cdvd.SeekToSector, cdvd.Sector,
            // delta);
            seektime = Cdvd_FullSeek_Cycles;
        } else {
            // CDVD_LOG("CdSeek Begin > to sector %d, from %d - delta=%d [FAST]", cdvd.SeekToSector, cdvd.Sector,
            // delta);
        ...
        if (delta == 0) {
            ...
            cdvd.Reading = 1;    // We don't need to wait for it to read a sector as it's already queued up, or we
                                 // adjust for it here.
            cdvd.RetryCntP = 0;
            ...
            if (!cdvd.nextSectorsBuffered)    // Buffering time hasn't completed yet so cancel it and simulate the
                                              // remaining time
            {
                if (psxRegs.interrupt & (1 << IopEvt_CdvdSectorReady)) {
                    // DevCon.Warning("coming back from ready sector early reducing %d cycles by %d cycles", seektime,
                    // psxRegs.cycle - psxRegs.sCycle[IopEvt_CdvdSectorReady]);
    ...
    if (delta && cdvd.nCommand != N_CD_SEEK) {
        ...
        // DevCon.Warning("%s rotational latency at sector %d is %d cycles", (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) ?
    ...
    switch (key) {
        ...
        case 0x0F:    // TYPE
            if (cdvd.Tray.trayState == CDVD_DISC_ENGAGED) {
                ...
                // CDVD_LOG("cdvdRead0F(Disc Type) Detecting %x", (cdvd.Tray.trayState <= CDVD_DISC_SEEKING) ?
                // cdvdTrayStateDetecting() : 0);
                return (cdvd.Tray.trayState <= CDVD_DISC_SEEKING) ? cdvdTrayStateDetecting()
                                                                  : 0;    // Detecting Disc / No Disc
            }
...
{    // NCOMMAND
    ...
    switch (rt) {
        ...
        case N_CD_READ:    // CdRead
            // Assign the seek to sector based on cdvd.Param[0]-[3], and the number of  sectors based on
            // cdvd.Param[4]-[7].
            ...
            // CDVD_LOG("CDRead > startSector=%d, seekTo=%d nSectors=%d, RetryCnt=%x, Speed=%dx(%s), ReadMode=%x(%x)
            // SpindleCtrl=%x", 	cdvd.Sector, cdvd.SeekToSector, cdvd.nSectors, cdvd.RetryCnt, cdvd.Speed,
            // (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) ? L"CAV" : L"CLV", cdvd.ReadMode, cdvd.Param[10], cdvd.SpindlCtrl);

            if (EmuConfig.CdvdVerboseReads)
                Console.WriteLn(Color_Gray,
                                L"CDRead: Reading Sector %07d (%03d Blocks of Size %d) at Speed=%dx(%s) Spindle=%x",
                                cdvd.SeekToSector, cdvd.nSectors, cdvd.BlockSize, cdvd.Speed,
                                (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) ? L"CAV" : L"CLV", cdvd.SpindlCtrl);
        ...
        case N_CD_READ_XCDDA:    // CdReadXCDDA
            ...
            if (EmuConfig.CdvdVerboseReads)
                Console.WriteLn(
                    Color_Gray,
                    L"CdAudioRead: Reading Sector %07d (%03d Blocks of Size %d) at Speed=%dx(%s) Spindle=%x",
        ...
        case N_DVD_READ:    // DvdRead
            ...
            // CDVD_LOG("DvdRead > startSector=%d, seekTo=%d nSectors=%d, RetryCnt=%x, Speed=%dx(%s), ReadMode=%x(%x)
            // SpindleCtrl=%x", 	cdvd.Sector, cdvd.SeekToSector, cdvd.nSectors, cdvd.RetryCnt, cdvd.Speed,
            // (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) ? L"CAV" : L"CLV", cdvd.ReadMode, cdvd.Param[10], cdvd.SpindlCtrl);
            ...
            if (EmuConfig.CdvdVerboseReads)
                Console.WriteLn(
                    Color_Gray,
                    L"DvdRead: Reading Sector %07d (%03d Blocks of Size %d) at Speed=%dx(%s) SpindleCtrl=%x",
                    cdvd.SeekToSector, cdvd.nSectors, cdvd.BlockSize, cdvd.Speed,
                    (cdvd.SpindlCtrl & CDVD_SPINDLE_CAV) ? L"CAV" : L"CLV", cdvd.SpindlCtrl);
...
{
    // Rama Or WISI guessed that "2" literally meant 2x but we can get 0x02 or 0xfe for "Standard" or "Fast" it appears.
    // It is unsure what those values are meant to be Tests with ref suggest this register is write only? - Weirdbeard
    if (rt == 0xFE)
    ...
    try {
        ...
        switch (rt) {
            ...
            case 0x08:    // CdReadRTC (0:8)
                ...
                /*Console.WriteLn("RTC Read Sec %x Min %x Hr %x Day %x Month %x Year %x", cdvd.Result[1],
                  cdvd.Result[2], cdvd.Result[3], cdvd.Result[5], cdvd.Result[6], cdvd.Result[7]); Console.WriteLn("RTC
                  Read Real Sec %d Min %d Hr %d Day %d Month %d Year %d", cdvd.RTC.second, cdvd.RTC.minute,
            ...
            case 0x09:    // sceCdWriteRTC (7:1)
                ...
                /*Console.WriteLn("RTC write incomming Sec %x Min %x Hr %x Day %x Month %x Year %x",
                  cdvd.Param[cdvd.ParamP-7], cdvd.Param[cdvd.ParamP-6], cdvd.Param[cdvd.ParamP-5],
                  cdvd.Param[cdvd.ParamP-3], cdvd.Param[cdvd.ParamP-2], cdvd.Param[cdvd.ParamP-1]); Console.WriteLn("RTC
                  Write Sec %d Min %d Hr %d Day %d Month %d Year %d", cdvd.RTC.second, cdvd.RTC.minute, cdvd.RTC.hour,
                  cdvd.RTC.day, cdvd.RTC.month, cdvd.RTC.year);*/
            ...
            case 0x12:    // sceCdReadILinkId (0:9)
                ...
                if ((!cdvd.Result[3]) &&
                    (!cdvd.Result[4]))    // nvm file is missing correct iLinkId, return hardcoded one
                {
                    cdvd.Result[0] = 0x00;
                    cdvd.Result[1] = 0x00;
                    cdvd.Result[2] = 0xAC;
            ...
            case 0x32:    // sceCdGetMediumRemoval (0:2)
                SetResultSize(2);
                cdvd.Result[0] = 0;
                // cdvd.Result[0] = 0; // fixme: I'm pretty sure that the same variable shouldn't be set twice here.
                // Perhaps cdvd.Result[1]?
                break;
            ...
            case 0x36:    // cdvdman_call189 [__sceCdReadRegionParams - made up name] (0:15) i think it is 16, not 15
                ...
                //			cdvd.Result[4] == OSDVER[4] == CAP			Jjpn, Aeng, Eeng, Heng,
                // Reng, Csch, Kkor? 			cdvd.Result[5] == OSDVER[5] == small
                // cdvd.Result[6] == OSDVER[6] == small 			cdvd.Result[7]
                //== OSDVER[7] == small 			cdvd.Result[8] == VERSTR[0x22] == *0xBFC7FF52
                // cdvd.Result[9] == DVDID J U O E A R C M 			cdvd.Result[10]== 0;
                // //??
                cdvd.Result[11] = 0;    //??
                cdvd.Result[12] = 0;    //??
                //////////////////////////////////////////
            ...
            case 0x38:    // used to fix the MAC back after accidentally trashed it :D [sceCdWriteMAC - made up name]
                          // (8:1)
                SetResultSize(1);
            ...
            case 0x8F:                        // secrman: __mechacon_auth_0x8F
                ...
                if (cdvd.mg_datatype == 1)    // header data
                {
                    ...
                    if ((cdvd.mg_maxsize != cdvd.mg_size) || (cdvd.mg_size < 0x20) ||
                        (cdvd.mg_size != *(u16 *)&cdvd.mg_buffer[0x14])) {
                        fail_pol_cal();
                        break;
                    }
                    ...
                    Console.WriteLn("[MG] ELF_size=0x%X Hdr_size=0x%X unk=0x%X flags=0x%X count=%d zones=%s",
                                    *(u32 *)&cdvd.mg_buffer[0x10], *(u16 *)&cdvd.mg_buffer[0x14],
                                    *(u16 *)&cdvd.mg_buffer[0x16], *(u16 *)&cdvd.mg_buffer[0x18],
                                    *(u16 *)&cdvd.mg_buffer[0x1A], zoneStr.c_str());
            ...
            case 0x90:               // sceMgWriteHeaderStart
                ...
                Console.WriteLn("[MG] hcode=%d cnum=%d a2=%d length=0x%X", cdvd.Param[0], cdvd.Param[3], cdvd.Param[4],
                                cdvd.mg_maxsize = cdvd.Param[1] | (((int)cdvd.Param[2]) << 8));
        ...
        Cpu->ThrowException(Exception::RuntimeError()
                                ...
                                .SetUserMsg(wxString(
                                    L"Failed to read/write NVM/MEC file. Check your BIOS setup/permission settings.")));
    }
}
