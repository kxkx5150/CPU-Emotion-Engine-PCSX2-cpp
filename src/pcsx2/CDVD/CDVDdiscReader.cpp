...
void keepAliveThread()
{
    ...
    while (!s_keepalive_cv.wait_for(guard, std::chrono::seconds(30), []() { return !s_keepalive_is_open; })) {
...
{
    // the formatted subq command returns:  control/adr, track, index, trk min, trk sec, trk frm, 0x00, abs min, abs
    // sec, abs frm
...
CDVD_API CDVDapi_Disc = {
    DISCclose,      DISCopen,        DISCreadTrack,   DISCgetBuffer,     DISCreadSubQ,     DISCgetTN,
    DISCgetTD,      DISCgetTOC,      DISCgetDiskType, DISCgetTrayStatus, DISCctrlTrayOpen, DISCctrlTrayClose,

    DISCnewDiskCB,

    DISCreadSector, DISCgetDualInfo,
};
