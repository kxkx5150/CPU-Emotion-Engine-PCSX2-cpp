...
const wxChar *CDVD_SourceLabels[] = {L"ISO", L"Disc", L"NoDisc", NULL};
...
static int FindDiskType(int mType)
{
    ...
    switch (iCDType) {
        case CDVD_TYPE_DETCTCD:
            Console.WriteLn(" * CDVD Disk Open: CD, %d tracks (%d to %d):", tn.etrack - tn.strack + 1, tn.strack,
                            tn.etrack);
            break;
    ...
    for (int i = tn.strack; i <= tn.etrack; i++) {
        ...
        if (td.type == CDVD_AUDIO_TRACK) {
            ...
            dataTracks++;
            Console.WriteLn(" * * Track %d: Data (Mode %d) (%d sectors)", i, ((td.type == CDVD_MODE1_TRACK) ? 1 : 2),
                            tlength);
        }
    }
...
bool DoCDVDopen()
{
    ...
    int  ret = CDVD->open(!m_SourceFilename[CurrentSourceType].empty() ? m_SourceFilename[CurrentSourceType].c_str()
                                                                       : nullptr);
    if (ret == -1)
    ...
    temp += pxsFmt(L" (%04d-%02d-%02d %02d-%02d-%02d)", curtime.GetYear(), curtime.GetMonth(), curtime.GetDay(),
...
CDVD_API CDVDapi_NoDisc = {
    NODISCclose,      NODISCopen,        NODISCreadTrack,   NODISCgetBuffer,     NODISCreadSubQ, NODISCgetTN,
    NODISCgetTD,      NODISCgetTOC,      NODISCgetDiskType, NODISCgetTrayStatus, NODISCdummyS32, NODISCdummyS32,

    NODISCnewDiskCB,

    NODISCreadSector, NODISCgetDualInfo,
};
